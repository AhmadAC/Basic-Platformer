...
Version 2.1.6 (Refined camera setup timing, robust map change flag handling)
"""
import os
import sys
import time
...
# Ensure main_game package is in path if running this file directly for testing
_APP_GAME_MODES_DIR = os.path.dirname(os.path.abspath(__file__))
if _APP_GAME_MODES_DIR not in sys.path and os.path.basename(_APP_GAME_MODES_DIR) == "main_game":
    sys.path.insert(0, os.path.dirname(_APP_GAME_MODES_DIR)) # Add project root

from main_game.logger import info, debug, warning, error, critical # Corrected import
import main_game.constants as C
import main_game.config as game_config

from main_game.app_ui_creator import ( # Corrected import
    _show_status_dialog,
...
from main_game.camera import Camera # Corrected import
from main_game.level_loader import LevelLoader # Corrected import
# DEFERRED IMPORT: from main_game.game_setup import initialize_game_elements

from network.server_logic import ServerState
from network.client_logic import ClientState, find_server_on_lan

if TYPE_CHECKING:
    from main_game.app_core import MainWindow # Corrected import
else:
    MainWindow = Any # Keep Any for runtime if not type checking to avoid circularity

class LANServerSearchThread(QThread): # No changes needed here
    search_event_signal = Signal(str, object)
    def __init__(self, parent=None):
        super().__init__(parent); self._running = False; self.client_state_for_search = ClientState()
    def run(self):
        self._running = True; self.search_event_signal.emit("searching", "Searching for LAN games..."); info("LAN_SEARCH_THREAD: Actual search started using find_server_on_lan.")
        def search_update_callback(status_key: str, message_data: Any):
            if not self._running: return
            if status_key == "found" and isinstance(message_data, tuple) and len(message_data) == 3: self.search_event_signal.emit(status_key, message_data)
            else: self.search_event_signal.emit(status_key, message_data)
        result_server_info_tuple = find_server_on_lan(self.client_state_for_search, search_update_callback)
        if self._running: self.search_event_signal.emit("final_result", result_server_info_tuple)
        info("LAN_SEARCH_THREAD: Actual search finished."); self._running = False
    def stop_search(self):
        info("LAN_SEARCH_THREAD: Stop requested."); self._running = False
        if hasattr(self.client_state_for_search, 'app_running'): self.client_state_for_search.app_running = False
...
def _initialize_game_entities(main_window: 'MainWindow', map_name_folder_stem: str, mode: str, num_players_for_couch_coop: Optional[int] = None) -> bool:
    info(f"AppGameModes (_initialize_game_entities): Delegating to game_setup.initialize_game_elements for map '{map_name_folder_stem}', mode '{mode}', couch_players: {num_players_for_couch_coop}.")
    
    # --- DEFERRED IMPORT of initialize_game_elements ---
    try:
        from main_game.game_setup import initialize_game_elements
    except ImportError:
        critical("AppGameModes CRITICAL (_initialize_game_entities): Failed to import initialize_game_elements! Game setup will fail.")
        _update_status_dialog(main_window, title="Critical Error", message="Failed to load game setup components.", progress=-1.0)
        return False
    # --- END DEFERRED IMPORT ---

    debug(f"AppGameModes DEBUG (_initialize_game_entities): BEFORE game_setup call, main_window.game_elements['camera'] is type: {type(main_window.game_elements.get('camera'))}, ID: {id(main_window.game_elements.get('camera'))}")

    screen_w = main_window.game_scene_widget.width() if main_window.game_scene_widget and main_window.game_scene_widget.width() > 1 else main_window.width()
    screen_h = main_window.game_scene_widget.height() if main_window.game_scene_widget and main_window.game_scene_widget.height() > 1 else main_window.height()
    ...
    debug(f"AppGameModes DEBUG (_initialize_game_entities): AFTER game_setup call, success: {success}, main_window.game_elements['camera'] is type: {type(main_window.game_elements.get('camera'))}, ID: {id(main_window.game_elements.get('camera'))}")

    if not success:
        error_msg = f"Failed to set up game for map: {map_name_folder_stem}"
        error(f"AppGameModes: game_setup.initialize_game_elements FAILED for map '{map_name_folder_stem}', mode '{mode}'. Message: {error_msg}")
        _update_status_dialog(main_window, title="Map Load Error", message=error_msg, progress=-1.0)
    else:
        info(f"AppGameModes: game_setup.initialize_game_elements SUCCEEDED for map '{map_name_folder_stem}', mode '{mode}'.")
    return success

# --- Map Selection Initiators ---
def initiate_couch_play_map_selection(main_window: 'MainWindow'):
    info(f"AppGameModes: Initiating map selection for Couch Co-op ({main_window.selected_couch_coop_players} players).")
...
def initiate_host_game_map_selection(main_window: 'MainWindow'):
    info("AppGameModes: Initiating map selection for Hosting Game.")
...
def start_couch_play_logic(main_window: 'MainWindow', map_name_folder_stem: str):
    info(f"AppGameModes: Starting Couch Co-op with map '{map_name_folder_stem}' for {main_window.selected_couch_coop_players} players.")
...
def start_host_game_logic(main_window: 'MainWindow', map_name_folder_stem: str):
    info(f"AppGameModes: Starting Host Game with map '{map_name_folder_stem}'.")
...
def initiate_join_lan_dialog(main_window: 'MainWindow'):
    info("AppGameModes: Initiating Join LAN Dialog."); _show_lan_search_dialog(main_window)

def initiate_join_ip_dialog(main_window: 'MainWindow'):
    info("AppGameModes: Initiating Join by IP Dialog.")
    if main_window.ip_input_dialog is None:
        main_window.ip_input_dialog = IPInputDialog(parent=main_window) # IPInputDialog is from game_ui
        ...
        # Ensure buttons_ref is populated if dialog is created here
        ...
        main_window._ip_dialog_buttons_ref = [btn for btn in [ok_button, cancel_button] if btn] # Store valid buttons
...
def prepare_and_start_game_logic(main_window: 'MainWindow', mode: str,
    ...
    info(f"AppGameModes: Preparing game. Mode: {mode}, Map: {map_name_folder_stem}, Target: {target_ip_port}, CouchPlayers: {num_players_for_couch_coop}")

    if not main_window or not hasattr(main_window, 'app_status') or not main_window.app_status.app_running:
        info(f"AppGameModes: Prepare call aborted. App not running or main_window/app_status invalid (Mode: {mode}).")
    ...
    if mode == "couch_play":
        if not map_name_folder_stem: error("Map name required for couch_play."); main_window.show_view("menu"); return
        ...
        _update_status_dialog(main_window, message="Entities initialized.", progress=50.0, title=f"Starting Couch Co-op")
        game_initialized_successfully = True
    elif mode == "host_game":
        if not map_name_folder_stem: error("Map name required for host_game."); main_window.show_view("menu"); return
        ...
        if not _initialize_game_entities(main_window, map_name_folder_stem, mode):
            _close_status_dialog(main_window); main_window.show_view("menu"); return
        _update_status_dialog(main_window, message="Entities initialized.", progress=50.0, title=f"Starting Host Game")
        game_initialized_successfully = True
        ...
        initial_sw = float(main_window.game_scene_widget.width()) if main_window.game_scene_widget and main_window.game_scene_widget.width() > 1 else float(C.GAME_WIDTH)
        initial_sh = float(main_window.game_scene_widget.height()) if main_window.game_scene_widget and main_window.game_scene_widget.height() > 1 else float(C.GAME_HEIGHT)
        main_window.game_elements['camera'] = Camera(initial_sw, 0.0, 0.0, initial_sh, initial_sw, initial_sh)
        debug(f"AppGameModes DEBUG (prepare_and_start for JOIN): Basic camera created. ID: {id(main_window.game_elements.get('camera'))}")
        main_window.game_elements['camera_level_dims_set'] = False # Will be set after map sync
        game_initialized_successfully = True
    else: error(f"Unknown game mode: {mode}"); main_window.show_view("menu"); return

    if not game_initialized_successfully:
        error(f"AppGameModes: Game initialization failed for mode {mode} before final camera setup. Aborting."); _close_status_dialog(main_window); main_window.show_view("menu"); return

    camera = main_window.game_elements.get("camera")
    game_scene_widget = main_window.game_scene_widget
    if not camera or not game_scene_widget:
        critical("AppGameModes CRITICAL: Camera or GameSceneWidget is None after entity init/basic setup!"); _close_status_dialog(main_window); main_window.show_view("menu"); return

    # Ensure screen dimensions are up-to-date on the camera *before* showing the game scene
    # This is important if the window was resized while on menu/map_select
    actual_screen_w = float(game_scene_widget.width()) if game_scene_widget.width() > 1 else float(main_window.width())
    actual_screen_h = float(game_scene_widget.height()) if game_scene_widget.height() > 1 else float(main_window.height())
    debug(f"AppGameModes DEBUG: Setting camera screen dimensions to: {actual_screen_w}x{actual_screen_h} BEFORE show_view.")
    camera.set_screen_dimensions(actual_screen_w, actual_screen_h)

    # Set level dimensions for camera if host/couch (join mode waits for sync)
    if mode in ["couch_play", "host_game"]:
        if main_window.game_elements.get('level_pixel_width') is not None:
            debug(f"AppGameModes DEBUG: Setting camera level dimensions for {mode}.")
        ...
        else: warning(f"AppGameModes WARNING: Level dimensions not set in game_elements for {mode}. Camera bounds may be incorrect.")
    
    main_window.show_view("game_scene")
    QApplication.processEvents() # Allow UI to update
    debug("AppGameModes DEBUG: show_view('game_scene') called and events processed.")

    if mode == "couch_play": _close_status_dialog(main_window); info("AppGameModes: Couch play ready.")
    elif mode == "host_game":
...
def stop_current_game_mode_logic(main_window: 'MainWindow', show_menu: bool = True):
    ...
    info(f"AppGameModes: Stopping current game mode: {current_mode_being_stopped}")
    if main_window.network_thread and main_window.network_thread.isRunning():
        info("AppGameModes: Stopping network thread."); server_state = main_window.server_state; client_state = main_window.client_state
        if server_state: server_state.app_running = False
        if client_state: client_state.app_running = False
        main_window.network_thread.quit()
        if not main_window.network_thread.wait(1000): main_window.network_thread.terminate(); main_window.network_thread.wait(200) # type: ignore
        main_window.network_thread = None; info("AppGameModes: Network thread processing finished.")
    main_window.server_state = None; main_window.client_state = None; main_window.current_game_mode = None
    if 'game_elements' in main_window.__dict__ : # Check if key exists before accessing
        ...
        main_window.game_elements.clear(); info("AppGameModes: Cleared all game_elements.")
    _close_status_dialog(main_window)
    ...
    info(f"AppGameModes: Game mode '{current_mode_being_stopped}' stopped and resources cleaned up.")
...
def start_network_mode_logic(main_window: 'MainWindow', mode_name: str, target_ip_port: Optional[str] = None, map_to_host: Optional[str] = None):
    info(f"AppGameModes: Starting network mode: {mode_name}, MapToHost: {map_to_host}")
    if main_window.network_thread and main_window.network_thread.isRunning(): warning("AppGameModes: Network thread already running. Stopping existing one first."); main_window.network_thread.quit(); main_window.network_thread.wait(500); main_window.network_thread = None
    ...
    if mode_name == "host":
        ...
        main_window.server_state.current_map_name = server_map_name_to_use; debug(f"AppGameModes (Host): Server starting with map: {server_map_name_to_use}")
        ...
        if not target_ip_port: error("AppGameModes (Join): Target IP:Port required for join mode."); _update_status_dialog(main_window, title="Connection Error", message="No target IP specified.", progress=-1.0); return
    ...
    else: error(f"AppGameModes: Unknown network mode specified: {mode_name}"); return
    if main_window.network_thread: main_window.network_thread.status_update_signal.connect(main_window.on_network_status_update_slot); main_window.network_thread.operation_finished_signal.connect(main_window.on_network_operation_finished_slot); main_window.network_thread.client_fully_synced_signal.connect(main_window.on_client_fully_synced_for_host); main_window.network_thread.start(); info(f"AppGameModes: NetworkThread for '{mode_name}' started.")
    else: error(f"AppGameModes: Failed to create NetworkThread for mode '{mode_name}'.")

def on_client_fully_synced_for_host_logic(main_window: 'MainWindow'):
    info("AppGameModes (Host): Client fully synced. Transitioning game to active state.")
    if main_window.current_game_mode == "host_waiting" and main_window.server_state: main_window.current_game_mode = "host_active"; main_window.server_state.client_ready = True; _close_status_dialog(main_window); info("AppGameModes (Host): Client connected and synced. Game is now fully active for host.")
    else: warning(f"AppGameModes: Received client_fully_synced_for_host in unexpected state: {main_window.current_game_mode}")

def on_network_status_update_logic(main_window: 'MainWindow', title: str, message: str, progress: float):
    debug(f"AppGameModes (Net Status Update): Title='{title}', Msg='{message}', Prog={progress}"); is_network_setup_phase = main_window.current_game_mode in ["host_waiting", "join_ip", "join_lan"]
    if progress == -2.0: _close_status_dialog(main_window); return # Special code to just close
    ...
    if main_window.current_view_name == "game_scene" and (main_window.current_game_mode == "join_active" or main_window.current_game_mode == "host_active"):
        if hasattr(main_window.game_scene_widget, 'update_game_state'):
            ...
            else: main_window.game_scene_widget.update_game_state(0) # General repaint
    if (title == "Client Map Sync" and "Map ready, starting game" in message and progress >= 99.9) or \
       (title == "Server Hosting" and "Ready for game start" in message and progress >= 99.9 and main_window.current_game_mode == "host_waiting"):
        if main_window.current_game_mode in ["join_lan", "join_ip"]: main_window.current_game_mode = "join_active"; info("AppGameModes (Client): Map synced and server signaled start. Game active.")
        _close_status_dialog(main_window)

def on_network_operation_finished_logic(main_window: 'MainWindow', result_message: str):
    info(f"AppGameModes: Network operation finished: {result_message}"); _close_status_dialog(main_window); current_mode_that_finished = str(main_window.current_game_mode)
    stop_current_game_mode_logic(main_window, show_menu=False) # Don't show menu yet
    ...
    main_window.show_view("menu") # Now show menu

_lan_search_thread_instance: Optional[LANServerSearchThread] = None # Keep track of the thread
def on_lan_server_search_status_update_logic(main_window: 'MainWindow', data_tuple: Any):
    ...
    if status_key == "searching": main_window.lan_search_status_label.setText(str(data))
        ...
        msg = str(data) if data else f"Search {status_key}."; main_window.lan_search_status_label.setText(f"{msg} No servers found." if main_window.lan_servers_list_widget.count() == 0 else f"{msg} Select a server or retry.")
        ...
        if data is None and main_window.lan_servers_list_widget.count() == 0: main_window.lan_search_status_label.setText("Search complete. No servers found.")
        elif data is not None and isinstance(data, tuple) and len(data) == 2: # Should be 3 if map_name is included
            ip, port = data[0], data[1]; map_name_final = data[2] if len(data) > 2 else "Map via TCP"
            item_text_final = f"Server at {ip}:{port} (Map: {map_name_final})"; found_existing_final = False
            for i in range(main_window.lan_servers_list_widget.count()):
            ...
            if not found_existing_final:
                list_item_final = QListWidgetItem(item_text_final); list_item_final.setData(Qt.ItemDataRole.UserRole, (ip, port, map_name_final));
...
def start_lan_server_search_thread_logic(main_window: 'MainWindow'):
    ...
    info("AppGameModes: Starting LAN server search thread.")
    if _lan_search_thread_instance and _lan_search_thread_instance.isRunning():
        info("LAN search already running. Stopping and restarting."); _lan_search_thread_instance.stop_search();
        if not _lan_search_thread_instance.wait(500): _lan_search_thread_instance.terminate(); _lan_search_thread_instance.wait(100)
    _lan_search_thread_instance = None # Clear old instance
    ...
    if hasattr(main_window.lan_search_dialog, 'button_box'): main_window.lan_search_dialog.button_box.button(QDialogButtonBox.StandardButton.Ok).setEnabled(False) # type: ignore
    
    _lan_search_thread_instance = LANServerSearchThread(main_window) # Create new instance
    _lan_search_thread_instance.search_event_signal.connect(main_window.on_lan_server_search_status_update_slot)
    _lan_search_thread_instance.finished.connect(_lan_search_thread_instance.deleteLater) # Ensure cleanup
    _lan_search_thread_instance.start()

def join_selected_lan_server_from_dialog_logic(main_window: 'MainWindow'):
    info("AppGameModes: Attempting to join selected LAN server.");
