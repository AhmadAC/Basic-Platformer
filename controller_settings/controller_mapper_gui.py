...
# -*- coding: utf-8 -*-
"""
## version 1.0.1 (Asset Path Refactor Verified)
GUI for mapping game controller inputs to game actions and GUI navigation.
- Saves mappings to a JSON file defined by game_config.
- Simulates keyboard events using pynput if available.
- Handles Pygame joystick events for mapping and GUI control.
- Verified that its dependencies (config files, main_game.config import)
  are not broken by the top-level 'assets/' folder refactor.
"""
...
if __name__ == "__main__":
    # When running this script directly (e.g., for testing the GUI standalone)
...
try:
    ...
    print("CRITICAL ERROR in controller_mapper_gui.py: Could not import 'main_game.config as game_config'. Using fallback GameConfigFallback.")
    class GameConfigFallback:
        ...
        MAPPINGS_AND_DEVICE_CHOICES_FILE_PATH = "controller_mappings_fallback.json" # Saved in controller_settings/
        ...
        DEFAULT_GENERIC_JOYSTICK_MAPPINGS: Dict[str, Any] = { # Runtime format
            ...
            GUI_NAV_UP: {"type": "hat", "id": 0, "value": (0, 1)},
            GUI_NAV_DOWN: {"type": "hat", "id": 0, "value": (0, -1)},
            GUI_NAV_LEFT: {"type": "hat", "id": 0, "value": (-1, 0)},
            GUI_NAV_RIGHT: {"type": "hat", "id": 0, "value": (1, 0)},
            GUI_NAV_CONFIRM: {"type": "button", "id": 0},
            GUI_NAV_CANCEL: {"type": "button", "id": 1},
        }
        ...
        def init_pygame_and_joystick_globally(force_rescan=False):
            print("GameConfigFallback: Pygame Init Triggered")
            ...
            print(f"GameConfigFallback: Pygame initialized. Joystick count: {pygame.joystick.get_count() if GameConfigFallback._joystick_initialized_globally else 'N/A'}")
        ...
        def get_available_joystick_names_with_indices_and_guids() -> List[Tuple[str, str, Optional[str], int]]:
            ...
            for i in range(pygame.joystick.get_count()):
                try:
                    ...
                    # Construct internal_id similar to how main_game.config might
                    internal_id_for_assignment = f"joystick_pygame_{i}" # Use index for assignment
                    display_name = f"Joy {i}: {name}"
                    joysticks_data.append((display_name, internal_id_for_assignment, guid, i))
                except pygame.error as e:
                    print(f"GameConfigFallback: Error getting joystick {i}: {e}")
                    continue
            return joysticks_data
        
        @staticmethod
        def get_joystick_objects() -> List[Any]: 
            return [pygame.joystick.Joystick(i) for i in range(pygame.joystick.get_count())] if GameConfigFallback._joystick_initialized_globally else []
        
        @staticmethod
        def save_config():
            print("GameConfigFallback: save_config called.")
            data_to_save = {
                "joystick_mappings": GameConfigFallback.LOADED_PYGAME_JOYSTICK_MAPPINGS,
                "player_devices": {} # Using a nested dict for player_devices now
            }
            for i in range(1, 5):
            ...
            try:
                # Save fallback in the same directory as the script for simplicity
                fallback_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), GameConfigFallback.MAPPINGS_AND_DEVICE_CHOICES_FILE_PATH)
                with open(fallback_path, 'w') as f:
                    json.dump(data_to_save, f, indent=4)
                return True
            except Exception as e:
                print(f"GameConfigFallback save_config error: {e}")
                return False

        @staticmethod
        def load_config():
            print("GameConfigFallback: load_config called.")
            try:
                fallback_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), GameConfigFallback.MAPPINGS_AND_DEVICE_CHOICES_FILE_PATH)
                ...
                for i in range(1, 5):
                    p_settings = player_devices_loaded.get(f"P{i}", {}) # Get settings for "P1", "P2", etc.
                    ...
                    setattr(GameConfigFallback, f"P{i}_KEYBOARD_ENABLED", p_settings.get("kbd_enabled", i <= 2)) # Default P1/P2 kbd
                    setattr(GameConfigFallback, f"P{i}_CONTROLLER_ENABLED", p_settings.get("ctrl_enabled", False))
            except FileNotFoundError:
                print("GameConfigFallback: Mappings file not found, using defaults.")
                GameConfigFallback.LOADED_PYGAME_JOYSTICK_MAPPINGS = {}
                for i in range(1, 5): # Set defaults if file not found
                    setattr(GameConfigFallback, f"CURRENT_P{i}_INPUT_DEVICE", getattr(GameConfigFallback, f"DEFAULT_P{i}_INPUT_DEVICE"))
                    setattr(GameConfigFallback, f"P{i}_KEYBOARD_ENABLED", i <= 2) # P1/P2 default to keyboard
                    setattr(GameConfigFallback, f"P{i}_CONTROLLER_ENABLED", False)
            except Exception as e:
                print(f"GameConfigFallback load_config error: {e}")
                # Optionally, set defaults on any error
                GameConfigFallback.LOADED_PYGAME_JOYSTICK_MAPPINGS = {}
                for i in range(1, 5):
                    setattr(GameConfigFallback, f"CURRENT_P{i}_INPUT_DEVICE", getattr(GameConfigFallback, f"DEFAULT_P{i}_INPUT_DEVICE"))
        ...
        @staticmethod
        def update_player_mappings_from_config(): print("GameConfigFallback: update_player_mappings_from_config called")
        
        @staticmethod
        def _translate_and_validate_gui_json_to_pygame_mappings(raw_gui_json_joystick_mappings: Dict[str, Any]) -> Dict[str, Any]:
            print("GameConfigFallback: _translate_and_validate_gui_json_to_pygame_mappings called")
...
MENU_SPECIFIC_ACTIONS = ["menu_confirm", "menu_cancel", "menu_up", "menu_down", "menu_left", "menu_right", "pause"] + GUI_NAV_ACTIONS
...
def get_pynput_key(key_str: str) -> Optional[Any]:
    ...
    key_map = {
        ...
        # Game action to pynput key (example, assuming these are in game_config.GAME_ACTIONS)
    ...
    if key_str not in GUI_NAV_ACTIONS: logger_cmg.warning(f"Pynput key for game action '{key_str}' not found or not typically simulated.")
    return None
            ...
            try:
                ...
                for event in pygame.event.get():
                    ...
                    if self.is_listening_for_mapping and event_details:
                        event_details["type"] = event_details.get("type") # Ensure 'type' key exists
...
class ControllerSettingsWindow(QWidget):
    def __init__(self, parent=None):
        ...
        self.all_joystick_mappings_by_guid: Dict[str, Dict[str, Any]] = {} # GUID -> {action: gui_map_entry}
        self.current_translated_mappings_for_thread_sim: Dict[str, Any] = {} # action -> runtime_map_entry
        self.current_listening_key_for_joystick_map: Optional[str] = None
        self.last_selected_row_for_joystick_mapping = -1
        # UI settings file will be in controller_settings/controller_gui_ui_settings.ini
        ...
        # Only load settings and activate monitoring if it's the main window (not embedded)
        if parent is None: # Assuming parent=None means it's the top-level widget
            self.load_settings_into_ui()
            self.activate_controller_monitoring()
    ...
    def _setup_ui(self):
        ...
        main_layout.addWidget(self.main_splitter, 1) # Give splitter more space
        ...
        for i in range(4): # P1 to P4
            ...
            dev_combo.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        ...
        self.mappings_table = QTableWidget(); self.mappings_table.setColumnCount(8) # 2 sets of (Action, Input, RenameBtn, ClearBtn)
        ...
        self.mappings_table.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        ...
        self.joystick_select_combo_for_mapping.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        ...
        self.key_to_map_combo.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        for ik_idx, ik in enumerate(MAPPABLE_KEYS): # MAPPABLE_KEYS includes GUI actions
        ...
        self.listen_button.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        ...
        self.cancel_listen_button.setFocusPolicy(Qt.FocusPolicy.StrongFocus) # For keyboard/controller nav
        ...
        self.load_splitter_sizes() # Load AFTER splitter is added to layout and has parent
    ...
    def _populate_focusable_widgets_list(self):
        ...
        # Order matters for Tab navigation if not using advanced layout navigation
        ...
        if self.joystick_select_combo_for_mapping.count() > 0 and self.joystick_select_combo_for_mapping.itemData(0) is not None:
            self.set_current_focus_widget(self.joystick_select_combo_for_mapping)
             ...
             if first_valid_widget:
                 self.set_current_focus_widget(first_valid_widget)
    ...
    def on_monitor_joystick_changed(self, index_in_combo: int):
        ...
        if pygame_joy_idx is not None and current_guid:
            ...
            if current_guid not in self.all_joystick_mappings_by_guid:
                ...
                # Try to find any other controller's mappings to use as a base
                ...
                if source_guid_for_fallback:
                    ...
                    # Convert default generic runtime mappings to GUI storage format
                    ...
                    else: self.all_joystick_mappings_by_guid[current_guid] = {} # Ensure dict exists
                else: self.all_joystick_mappings_by_guid[current_guid] = {} # Ensure dict exists
    ...
    def update_simulation_thread_mappings(self, raw_gui_mappings: Dict[str, Any]):
        ...
        else: # Fallback if the method is missing (should not happen with the GameConfigFallback class)
            logger_cmg.error("Config module is missing '_translate_and_validate_gui_json_to_pygame_mappings' function. Using internal fallback for translation.")
            for action_key, map_info_gui in raw_gui_mappings.items(): # Basic manual translation as a last resort
                if isinstance(map_info_gui, dict):
                    ...
                    if runtime_map_entry.get("id") is not None: # Basic validation
    ...
    def load_splitter_sizes(self):
        if hasattr(self, 'main_splitter') and self.main_splitter and self.main_splitter.count() == 3:
            # Default to proportional sizes if no saved setting or if saved is invalid
            ...
            if sizes_to_apply and sum(sizes_to_apply) > 50 : # Basic sanity check sum
                self.main_splitter.setSizes(sizes_to_apply)
    ...
    def refresh_joystick_mappings_table(self, current_controller_guid: Optional[str], preserve_scroll=False, target_row_action_key: Optional[str] = None):
        ...
        num_actions = len(MAPPABLE_KEYS); num_rows_needed = (num_actions + 1) // 2 # Two columns of actions
        self.mappings_table.setRowCount(num_rows_needed); target_table_row_to_ensure_visible = -1
        min_button_width = 70
        for i in range(num_rows_needed):
            for col_group in range(2): # 0 for left column group, 1 for right
                action_idx = i * 2 + col_group; # Calculate index in MAPPABLE_KEYS
                if action_idx >= num_actions: continue # No more actions to display
                ...
                base_col = col_group * 4 # Each group has 4 table columns (Action, Input, Rename, Clear)
        ...
        if self.current_listening_key_for_joystick_map == target_row_action_key : self.current_listening_key_for_joystick_map = None # Clear if it was just mapped/refreshed
    ...
    def save_all_settings(self):
        ...
        for i in range(4): # P1 to P4
            player_idx = i # 0-indexed for list access
            ...
            elif hasattr(game_config, default_device_attr): # Fallback to default if combo doesn't exist (should not happen)
    ...
    def load_settings_into_ui(self):
        logger_cmg.info("Loading settings into UI..."); game_config.load_config() # This might trigger its own init_pygame
        ...
        for i in range(len(self.player_device_combos)):
            ...
            combo.setCurrentIndex(idx if idx != -1 else 0) # Default to first item (Unassigned) if not found
    ...
    def perform_reset_all_settings(self):
        ...
        for i in range(4): # P1 to P4
            player_idx = i # 0-indexed
            default_device_attr = f"DEFAULT_P{player_idx+1}_INPUT_DEVICE"
            if hasattr(game_config, default_device_attr):
                setattr(game_config, f"CURRENT_P{player_idx+1}_INPUT_DEVICE", getattr(game_config, default_device_attr))
            else: setattr(game_config, f"CURRENT_P{player_idx+1}_INPUT_DEVICE", UNASSIGNED_DEVICE_ID if i >= 2 else (f"keyboard_p{i+1}")) # Fallback if config doesn't have defaults
    ...
    def log_to_debug_console(self, message: str):
        if hasattr(self, 'debug_console'): # Ensure it's initialized
    ...
    def populate_joystick_device_combos(self):
        ...
        for i, player_combo in enumerate(self.player_device_combos):
            ...
            if hasattr(game_config, 'KEYBOARD_DEVICE_IDS') and hasattr(game_config, 'KEYBOARD_DEVICE_NAMES'):
                for k_idx, k_id in enumerate(game_config.KEYBOARD_DEVICE_IDS):
                    if k_id == UNASSIGNED_DEVICE_ID: continue # Unassigned already added
            ...
            else: # Fallback if these attributes are missing from config
                player_combo.addItem("Keyboard (P1)", "keyboard_p1"); player_combo.addItem("Keyboard (P2)", "keyboard_p2")
            ...
            player_combo.setCurrentIndex(idx_to_set if idx_to_set != -1 else 0) # Default to "Unassigned"
        ...
        if not available_joysticks_full_data:
            ...
            for joy_display_name, _, joy_guid_str, pygame_joy_idx in available_joysticks_full_data:
                name_part = joy_display_name.split(': ', 1)[-1].split(' (GUID:')[0] # Clean up name
    ...
    def initiate_listening_sequence_for_joystick_map(self, internal_key_to_map_str: str, originating_row_idx_in_table:int = -1):
        ...
        self.set_current_focus_widget(self.cancel_listen_button)
    ...
    def on_controller_event_captured_for_mapping(self, event_details_from_thread: dict, raw_event_str: str):
        ...
        if perform_conflict_check:
            for existing_key, mapping_info in list(current_controller_mappings_gui_format.items()):
                if existing_key in MENU_SPECIFIC_ACTIONS: continue # Allow menu actions to share inputs
    ...
    def on_mapped_event_triggered_for_simulation(self, internal_action_key_str: str, is_press_event: bool):
        if internal_action_key_str in GUI_NAV_ACTIONS: return # GUI Nav actions are handled by guiNavigationRequested
...
if __name__ == "__main__":
    ...
    def on_main_window_close(event): 
        ...
        settings_widget.closeEvent(event) 
        event.accept()
