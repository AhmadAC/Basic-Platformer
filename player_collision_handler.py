...
"""
# version 2.0.8 (Chest pushing refinement, ensure player stops when pushing)
...
from items import Chest # Added this import
...
def check_player_character_collisions(player: 'PlayerClass_TYPE', direction: str, characters_list: List[Any]) -> bool:
    ...
    for other_char in characters_list:
        ...
        # General validity check for non-chest characters
        if not is_chest and \
        ...
        if player.rect.intersects(other_char.rect):
            ...
            if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED:
                log_player_physics(player, f"CHAR_COLL_CHECK DIR:{direction}", (current_player_rect_for_log, QRectF(other_char.rect), getattr(other_char, 'player_id', getattr(other_char, 'enemy_id', getattr(other_char, 'statue_id', 'UnknownChar')))))
            collision_occurred_this_axis = True

            if is_chest:
                chest_obj = other_char # type: Chest
                if chest_obj.state == 'closed': # Only pushable if closed
                    if direction == 'x':
                        push_force_direction = 0
                        # Player pushing chest to the right
                        if player.vel.x() > 0.05 and player.rect.right() > chest_obj.rect.left() and player.rect.center().x() < chest_obj.rect.center().x():
                            push_force_direction = 1
                            player.rect.moveRight(chest_obj.rect.left() - 0.1) # Player stops just before chest
                        # Player pushing chest to the left
                        elif player.vel.x() < -0.05 and player.rect.left() < chest_obj.rect.right() and player.rect.center().x() > chest_obj.rect.center().x():
                            push_force_direction = -1
                            player.rect.moveLeft(chest_obj.rect.right() + 0.1) # Player stops just before chest

                        if push_force_direction != 0:
                            chest_obj.acc_x = C.CHEST_PUSH_ACCEL_BASE * push_force_direction * player.vel.x() / C.PLAYER_RUN_SPEED_LIMIT # Scale push by player speed
                            player.vel.setX(0) # Player stops when pushing chest
                            if _SCRIPT_LOGGING_ENABLED: debug(f"Player {player.player_id} pushing chest. Chest acc_x set to {chest_obj.acc_x:.2f}")
                        
                        player.pos.setX(player.rect.center().x())
                    elif direction == 'y': # Vertical collision with chest
                        if player.vel.y() > 0 and player.rect.bottom() > chest_obj.rect.top() and \
                           current_player_rect_for_log.bottom() <= chest_obj.rect.top() + 1: # Landed on chest
                            player.rect.moveBottom(chest_obj.rect.top())
                            player.on_ground = True
                            player.vel.setY(0)
                        elif player.vel.y() < 0 and player.rect.top() < chest_obj.rect.bottom() and \
                             current_player_rect_for_log.top() >= chest_obj.rect.bottom() -1: # Hit chest from below
                            player.rect.moveTop(chest_obj.rect.bottom())
                            player.vel.setY(0)
                        player.pos.setY(player.rect.bottom())

                continue # Handled chest interaction for this axis, move to next character

            # Logic for Enemy/Statue/Other Player collisions
            ...
            if is_enemy and getattr(player, 'is_aflame', False) and hasattr(other_char, 'apply_aflame_effect') and callable(other_char.apply_aflame_effect) and not getattr(other_char, 'is_aflame', False) and not getattr(other_char, 'is_deflaming', False) and not getattr(other_char, 'is_frozen', False) and not getattr(other_char, 'is_defrosting', False) and not getattr(other_char, 'is_petrified', False):
                ...
                if _SCRIPT_LOGGING_ENABLED: debug(f"Player {player.player_id} (aflame) touched Enemy {other_char_id_log}. Igniting.")
                other_char.apply_aflame_effect()
                continue

            is_enemy_stompable = is_enemy and not other_char.is_dead and not getattr(other_char, 'is_stomp_dying', False) and not getattr(other_char, 'is_aflame', False) and not getattr(other_char, 'is_frozen', False) and not getattr(other_char, 'is_petrified', False)
                ...
                if rect_before_y_move_bottom <= target_stomp_effective_top_y + 1.0 and \
                   ...
                   player.rect.bottom() <= other_char.rect.top() + (other_char.rect.height() * 0.50): # Landed on upper half
                    ...
                    if is_enemy_stompable and hasattr(other_char, 'stomp_kill') and callable(other_char.stomp_kill):
                        ...
                        other_char.take_damage(999); stomp_processed_successfully = True # Assuming high damage smashes statue
                    ...
                    if stomp_processed_successfully:
                        ...
                        player.rect.moveBottom(other_char.rect.top() - 1.0) # Position slightly above
                        player.pos = QPointF(player.rect.center().x(), player.rect.bottom()) # Sync pos
                        if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED: log_player_physics(player, f"CHAR_COLL_STOMP_{type(other_char).__name__}", (QRectF(player.rect), current_player_rect_for_log, (player.pos.x(),player.pos.y()), player.vel.y(), f'y_stomp'))
                    return True # Stomp handled, don't do other pushback

            # Default pushback if not a stomp or chest interaction
            ...
            if direction == 'x':
                if is_attacking_self and not is_other_petrified_solid:
                ...
                else: # General collision pushback
                    ...
                    if player.rect.center().x() < other_char.rect.center().x(): # Player is to the left
                        player.rect.moveRight(other_char.rect.left() - 0.1); push_dir_self = -1
                    else: # Player is to the right
                        player.rect.moveLeft(other_char.rect.right() + 0.1); push_dir_self = 1
                    ...
                    if hasattr(other_char, 'pos') and hasattr(other_char, 'rect') and can_push_other_char:
                        ...
                        if hasattr(other_char, '_update_rect_from_image_and_pos'):
                ...
                player.pos.setX(player.rect.center().x()) # Sync player pos
                if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED: log_player_physics(player, f"CHAR_COLL_X_RESOLVED", (QRectF(player.rect), current_player_rect_for_log, (player.pos.x(),player.pos.y()), player.vel.x(), 'x_push/stop'))
            
            elif direction == 'y': # Vertical pushback for non-stomp, non-chest
                if player.vel.y() > 0 and player.rect.bottom() > other_char.rect.top() and \
                   player.rect.center().y() < other_char.rect.center().y(): # Player landing on other
                    ...
                    player.vel.setY(0.0)
                player.pos.setY(player.rect.bottom()) # Sync player pos
                if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED: log_player_physics(player, f"CHAR_COLL_Y_STOP", (QRectF(player.rect), current_player_rect_for_log, (player.pos.x(),player.pos.y()), player.vel.y(), 'y_char_stop'))
...
def check_player_hazard_collisions(player: 'PlayerClass_TYPE', hazards_list: List[Any]):
    ...
    for hazard_obj in hazards_list:
        ...
        if isinstance(hazard_obj, Lava):
            ...
            actual_overlap_width = min(player.rect.right(), hazard_obj.rect.right()) - \
                                   max(player.rect.left(), hazard_obj.rect.left())

            if player_feet_in_lava and actual_overlap_width >= min_horizontal_hazard_overlap:
                if not damaged_this_frame_by_hazard:
                    if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED: log_player_physics(player, f"HAZARD_COLL_LAVA", (QRectF(player.rect), QRectF(hazard_obj.rect)))
                    ...
                    if not player.is_dead: # If still alive after lava damage
