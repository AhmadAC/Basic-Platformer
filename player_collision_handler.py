...
def check_player_ladder_collisions(player, ladders_list: List[Any]):
    if not player._valid_init: return
        ...
        if player.rect.intersects(other_char.rect):
            ...
            if direction == 'x':
                if is_attacking_self and not is_other_petrified_solid:
                    if player.vel.x() > 0 : player.rect.moveRight(other_char.rect.left())
                    elif player.vel.x() < 0 : player.rect.moveLeft(other_char.rect.right())
                    player.vel.setX(0.0)
                    player.pos = QPointF(player.rect.center().x(), player.rect.bottom())
                    log_player_physics(player, f"CHAR_COLL_ATTACK_X_STOP", (QRectF(player.rect), current_player_rect_for_log, (player.pos.x(),player.pos.y()), player.vel.x(), 'x_attack_stop'))
                else:
                    ...
                    if hasattr(other_char, 'vel') and hasattr(other_char.vel, 'setX') and can_push_other: # Check if vel has setX
                    ...
                    player.pos = QPointF(player.rect.center().x(), player.rect.bottom())
                    log_player_physics(player, f"CHAR_COLL_X_PUSHBACK", (QRectF(player.rect), current_player_rect_for_log, (player.pos.x(),player.pos.y()), player.vel.x(), 'x_push'))
...
def check_player_hazard_collisions(player, hazards_list: List[Any]):
    current_time_ms = get_current_ticks() # Use the defined function
    ...
    collided_hazards = [h for h in hazards_list if hasattr(h, 'rect') and player.rect.intersects(h.rect)]
        ...
        if isinstance(hazard_obj, Lava):
            ...
            if player_feet_in_lava and actual_overlap_width >= min_horizontal_hazard_overlap:
                if not damaged_this_frame:
                    log_player_physics(player, f"HAZARD_COLL_LAVA", (QRectF(player.rect), hazard_obj.rect))
                    if hasattr(player, 'apply_aflame_effect'): player.apply_aflame_effect()
