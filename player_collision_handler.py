...
"""
# version 2.0.10 (Defined _SCRIPT_LOGGING_ENABLED)
...
# --- Per-script logging toggle for this file ---
_SCRIPT_LOGGING_ENABLED = True # SET TO True or False AS NEEDED FOR THIS MODULE'S LOGS
...
try:
    ...
    ENABLE_DETAILED_PHYSICS_LOGS = False # Assume False if logger fails to import
...
def check_player_platform_collisions(player: 'PlayerClass_TYPE', direction: str, platforms_list: List[Any]):
    ...
    if not hasattr(player, 'rect') or not isinstance(player.rect, QRectF) or not player.rect.isValid():
        # Use the locally defined _SCRIPT_LOGGING_ENABLED
    ...
    for platform_obj in platforms_list:
        ...
        if player.rect.intersects(platform_obj.rect):
            if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED: # Check both flags
            ...
            if direction == 'x':
                ...
                # Logging for resolved X collision
                ...
                if player.vel.y() >= 0:
                    ...
                    if overlap_y > 0:
                        ...
                        if actual_h_overlap >= min_h_overlap_pixels:
                            ...
                            if ENABLE_DETAILED_PHYSICS_LOGS and _SCRIPT_LOGGING_ENABLED:
                                 log_player_physics(player, f"PLAT_COLL_Y_LANDED")
                elif player.vel.y() < 0:
...
def check_player_ladder_collisions(player: 'PlayerClass_TYPE', ladders_list: List[Any]):
    # (Code remains the same as previous version, just ensure _SCRIPT_LOGGING_ENABLED is used locally if needed)
    if not player._valid_init: return
    ...
    for other_char in characters_list:
        ...
        if player.rect.intersects(other_char.rect):
            ...
            if (is_enemy_stompable or is_statue_stompable) and direction == 'y' and player.vel.y() > 0.5:
                ...
                if rect_before_char_coll_resolve.bottom() <= target_stomp_effective_top_y + 1.0 and player.rect.bottom() >= other_char.rect.top() and player.rect.bottom() <= other_char.rect.top() + (other_char.rect.height() * 0.50):
                    stomp_processed = False
                    if is_enemy_stompable and hasattr(other_char, 'stomp_kill'): other_char.stomp_kill(); stomp_processed = True
                    elif is_statue_stompable and hasattr(other_char, 'take_damage'): other_char.take_damage(999); stomp_processed = True
                    if stomp_processed:
            ...
            if direction == 'x':
                ...
                overlap_y_char = 0.0
                if player.vel.y() > 0 and player.rect.bottom() > other_char.rect.top() and player.rect.center().y() < other_char.rect.center().y():
                    overlap_y_char = player.rect.bottom() - other_char.rect.top()
                    if overlap_y_char > 0: player.rect.translate(0, -overlap_y_char); player.on_ground = True; player.vel.setY(0.0)
                elif player.vel.y() < 0 and player.rect.top() < other_char.rect.bottom() and player.rect.center().y() > other_char.rect.center().y():
                    overlap_y_char = other_char.rect.bottom() - player.rect.top()
                    if overlap_y_char > 0: player.rect.translate(0, overlap_y_char); player.vel.setY(0.0)
                if hasattr(player, 'pos'): player.pos.setY(player.rect.bottom())
...
def check_player_hazard_collisions(player: 'PlayerClass_TYPE', hazards_list: List[Any]):
    ...
    if not player._valid_init or player.is_dead or not player.alive() or \
       ...
       player.is_petrified or player.is_frozen: return
    if not hasattr(player, 'rect') or not isinstance(player.rect, QRectF): return
