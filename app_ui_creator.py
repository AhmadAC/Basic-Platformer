...
def _populate_map_list_for_selection(main_window: 'MainWindow', purpose: str):
    ...
    if not os.path.isabs(maps_dir): maps_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), maps_dir) # Assumes app_core is one level down
    available_maps = [];
...
def _ensure_editor_instance(main_window: 'MainWindow'):
    from PySide6.QtWidgets import QMainWindow as EditorMainWindowType # Local import
    ...
    if main_window.actual_editor_module_instance is None or \
       not isinstance(main_window.actual_editor_module_instance, EditorMainWindowType):
        ...
        try:
            from editor.editor import editor_main # Local import
...
def _ensure_controls_settings_instance(main_window: 'MainWindow'):
    from controller_settings.controller_mapper_gui import ControllerSettingsWindow # Local import
    if main_window.actual_controls_settings_instance and \
       main_window.actual_controls_settings_instance.parent() is main_window.settings_content_container:
        ...
        if hasattr(main_window.actual_controls_settings_instance, 'activate_controller_monitoring'):
            main_window.actual_controls_settings_instance.activate_controller_monitoring()
        return
    ...
    if main_window.actual_controls_settings_instance is None or \
       not isinstance(main_window.actual_controls_settings_instance, ControllerSettingsWindow) :
        ...
        try:
            game_config.load_config() # Ensure config is loaded before creating settings UI
    ...
    else: # If instance creation failed
...
def _poll_pygame_joysticks_for_ui_navigation(main_window: 'MainWindow'):
    ...
    # Use main_window._pygame_joysticks which should be populated by app_core._refresh_appcore_joystick_list
    ...
    if current_time - main_window._last_pygame_joy_nav_time < 0.18: # Slightly reduced delay for responsiveness
        return

    navigated_this_poll = False
    action_input_source: Optional[str] = None

    # Iterate through the joysticks available to app_core/MainWindow
    # main_window._pygame_joysticks should be a list of Pygame Joystick objects
    for ui_controller_idx, joy in enumerate(main_window._pygame_joysticks):
        if ui_controller_idx >= 4: break # Limit to P1-P4 for UI navigation

        if not joy or not joy.get_init(): # Ensure joy object exists and is initialized
            # Attempt to initialize if not already (app_core might also do this)
            if joy:
                try: joy.init()
                except pygame.error: continue # Skip if init fails
            if not joy or not joy.get_init(): continue


        joy_instance_id = joy.get_instance_id()
        # Ensure prev_state buffer is large enough
        while joy_instance_id >= len(main_window._pygame_joy_button_prev_state):
            main_window._pygame_joy_button_prev_state.append({})


        # --- Hat and Axis Navigation (Remains largely the same) ---
        ...
        if joy.get_numhats() > JOY_NAV_HAT_ID:
            ...
            if nav_dir_hat != 0:
                ...
                if active_ui_element == "map_select":
                    if nav_dir_hat == 2: grid_nav_val = 1 # Map D-Pad Right to simple column right
                    elif nav_dir_hat == -2: grid_nav_val = -1 # Map D-Pad Left to simple column left
                elif active_ui_element == "ip_input" and nav_dir_hat not in [-2, 2]: # IP dialog only horizontal
                    continue
        ...
        # --- Button Press Logic (Updated to use per-player maps) ---
        ...
        # Determine which player this joystick (by ui_controller_idx) corresponds to
        # and get their specific runtime mappings.
        joy_mappings_to_use: Dict[str, Any] = {}
        player_assigned_to_this_joy: Optional[str] = None

        for p_idx in range(1, 5): # Check P1 to P4
            player_device_setting = getattr(game_config, f"CURRENT_P{p_idx}_INPUT_DEVICE", game_config.UNASSIGNED_DEVICE_ID)
            expected_joy_device_id = f"joystick_pygame_{ui_controller_idx}"
            if player_device_setting == expected_joy_device_id and getattr(game_config, f"P{p_idx}_CONTROLLER_ENABLED", False):
                joy_mappings_to_use = getattr(game_config, f"P{p_idx}_MAPPINGS", {})
                player_assigned_to_this_joy = f"P{p_idx}"
                # debug(f"UI Poll: Using {player_assigned_to_this_joy} mappings for UI joystick index {ui_controller_idx}")
                break # Found the player assigned to this joystick

        if not joy_mappings_to_use: # If no specific player map, use generic defaults
            # debug(f"UI Poll: UI joystick index {ui_controller_idx} not assigned to a player or no specific map. Using generic map for UI buttons.")
        ...
        # Using "reset" for "Retry" in LAN search is a bit unusual, but follows original logic.
        # Consider a specific "menu_retry" or "menu_action_extra" if "reset" has other game meanings.
        retry_mapping = joy_mappings_to_use.get("reset") # Or a more UI-specific action
    ...
    if navigated_this_poll and action_input_source:
        ...
        # _reset_all_prev_press_flags(main_window) # Reset axis flags only if axis/hat caused navigation, not buttons.
                                                # Button presses are discrete.
        ...
        # Update prev_state for the joystick that acted
        ...
        if action_input_source.startswith("controller_"):
            try:
                ...
                if 0 <= acted_ui_idx < len(main_window._pygame_joysticks):
                    ...
                    if acted_joy_object and acted_joy_object.get_init(): # Check joy object exists
                        acted_joy_instance_id = acted_joy_object.get_instance_id()
            except (ValueError, IndexError, AttributeError): # Added AttributeError
                pass
        ...
        if acted_joy_instance_id != -1 and acted_joy_instance_id < len(main_window._pygame_joy_button_prev_state):
            ...
            # Find the specific joystick object by its instance ID from the list app_core maintains
            for j_obj in main_window._pygame_joysticks:
                if j_obj and j_obj.get_instance_id() == acted_joy_instance_id: # Check j_obj exists
        ...
        _update_current_menu_button_focus(main_window)
        return # Exited due to navigation

    # If no navigation occurred, still update all joystick button prev_states
...
def _navigate_current_menu_pygame_joy(main_window: 'MainWindow', direction: int, input_source: str):
    ...
    if active_ui == "map_select":
        ...
        if direction == -1 : # Up
            ...
            # Calculate items in the current row to correctly clamp column
            current_row_start_index = row * main_window.NUM_MAP_COLUMNS
            items_in_this_row = min(main_window.NUM_MAP_COLUMNS, num_buttons - current_row_start_index)
            col = min(items_in_this_row - 1, col + 1) if items_in_this_row > 0 else 0


        new_idx_candidate = row * main_window.NUM_MAP_COLUMNS + col
        # Ensure the new index is valid, especially if navigating to a shorter row
        if new_idx_candidate < num_buttons:
            new_idx = new_idx_candidate
        else: # If target column doesn't exist in new row, try to land on last item of that row
            new_row_start_index = row * main_window.NUM_MAP_COLUMNS
            items_in_new_row = min(main_window.NUM_MAP_COLUMNS, num_buttons - new_row_start_index)
            if items_in_new_row > 0:
                new_idx = new_row_start_index + items_in_new_row -1
            # else new_idx remains current_idx (should not happen if num_buttons > 0)


        main_window._map_selection_selected_button_idx = new_idx
        _set_selected_idx_for_source(main_window, new_idx, input_source) # Update for the specific controller
        ...
        if main_window.lan_servers_list_widget:
            ...
            if main_window.lan_servers_list_widget.count() > 0:
                ...
                else: new_lan_idx = current_lan_idx # No change for horizontal in list
        ...
        if direction in [-2, 2]: # Only horizontal (Left/Right) for IP dialog buttons
        ...
        actual_direction = 0 # -1 for Up, 1 for Down
        if direction == -1: actual_direction = -1 # Up
        elif direction == 1: actual_direction = 1  # Down
        # For simple vertical lists, map horizontal input also to vertical
...
def _activate_current_menu_selected_button_pygame_joy(main_window: 'MainWindow', input_source: str):
    ...
    if active_ui == "map_select":
        ...
        current_idx = main_window._map_selection_selected_button_idx # Map select uses its own index
    elif active_ui == "menu":
        buttons_to_activate = main_window._main_menu_buttons_ref
        current_idx = _get_selected_idx_for_source(main_window, input_source) # Get index for the acting controller
    else:
    ...
    if not (0 <= current_idx < len(buttons_to_activate)):
        warning(f"MERGE_DEBUG_UI: Activation index {current_idx} out of bounds for {len(buttons_to_activate)} buttons for UI {active_ui}. Input source: {input_source}"); return

    selected_button = buttons_to_activate[current_idx]
    selected_button.click()

def _reset_all_prev_press_flags(main_window: 'MainWindow'):
    """Resets axis press flags. Called when UI context changes or after some UI actions."""
    main_window._pygame_joy_axis_was_active_neg.clear()
    main_window._pygame_joy_axis_was_active_pos.clear()
    # print("MERGE_DEBUG_UI: Reset all previous joystick axis press flags.")
...
def _update_current_menu_button_focus(main_window: 'MainWindow'):
    ...
    input_source_for_styling = main_window._last_active_input_source # Use the globally last active source for consistent styling
    ...
    if active_ui == "menu":
        ...
        current_selected_idx = main_window._map_selection_selected_button_idx # Map select uses its own single index
    else:
        return
    ...
    if not (0 <= current_selected_idx < len(buttons_to_update)):
        if len(buttons_to_update) > 0:
            ...
            _set_selected_idx_for_source(main_window, 0, input_source_for_styling) # Update the source's index
            if active_ui == "map_select": # Also update map_select's specific index
                main_window._map_selection_selected_button_idx = 0
        else:
            return

    selected_button_widget = None
    focus_color_hex = main_window._keyboard_ui_focus_color_str # Default to keyboard
