...
def check_enemy_attack_collisions(enemy, hittable_targets_list: List[Any]):
    ...
    for target_sprite in hittable_targets_list:
        ...
        if getattr(target_sprite, 'is_petrified', False): continue # Petrified targets (not smashed) usually ignore damage
...
def enemy_take_damage(enemy, damage_amount: int):
    ...
    if getattr(enemy, 'is_dead', False) and not getattr(enemy, 'is_petrified', False) : return # Dead and not petrified means truly dead
    ...
    if getattr(enemy, 'is_petrified', False): # If petrified, damage might lead to smashing
        if enemy.current_health <= 0:
            debug(f"EnemyCombatHandler: Petrified Enemy {enemy_id_log} health 0. Smashing.")
            # Lazy import to potentially avoid circular dependencies if enemy_status_effects imports this module
    ...
    enemy.is_taking_hit = True; enemy.hit_timer = current_time_ms
    if enemy.current_health <= 0:
        if not getattr(enemy, 'is_dead', False): # Check if not already marked as dead
            debug(f"EnemyCombatHandler ({enemy_id_log}): Health <= 0. Setting state to 'death'.")
            set_enemy_state(enemy, 'death')
    else: # Health > 0
        # Only set to 'hit' state if not already in a fire-related visual state
