...
"""
# version 2.0.2
# version 2.0.3 (Fixed set_state logic for petrified/smashed, refined fire state transitions)
...
def set_player_state(player, new_state: str):
    ...
    if getattr(player, 'is_petrified', False) and not getattr(player, 'is_stone_smashed', False) and \
       new_state not in ['petrified', 'smashed', 'death', 'death_nm', 'idle']: # Allow 'death' if health becomes 0 while petrified
        ...
        return
    if getattr(player, 'is_stone_smashed', False) and new_state not in ['smashed', 'death', 'death_nm', 'idle']: # Allow 'death' if needed by game logic
            ...
            if player_animations and fallback_state_key in player_animations and player_animations.get(fallback_state_key):
                ...
                if not first_available_anim_key:
                    if hasattr(player, 'print_limiter') and player.print_limiter.can_print(f"player_set_state_no_anims_{player.player_id}"):
                        critical(f"Player CRITICAL ({player_id_str}): No animations loaded. Requested: '{original_new_state_request}'. Player invalid.")
    ...
    if new_state == 'dash':
        ...
        player.is_crouching = False # Ensure not stuck crouching if entering idle
