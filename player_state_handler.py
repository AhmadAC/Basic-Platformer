...
# -*- coding: utf-8 -*-
"""
## version 1.0.0.5 (Handle new player burning/crouch fire states)
...
def set_player_state(player, new_state: str):
    ...
    # MODIFICATION: Handle aflame/deflame based on crouching status
    if player.is_aflame and new_state not in ['burning', 'aflame_crouch', 'burning_crouch', 'deflame', 'deflame_crouch', 'death', 'death_nm', 'petrified', 'smashed']:
        new_state = 'burning_crouch' if player.is_crouching else 'burning'
    elif player.is_deflaming and new_state not in ['burning', 'aflame_crouch', 'burning_crouch', 'deflame', 'deflame_crouch', 'death', 'death_nm', 'petrified', 'smashed']:
    ...
    can_change_state_now = (
        ...
        new_state in getattr(player, 'allow_state_reset_list', ['death', 'death_nm', 'hit', 'petrified', 'smashed', 'burning', 'aflame_crouch', 'burning_crouch', 'deflame', 'deflame_crouch', 'frozen', 'defrost']) # MODIFIED: Added new fire states
    ) and not (
        player.is_dead and player.death_animation_finished and new_state not in ['death', 'death_nm', 'petrified', 'smashed']
    )
    ...
    if can_change_state_now:
        ...
        # MODIFIED: Handle new fire state flags
        if new_state not in ['burning', 'aflame_crouch', 'burning_crouch']: player.is_aflame = False
        if new_state not in ['deflame', 'deflame_crouch']: player.is_deflaming = False
        ...
        if new_state == 'dash':
            ...
            player.is_dead = True; player.death_animation_finished = True # Petrified is an immediate "end" visually until smashed
            ...
            player.stone_smashed_timer_start = player.state_timer # Timer for smashed duration
            player.is_dead = True # Smashed is definitely "dead"
            player.death_animation_finished = False # Smashed animation needs to play out
            player.vel.xy = 0,0; player.acc.xy = 0,0
        # MODIFIED: Handle new fire states
        elif new_state == 'burning' or new_state == 'aflame_crouch' or new_state == 'burning_crouch':
            if not player.is_aflame: # Only if just becoming aflame (or transitioning between fire states)
                player.aflame_timer_start = player.state_timer # Reset timer if it's a fresh burn
                player.aflame_damage_last_tick = player.state_timer
            player.is_aflame = True
            player.is_deflaming = False # Ensure deflaming is off
            ...
            player.is_aflame = False # Ensure aflame is off
