...
# Pygame import for controller input (menu and in-game)
import pygame
...
from game_state_manager import reset_game_state # Corrected import
...
# REMOVE: import joystick_handler # This was the 'inputs' library based handler
from player import Player
# player_input_handler now expects Pygame joystick data if joystick is selected
...
# REMOVE JoystickPollingThread class if it was only for the 'inputs' library joystick_handler
# If you still want it for some other background task, keep it.
# For Pygame joystick, polling is done in the main game_update_timer.
...
class MainWindow(QMainWindow):
    ...
    # REMOVE: joystick_poll_thread: Optional[JoystickPollingThread] = None

    _pygame_joysticks: List[pygame.joystick.Joystick]
    _pygame_joy_button_prev_state: List[Dict[int, bool]] # For "just pressed" logic for Pygame joysticks
    ...
    _pygame_joy_axis_was_active_neg: Dict[int, bool] # Per joystick_id
    _pygame_joy_axis_was_active_pos: Dict[int, bool] # Per joystick_id
    ...
    _prev_menu_confirm_pressed: bool # For Pygame joystick UI nav
    _prev_menu_cancel_pressed: bool  # For Pygame joystick UI nav
    # ... (other prev_flags for dialogs can remain if joystick still navigates them)
    ...
    def __init__(self):
        ...
        try:
            ...
            self._pygame_joy_button_prev_state = [] # Initialize list for previous button states
            for i in range(pygame.joystick.get_count()):
                joy = pygame.joystick.Joystick(i)
                joy.init()
                self._pygame_joysticks.append(joy)
                self._pygame_joy_button_prev_state.append({}) # Add a dict for each joystick
        ...
        self._pygame_joy_axis_was_active_neg = {} # Dict: {joy_idx: bool}
        self._pygame_joy_axis_was_active_pos = {} # Dict: {joy_idx: bool}
        ...
        # No longer try to init the 'inputs' library based joystick_handler here for the game itself
        # config.py will handle its own Pygame init for config loading purposes
        try:
            game_config.load_config() # This loads game_config_pygame.json and controller_mappings.json
        except Exception as e_cfg:
            critical(f"Error during game_config.load_config() in MainWindow: {e_cfg}", exc_info=True)
            self._handle_config_load_failure() # This method no longer calls _translate
        ...
        self.map_select_widget = self._create_map_select_widget()
        ...
        # REMOVE: self.joystick_poll_thread = JoystickPollingThread(self)
        # REMOVE: self.joystick_poll_thread.start()
    ...
    def _handle_config_load_failure(self):
        ...
        # config.py should have set up defaults if loading failed.
        # The translation of keyboard keys from strings to Qt.Key is now internal to config.py's loading process.

    # ... (rest of _create_view_page_with_back_button, _create_map_select_widget, _populate_map_list_for_selection,
    #      _on_map_selected_for_couch_coop, _on_map_selected_for_host_game, _create_main_menu_widget
    #      _update_current_menu_button_focus, _navigate_current_menu_pygame_joy, _activate_current_menu_selected_button_pygame_joy
    #      are mostly the same, ensure _update_current_menu_button_focus uses self.map_select_scroll_area.ensureWidgetVisible if needed)

    def _update_current_menu_button_focus(self): # MODIFIED FOR SCROLLING
        ...
        if selected_button_widget and self.current_view_name == "map_select" and self.map_select_scroll_area:
            self.map_select_scroll_area.ensureWidgetVisible(selected_button_widget, 50, 50)
        elif selected_button_widget and self.current_modal_dialog == "lan_search" and self.lan_servers_list_widget:
            list_item = self.lan_servers_list_widget.item(self._lan_search_list_selected_idx)
            if list_item: self.lan_servers_list_widget.scrollToItem(list_item, QListWidget.ScrollHint.EnsureVisible)


    def _poll_pygame_joysticks_for_ui_navigation(self): # Renamed from _poll_pygame_joysticks_for_menu_or_map_select
        ...
        if active_ui_element not in ["menu", "map_select", "lan_search", "ip_input"] or not self._pygame_joysticks:
            self._reset_all_prev_press_flags(); return # Reset all flags if not in a relevant UI

        pygame.event.pump();
        # For simplicity, using the first Pygame joystick for UI navigation.
        # Could be extended to allow choosing which joystick navigates menus.
        joy = self._pygame_joysticks[0]
        joy_idx = 0 # Since we are using joy = self._pygame_joysticks[0]

        JOY_NAV_AXIS_ID_Y = 1; JOY_NAV_AXIS_ID_X = 0 # Standard Y and X axes for sticks
        JOY_NAV_HAT_ID = 0
        current_time = time.monotonic()
        if current_time - self._last_pygame_joy_nav_time < 0.20: return # Cooldown reduced slightly
        ...
        if joy.get_numhats() > JOY_NAV_HAT_ID:
            ...
            if hat_y > 0.5: # Down
                if active_ui_element in ["menu", "map_select", "lan_search"]: self._navigate_current_menu_pygame_joy(1); navigated_by_hat = True
            elif hat_y < -0.5: # Up
                if active_ui_element in ["menu", "map_select", "lan_search"]: self._navigate_current_menu_pygame_joy(-1); navigated_by_hat = True
            if hat_x > 0.5: # Right
                if active_ui_element == "map_select" or (active_ui_element == "ip_input" and self._ip_dialog_buttons_ref): self._navigate_current_menu_pygame_joy(2); navigated_by_hat = True
            elif hat_x < -0.5: # Left
                if active_ui_element == "map_select" or (active_ui_element == "ip_input" and self._ip_dialog_buttons_ref): self._navigate_current_menu_pygame_joy(-2); navigated_by_hat = True
            if navigated_by_hat: self._last_pygame_joy_nav_time = current_time; self._reset_all_prev_press_flags(); return
        ...
        if joy.get_numaxes() > JOY_NAV_AXIS_ID_Y:
            ...
            if axis_y_val > nav_threshold: # Down
                if not self._pygame_joy_axis_was_active_pos.get(joy_idx, False):
                    if active_ui_element in ["menu", "map_select", "lan_search"]: self._navigate_current_menu_pygame_joy(1); navigated_by_axis = True
                self._pygame_joy_axis_was_active_pos[joy_idx] = True
            else: self._pygame_joy_axis_was_active_pos[joy_idx] = False
            
            if axis_y_val < -nav_threshold: # Up
                if not self._pygame_joy_axis_was_active_neg.get(joy_idx, False):
                    if active_ui_element in ["menu", "map_select", "lan_search"]: self._navigate_current_menu_pygame_joy(-1); navigated_by_axis = True
                self._pygame_joy_axis_was_active_neg[joy_idx] = True
            else: self._pygame_joy_axis_was_active_neg[joy_idx] = False
        # Horizontal
        if joy.get_numaxes() > JOY_NAV_AXIS_ID_X:
             axis_x_val = joy.get_axis(JOY_NAV_AXIS_ID_X)
             if axis_x_val > nav_threshold: # Right
                if not self._pygame_joy_axis_was_active_pos.get(f"{joy_idx}_x", False): # Use a different key for X-axis state
                    if active_ui_element == "map_select" or (active_ui_element == "ip_input" and self._ip_dialog_buttons_ref): self._navigate_current_menu_pygame_joy(2); navigated_by_axis = True
                self._pygame_joy_axis_was_active_pos[f"{joy_idx}_x"] = True
             else: self._pygame_joy_axis_was_active_pos[f"{joy_idx}_x"] = False
             
             if axis_x_val < -nav_threshold: # Left
                if not self._pygame_joy_axis_was_active_neg.get(f"{joy_idx}_x", False):
                    if active_ui_element == "map_select" or (active_ui_element == "ip_input" and self._ip_dialog_buttons_ref): self._navigate_current_menu_pygame_joy(-2); navigated_by_axis = True
                self._pygame_joy_axis_was_active_neg[f"{joy_idx}_x"] = True
             else: self._pygame_joy_axis_was_active_neg[f"{joy_idx}_x"] = False
        if navigated_by_axis: self._last_pygame_joy_nav_time = current_time; self._reset_all_prev_press_flags(); return

        # Button Actions
        # Get current button states for "just pressed" logic
        current_joy_buttons = {i: joy.get_button(i) for i in range(joy.get_numbuttons())}
        prev_joy_buttons = self._pygame_joy_button_prev_state[joy_idx] # Get previous states for this joystick

        confirm_pressed_this_frame = False
        if confirm_mapping and confirm_mapping.get("type") == "button":
            btn_id = confirm_mapping.get("id")
            if current_joy_buttons.get(btn_id, False) and not prev_joy_buttons.get(btn_id, False):
                confirm_pressed_this_frame = True
        
        if confirm_pressed_this_frame:
            ...
            self._last_pygame_joy_nav_time = current_time; self._reset_all_prev_press_flags(); 
            self._pygame_joy_button_prev_state[joy_idx] = current_joy_buttons.copy(); return # Update prev state and return

        cancel_pressed_this_frame = False
        if cancel_mapping and cancel_mapping.get("type") == "button":
            btn_id = cancel_mapping.get("id")
            if current_joy_buttons.get(btn_id, False) and not prev_joy_buttons.get(btn_id, False):
                cancel_pressed_this_frame = True

        if cancel_pressed_this_frame:
            ...
            self._last_pygame_joy_nav_time = current_time; self._reset_all_prev_press_flags();
            self._pygame_joy_button_prev_state[joy_idx] = current_joy_buttons.copy(); return
        
        if active_ui_element == "lan_search": # Specific "Retry" logic for LAN search
            retry_mapping = game_config.LOADED_PYGAME_JOYSTICK_MAPPINGS.get("reset") # Example: "reset" action for retry
            retry_pressed_this_frame = False
            if retry_mapping and retry_mapping.get("type") == "button":
                btn_id = retry_mapping.get("id")
                if current_joy_buttons.get(btn_id, False) and not prev_joy_buttons.get(btn_id, False):
                    retry_pressed_this_frame = True
            if retry_pressed_this_frame:
        ...
        # Update previous button state for the next frame if no action was taken
        self._pygame_joy_button_prev_state[joy_idx] = current_joy_buttons.copy()

    def _reset_all_prev_press_flags(self): # Renamed for clarity
        # These flags were for a simpler polling model, the per-joystick dict is better for Pygame
        self._prev_menu_confirm_pressed = False; self._prev_menu_cancel_pressed = False
        # No need for separate _prev_lan or _prev_ip flags if using the dict above correctly
    ...
    def show_view(self, view_name: str):
        ...
        elif view_name == "map_select": target_page = self.map_select_widget; self._current_active_menu_buttons = self._map_selection_buttons_ref; self._current_active_menu_selected_idx_ref = "_map_selection_selected_button_idx"; self._map_selection_selected_button_idx = 0
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if active_ui_element in ["menu", "map_select"] and not is_auto_repeat:
            ...
            elif event.key() == Qt.Key.Key_Left and active_ui_element == "map_select": self._navigate_current_menu_pygame_joy(-2); event.accept(); return
            elif event.key() == Qt.Key.Key_Right and active_ui_element == "map_select": self._navigate_current_menu_pygame_joy(2); event.accept(); return
            ...
            if event.key() in [Qt.Key.Key_Left, Qt.Key.Key_Right]: self._ip_dialog_selected_button_idx = 1 - self._ip_dialog_selected_button_idx; self._update_ip_dialog_button_focus(); event.accept(); return
            elif event.key() in [Qt.Key.Key_Return, Qt.Key.Key_Enter, Qt.Key.Key_Space]: self._activate_ip_dialog_button(); event.accept(); return
        if event.key() == Qt.Key.Key_Escape and not is_auto_repeat:
            if active_ui_element == "menu": self.request_close_app()
            elif active_ui_element == "map_select": self.show_view("menu")
            elif active_ui_element == "lan_search" and self.lan_search_dialog: self.lan_search_dialog.reject() # This will trigger current_modal_dialog = None via rejected signal
            elif active_ui_element == "ip_input" and self.ip_input_dialog: self.ip_input_dialog.reject()
    ...
    def keyReleaseEvent(self, event: QKeyEvent):
        global _qt_keys_pressed_snapshot, _qt_key_events_this_frame; is_auto_repeat = event.isAutoRepeat(); qt_key_enum = Qt.Key(event.key())
        if not is_auto_repeat: _qt_keys_pressed_snapshot[qt_key_enum] = False
        if not event.isAccepted(): super().keyReleaseEvent(event)

    # ... (All other methods like _get_input_snapshot, _prepare_and_start_game, update_game_loop, on_start_..., _select_map_dialog,
    #      _start_network_mode, _show_status_dialog, _update_status_dialog, _close_status_dialog,
    #      on_network_status_update, on_network_operation_finished, _show_lan_search_dialog, _start_lan_server_search_thread,
    #      on_lan_server_search_status_update, _join_selected_lan_server_from_dialog,
    #      stop_current_game_mode, closeEvent remain the same as the previous version unless specifically mentioned above)
    ...
    def _get_input_snapshot(self, player_instance: Player, player_id: int) -> Dict[str, bool]:
        ...
        if joystick_pygame_idx_for_player is not None and 0 <= joystick_pygame_idx_for_player < len(self._pygame_joysticks):
            ...
            prev_buttons_state = self._pygame_joy_button_prev_state[joystick_pygame_idx_for_player]
            joystick_data_for_handler = {
                'axes': {i: joy.get_axis(i) for i in range(joy.get_numaxes())},
                'buttons_current': current_buttons_state,
                'buttons_prev': prev_buttons_state.copy(), # Pass a copy of the previous state
                'hats': {i: joy.get_hat(i) for i in range(joy.get_numhats())}
            }
            self._pygame_joy_button_prev_state[joystick_pygame_idx_for_player] = current_buttons_state.copy() # Update for next frame
    ...
    def _prepare_and_start_game(self, mode: str, map_name: Optional[str] = None, target_ip_port: Optional[str] = None):
        ...
        if map_name is None and mode != "join_ip" and mode != "join_lan": map_name = getattr(C, 'DEFAULT_LEVEL_MODULE_NAME', "level_default")
        initialized_elements = initialize_game_elements(current_width, current_height, mode, None, map_name if mode not in ["join_ip", "join_lan"] else None)
        ...
        if mode == "host_waiting": self._start_network_mode("host_listen_only")
        elif mode in ["host", "join_lan", "join_ip"]: self._start_network_mode(mode, target_ip_port)
    ...
    def on_client_fully_synced_for_host(self):
        ...
        else: warning("on_client_fully_synced_for_host called but not in 'host_waiting' mode.")
    ...
    def update_game_loop(self):
        ...
        if self.current_game_mode == "couch_play" and self.app_status.app_running:
            ...
            if p1:
                ...
                for enemy in list(self.game_elements.get("enemy_list",[])): enemy.update(dt_sec, active_players_for_ai, self.game_elements.get("platforms_list",[]), self.game_elements.get("hazards_list",[]), self.game_elements.get("enemy_list",[])) # etc.
                projectiles_current_list = self.game_elements.get("projectiles_list", [])
                for proj_obj in list(projectiles_current_list):
                    if hasattr(proj_obj, 'update'): proj_obj.update(dt_sec, self.game_elements.get("platforms_list",[]), active_players_for_ai + self.game_elements.get("enemy_list",[]))
    ...
    def on_start_join_ip(self):
        self.ip_input_dialog = IPInputDialog(parent=self); self._ip_dialog_buttons_ref = [self.ip_input_dialog.button_box.button(QDialogButtonBox.StandardButton.Ok), self.ip_input_dialog.button_box.button(QDialogButtonBox.StandardButton.Cancel)]
        self._ip_dialog_selected_button_idx = 0; self.current_modal_dialog = "ip_input"; self._update_ip_dialog_button_focus()
        if self.ip_input_dialog.exec() == QDialog.DialogCode.Accepted and self.ip_input_dialog.ip_port_string: self._prepare_and_start_game("join_ip", target_ip_port=self.ip_input_dialog.ip_port_string)
    ...
    def _select_map_dialog(self) -> Optional[str]:
        dialog = SelectMapDialog(self.fonts, self) # Used only by old direct host
    ...
    def _start_network_mode(self, mode_name: str, target_ip_port: Optional[str] = None):
        ...
        if mode_name == "host_listen_only": self.server_state = ServerState(); self.server_state.current_map_name = self.game_elements.get("loaded_map_name", "unknown_map"); self.network_thread = NetworkThread("host", self.game_elements, self.server_state, parent=self); self.network_thread.client_fully_synced_signal.connect(self.on_client_fully_synced_for_host)
        elif mode_name == "host": # Active hosting
            ...
            # If a NetworkThread for "host_listen_only" was already running, it continues.
            # This branch is more for re-establishing if something went wrong or for future direct host_active.
            if not (self.network_thread and self.network_thread.isRunning()): # Only start if not already listening
                self.network_thread = NetworkThread("host", self.game_elements, self.server_state, parent=self)
                self.network_thread.client_fully_synced_signal.connect(self.on_client_fully_synced_for_host)
        elif mode_name in ["join_lan", "join_ip"]: self.client_state = ClientState(); self.network_thread = NetworkThread("join", self.game_elements, client_state_ref=self.client_state, target_ip_port=target_ip_port, parent=self)
        if self.network_thread and not self.network_thread.isRunning(): # Check if it needs starting
            ...
            if mode_name != "host_listen_only": self._show_status_dialog("Network Operation", f"Initializing {mode_name} mode...")
        elif not self.network_thread and mode_name != "host_listen_only": # If thread failed to create for non-listening modes
            error(f"Failed to create/start NetworkThread for {mode_name}"); QMessageBox.critical(self, "Network Error", f"Could not start {mode_name} mode."); self.show_view("menu")
    ...
    def on_network_status_update(self, title: str, message: str, progress: float):
        is_hosting_waiting = (self.current_game_mode == "host_waiting")
        show_dialog_now = not (self.status_dialog and self.status_dialog.isVisible()) and \
                          (not is_hosting_waiting or (is_hosting_waiting and "Player 2 needs" in message))
        if show_dialog_now: self._show_status_dialog(title, message)
        if self.status_dialog and self.status_dialog.isVisible(): self._update_status_dialog(message, progress)
        if title in ["game_starting", "game_active"] or (title == "Map Sync" and "Player 2 has" in message and "Ready" in message and progress >= 99.9): self._close_status_dialog()
    @Slot(str)
    def on_network_operation_finished(self, message: str):
        ...
        elif "ended" in message.lower() and self.current_game_mode and self.current_game_mode != "host_waiting": info(f"Mode {self.current_game_mode} finished via network signal."); self.stop_current_game_mode(show_menu=True)
    ...
    def _update_lan_search_list_focus(self):
        ...
        if self.lan_servers_list_widget.count() > 0:
            ...
            if selected_item: self.lan_servers_list_widget.scrollToItem(selected_item, QListWidget.ScrollHint.EnsureVisible)
    ...
    def _join_selected_lan_server_from_dialog(self):
        ...
        selected_item = self.lan_servers_list_widget.currentItem()
        if selected_item:
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # Stop the JoystickPollingThread (for 'inputs' library, if it was started)
        # REMOVED: If self.joystick_poll_thread was removed, this is not needed.
        # If it's still there:
        ...
        #     if not self.joystick_poll_thread.wait(1000): warning("Joystick polling thread did not stop gracefully."); self.joystick_poll_thread.terminate(); self.joystick_poll_thread.wait(100)
        ...
        # else: warning("MAIN PySide6: 'inputs' joystick_handler.quit_joysticks() not found or not callable.") # Already logged during init if missing
