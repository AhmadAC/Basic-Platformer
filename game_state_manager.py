...
def reset_game_state(game_elements: Dict[str, Any]) -> Optional[Chest]:
    ...
    # Start with static elements that are always part of the map geometry
    ...
    for static_list_key in ["platforms_list", "ladders_list", "hazards_list", "background_tiles_list"]:
        ...
        if isinstance(static_list, list): # Ensure it's a list before extending
    ...
    if player1 and hasattr(player1, 'reset_state'):
        debug(f"GSM: Resetting P1 at {player1_spawn_pos_tuple if player1_spawn_pos_tuple else 'initial spawn (from player object)'}")
    ...
    if player2 and hasattr(player2, 'reset_state'):
        debug(f"GSM: Resetting P2 at {player2_spawn_pos_tuple if player2_spawn_pos_tuple else 'initial spawn (from player object)'}")
    ...
    enemy_list_ref.clear() # Clear current instances
    ...
    if (current_game_mode in ["host", "couch_play", "host_game"]) and enemy_spawns_data_cache:
        ...
        for i, spawn_info in enumerate(enemy_spawns_data_cache):
            try:
                ...
                if new_enemy_instance._valid_init and new_enemy_instance.alive():
                    enemy_list_ref.append(new_enemy_instance) # Add to the game_elements list
    ...
    # game_elements["enemy_list"] is already updated as enemy_list_ref is a reference to it.
    ...
    collectible_list_ref.clear() # Clear current collectibles
    ...
    # game_elements["collectible_list"] is already updated as collectible_list_ref is a reference
    ...
    if camera and hasattr(camera, 'set_offset'):
        ...
        elif player2 and player2.alive(): camera.update(player2)
...
def set_network_game_state(network_state_data: Dict[str, Any],
    ...
    """
    from player import Player # Local import to avoid top-level circular if Player imports GSM
    from enemy import Enemy
    from statue import Statue
    from items import Chest
    ...
    player1 = game_elements.get("player1")
    player2 = game_elements.get("player2")
    enemy_list_ref: List[Enemy] = game_elements.get("enemy_list", []) # Use ref for modification
    statue_objects_list_client_ref: List[Statue] = game_elements.get("statue_objects", [])
    current_chest_obj: Optional[Chest] = game_elements.get("current_chest") # Will be updated or re-created

    all_renderable_objects_ref: List[Any] = game_elements.get("all_renderable_objects", [])
    projectiles_list_ref: List[Any] = game_elements.get("projectiles_list", [])
    collectible_list_ref: List[Any] = game_elements.get("collectible_list", [])
    
    # Build a new all_renderable_objects list for this update
    new_all_renderables: List[Any] = []
    current_all_renderables_set = set() # For quick lookups to avoid duplicates
        ...
        for enemy_id_str, enemy_data_server in received_enemy_data_map.items():
            ...
            if enemy_data_server.get('_valid_init', False):
                ...
                if client_enemy and client_enemy._valid_init:
                    ...
                    new_enemy_list_client.append(client_enemy) # Add to new list for this frame
                    ...
                    if is_enemy_renderable: add_to_renderables(client_enemy)
                     ...
                     error(f"GSM Client: Failed to init new enemy {enemy_id_str} from server data (or existing became invalid).")

    game_elements["enemy_list"] = new_enemy_list_client # Assign the newly constructed list
        ...
        for statue_id_server_str, statue_data_server in received_statue_data_map.items():
            ...
            if statue_data_server.get('_valid_init', False):
                ...
                if client_statue and client_statue._valid_init:
                    ...
                    error(f"GSM Client: Failed to init new statue {statue_id_server_str} from server data (or existing became invalid).")
    game_elements["statue_objects"] = new_statue_list_client

    # Chest data
    new_collectible_list_client: List[Any] = [] # Start fresh for collectibles
    current_chest_obj_synced: Optional[Chest] = None # Track the chest for this sync
    ...
    if 'chest' in network_state_data:
        ...
        if chest_data_server and isinstance(chest_data_server, dict) and chest_data_server.get('_alive', True):
            # If current_chest_obj exists and is the same logical chest, update it.
            # Otherwise, create a new one. For simplicity in network sync, we often re-create or ensure one instance.
            # Let's assume only one chest is managed by 'current_chest' for now.
            if not current_chest_obj or not current_chest_obj._valid_init: # Create if missing or invalid
            ...
            if current_chest_obj._valid_init:
                ...
                current_chest_obj_synced = current_chest_obj # This is the active chest
                ...
                current_chest_obj_synced = None # Failed to create/update
        # else: server says no chest or it's not alive
    ...
    if 'projectiles' in network_state_data:
        ...
        for proj_id_server_str, proj_data_server in received_proj_data_map.items():
            ...
            if client_proj:
                ...
                if client_proj.alive(): # Only add if it's still alive after sync
