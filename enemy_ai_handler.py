...
def log_enemy_state(enemy, message: str, current_time_ms: int):
    if ENABLE_ENEMY_AI_DEBUG_PRINTS:
        ...
        # Use a more direct print or ensure logger is configured if this file is run standalone for testing
        # For now, assuming logger is available from the main application.
...
def enemy_ai_update(enemy, players_list_for_ai: list):
    ...
    if not hasattr(enemy, '_valid_init') or not enemy._valid_init or \
       (hasattr(enemy, 'is_dead') and enemy.is_dead) or not (hasattr(enemy, 'alive') and enemy.alive()):
        ...
        # log_enemy_state(enemy, "Entity invalid, dead, or not alive. No AI.", current_time_ms) # Already logged by caller or can be
        return
    ...
    if not (hasattr(enemy, 'pos') and hasattr(enemy.pos, 'x') and hasattr(enemy.pos, 'y')): # Ensure pos has x and y
    ...
    for player_candidate in players_list_for_ai:
        is_targetable = (
            player_candidate and 
            hasattr(player_candidate, '_valid_init') and player_candidate._valid_init and
        ...
        if is_targetable:
            ...
            dy = player_candidate.pos.y() - enemy.pos.y()
    ...
    has_vertical_los = vertical_distance_to_player < enemy_rect_height * 1.0 # Simplified LOS check
    ...
    if hasattr(enemy, 'is_aflame') and enemy.is_aflame:
        ...
        current_accel_x = 0.0
        # ... (rest of deflame logic, similar to aflame) ...
        if closest_target_player and is_player_in_detection_range:
    ...
    if hasattr(enemy, 'is_attacking') and enemy.is_attacking:
        ...
        if current_time_ms - enemy.attack_timer >= enemy.attack_duration:
            ...
            if hasattr(enemy, 'acc') and hasattr(enemy.acc, 'setX'): enemy.acc.setX(0.0) # Usually no movement during attack
    ...
    target_accel_x = 0.0
    target_facing_right = enemy.facing_right # Assume no change initially

    if not closest_target_player: # No player detected or targetable
        enemy.ai_state = 'patrolling'
        if enemy.state not in ['run', 'idle'] and hasattr(enemy, 'set_state'): enemy.set_state('patrolling')
        if abs(enemy.pos.x() - enemy.patrol_target_x) < 10: # Reached patrol target
            set_enemy_new_patrol_target(enemy)
    ...
    else: # Player exists but not in detection range (or LOS failed) -> patrol
        enemy.ai_state = 'patrolling'
        if enemy.state not in ['run', 'idle'] and hasattr(enemy, 'set_state'): enemy.set_state('patrolling')
    ...
    # Apply acceleration and facing direction
    ...
    log_enemy_state(enemy, f"End of AI. Acc.x: {enemy.acc.x():.2f if hasattr(enemy,'acc') and hasattr(enemy.acc,'x') else 'N/A'}, FacingRight: {enemy.facing_right}", current_time_ms)