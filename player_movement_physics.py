...
MODIFIED: Added logic for player "tipping" off ledges (from previous update).
"""
# version 2.0.9 (Re-applying gravity for dying player fix, and includes tipping)
...
_PHYSICS_FILE_LOG_KEY = "player_movement_physics_log_tick_v4" # Incremented key
...
def apply_player_movement_and_physics(player: 'PlayerClass_TYPE', platforms_list: List[Any]):
    ...
    if player.is_tipping:
        ...
        if abs(player.tipping_angle) >= max_tipping_angle:
            ...
            player.vel.setY(1.0) 
            set_player_state(player, 'fall', get_current_ticks())
            return 
    ...
    if player.is_petrified and not getattr(player, 'is_stone_smashed', False):
        if player.on_ground:
            ...
            should_apply_gravity_this_frame = True
    # MODIFICATION: Ensure gravity is applied to a normally dead player if their animation is not finished.
    elif player.is_dead and not getattr(player, 'is_petrified', False) and not getattr(player, 'death_animation_finished', False):
        player.acc.setY(float(getattr(C, 'PLAYER_GRAVITY', 0.7))) 
        should_apply_gravity_this_frame = True
    ...
    if player.is_rolling:
        ...
        should_apply_horizontal_physics = not (
            ...
            (player.is_dead and not player.is_petrified) # MODIFICATION: If normally dead, allow friction/speed limit but not player-controlled accel
        )
        ...
        # Apply friction and speed limits even if dead (for a more natural stop/fall)
        friction_coeff = 0.0
        ...
        # Apply speed limit unless in a special movement state that dictates its own speed
        ...
        # If dead, ensure player cannot control horizontal acceleration, but friction/limits apply
        if player.is_dead and not player.is_petrified:
            player.acc.setX(0.0) # Prevent further player-controlled horizontal movement
...
def check_and_initiate_tipping(player: 'PlayerClass_TYPE', platforms_list: List[Any]):
    ...
    if not supporting_platform:
        if player.on_ground: 
            player.on_ground = False; set_player_state(player, 'fall', get_current_ticks())
            _file_debug(f"P{player.player_id} No supporting platform found while on_ground=True. Transitioning to fall.")
        return False

    support_left_edge = supporting_platform.rect.left(); support_right_edge = supporting_platform.rect.right()
    tipping_threshold_px = player.rect.width() * 0.05
    tip_direction = 0; pivot_x = 0.0

    if player_center_x < support_left_edge - tipping_threshold_px and player.rect.left() < support_left_edge:
        if player.rect.right() > support_left_edge : tip_direction = -1; pivot_x = support_left_edge
    elif player_center_x > support_right_edge + tipping_threshold_px and player.rect.right() > support_right_edge:
        if player.rect.left() < support_right_edge : tip_direction = 1; pivot_x = support_right_edge
    ...
    if tip_direction != 0:
        ...
        _file_info(f"P{player.player_id} Initiated tipping. Dir: {tip_direction}, PivotX: {pivot_x:.1f}")
...
def update_player_core_logic(player: 'PlayerClass_TYPE', dt_sec: float, platforms_list: List[Any], ladders_list: List[Any],
    ...
    # Check for invalid player state first
    if not player._valid_init:
        _file_debug(f"{player_id_str} CoreLogic: Update skipped due to _valid_init={player._valid_init}.")
        return

    _file_log_player_physics(player, "UPDATE_START")

    # If player is dead (normal death) but animation isn't finished, OR if player is petrified (and not smashed on ground)
    # we still want to apply some physics (mainly gravity and basic Y collision).
    # Active player logic (input-driven movement, state timers for actions like dash/roll) is skipped.
    ...
    if not player.alive() and not is_dying_with_anim and not is_petrified_airborne_or_smashed_falling:
        # Player is fully "gone" or in a static petrified state on ground.
        _file_debug(f"{player_id_str} CoreLogic: Update skipped. Not alive and not in a physics-relevant death/petrified state.")
        if hasattr(player, 'animate'): player.animate() # Keep animating if there's a final frame to hold
        return

    # --- Limited Physics for Dying or Petrified (Airborne/Smashed) States ---
    if is_dying_with_anim or is_petrified_airborne_or_smashed_falling:
        ...
        apply_player_movement_and_physics(player, platforms_list) # This now handles gravity for these states

        # Only apply Y-movement and Y-collision for these simplified states
        ...
        player.pos = QPointF(player.rect.center().x(), player.rect.bottom()) # Sync pos Y from rect bottom

        if hasattr(player, 'animate'): player.animate()
        _file_log_player_physics(player, "UPDATE_END_LIMITED_PHYSICS", f"State: {player.state}")
        return
    # --- End Limited Physics ---

    # --- Full Physics and Logic for Active Player ---
    ...
    if player.game_elements_ref_for_projectiles: 
        ...
        for item in player.game_elements_ref_for_projectiles.get("statue_objects", []):
            if isinstance(item, Statue) and item.alive() and not item.is_smashed: all_other_char_sprites.append(item)
