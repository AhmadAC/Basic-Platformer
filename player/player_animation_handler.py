...
import time
import sys # Added sys for logger fallback pathing
import os # Added os for logger fallback pathing
from typing import List, Optional, Any

# PySide6 imports
from PySide6.QtGui import QPixmap, QImage, QTransform, QColor, QFont # Added QFont
...
if not _anim_fallback_logger.hasHandlers():
    # Determine path for fallback log file (optional, could just use console)
    _fallback_log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs_fallback")
    if not os.path.exists(_fallback_log_dir):
        try: os.makedirs(_fallback_log_dir)
        except OSError: pass # Ignore if cannot create
    ...
    # Optional: Add file handler for fallback if directory was created
    if os.path.exists(_fallback_log_dir):
        try:
            _h_file = logging.FileHandler(_fallback_log_file, mode='a') # Append mode
    ...
    _anim_fallback_logger.propagate = False # Don't pass to root logger
...
def _fb_error(msg, *args, **kwargs): _anim_fallback_logger.error(msg, *args, **kwargs) # Added error fallback
...
error = _fb_error # Assign error fallback
...
try:
    from main_game.logger import (
        ...
        error as _project_error, # Import error from project logger
    ...
    error = _project_error # Override with project's error
...
def determine_animation_key(player: Any) -> str:
    ...
    if is_petrified:
        # Petrified state uses specific frames based on crouch, handled by Player class assets
        # The animation key should reflect this to pick the correct frames.
    ...
    if is_dead:
        is_still_nm = abs(vel_x) < 0.5 and abs(vel_y) < 1.0 # Stricter check for "no movement"
        ...
        return key_variant if animations_available and animations_available.get(key_variant) else \
               ('death' if animations_available and animations_available.get('death') else 'idle') # Fallback if no death animations

    # --- Next Priority: Overriding Status Effects (Frozen before Fire, Zapped has its own logic) ---
    ...
    # These need to be checked before general movement/actions if they imply different animations
    if is_aflame:
    ...
    # Post-attack pause check (might visually be idle)
    ...
    if post_attack_pause_timer_val > 0 and get_current_ticks_monotonic() < post_attack_pause_timer_val:
        return 'idle' # During post-attack pause, show idle

    if is_attacking:
        attack_type = getattr(player, 'attack_type', 0)
        if attack_type == 4 or current_logical_state == 'crouch_attack': # Player.attack_type 4 is crouch_attack
        ...
        if base_key:
            # Player's input intent matters for selecting moving vs non-moving attack animations
            ...
            # Player's actual velocity also matters
            is_player_actually_moving_slow_anim = abs(vel_x) < 0.5 # Threshold

            # Use no-movement variant if:
            # 1. Player is NOT intending to move AND is actually slow, AND NM anim exists.
            # 2. OR if moving variant doesn't exist but NM does.
            ...
            elif animations_available and animations_available.get(moving_variant_key): # Prefer moving variant if available and not explicitly NM
                return moving_variant_key
            elif animations_available and animations_available.get(nm_variant_key): # Fallback to NM if moving one is missing
                return nm_variant_key
            else: # Fallback if no attack anims are found for the type
                return 'idle'
    ...
    if on_ladder:
        return 'ladder_climb' if abs(vel_y) > 0.1 else 'ladder_idle' # Check if actually moving on ladder

    if touching_wall != 0 and not on_ground: # Wall interaction
        ...
        # 'wall_hang' and 'wall_climb' states are typically set explicitly by state_handler.
        # If in those states, use them. Otherwise, if just touching wall and falling, it's 'fall'.
    ...
    if not on_ground: # General airborne states
        if current_logical_state == 'jump':
            return 'jump' if animations_available and animations_available.get('jump') else 'fall'
        if current_logical_state == 'jump_fall_trans': # Transition between jump and fall
            return 'jump_fall_trans' if animations_available and animations_available.get('jump_fall_trans') else 'fall'
        # Default airborne animation
        return 'fall' if animations_available and animations_available.get('fall') else 'idle' # Fallback if no fall anim

    # Grounded states (Crouch, Run, Idle)
    if is_crouching:
        # If logically 'crouch' or 'crouch_walk', try to use those animations
        ...
        # Fallback for crouch based on movement intent
        ...
        return crouch_key_variant if animations_available and animations_available.get(crouch_key_variant) else \
               ('crouch' if animations_available and animations_available.get('crouch') else 'idle') # Final fallback if no crouch anims

    # Standing movement
    player_is_intending_to_move_lr_stand = getattr(player, 'is_trying_to_move_left', False) or \
                                           getattr(player, 'is_trying_to_move_right', False)
    if player_is_intending_to_move_lr_stand: # If intending to move, try 'run'
        return 'run' if animations_available and animations_available.get('run') else 'idle'

    # Absolute fallback
    return 'idle' if animations_available and animations_available.get('idle') else 'idle' # Ensure 'idle' is always a valid key
...
def advance_frame_and_handle_state_transitions(player: Any, current_animation_frames_list: List[QPixmap], current_time_ms: int, current_animation_key: str):
    ...
    if player.is_frozen:
        ...
        player.current_frame = len(current_animation_frames_list) - 1 if len(current_animation_frames_list) > 1 and current_animation_key == 'frozen' else 0
        return
    ...
    if current_time_ms - player.last_anim_update > ms_per_frame:
        ...
        if player.current_frame >= len(current_animation_frames_list):
            ...
            # Define keys that have specific next states or reset to idle/fall
            ...
            if is_transitional_anim:
                ...
                if current_animation_key == 'jump':
                    # If jump animation has a specific transition frame/animation, use it
                    ...
                    next_state_after_transition = 'crouch' # Return to crouch state
                ...
                # Change state if different and valid
                if player.state == current_animation_key and next_state_after_transition != player.state:
                    set_player_state(player, next_state_after_transition, current_time_ms)
                    return # State changed, animation will be updated by new state
                else: # If no state change, or if state changed but to same anim key (e.g. slide -> slide)
...
def update_player_animation(player: Any):
    ...
    if not getattr(player, '_valid_init', False) or not hasattr(player, 'animations') or not player.animations:
        if hasattr(player, 'image') and player.image and not player.image.isNull():
            player.image.fill(qcolor_magenta) # Fill with magenta if invalid/no anims
        return
    ...
    can_animate_now = (hasattr(player, 'alive') and player.alive()) or \
                      ...
                      getattr(player, 'is_petrified', False) # Petrified players still need visual updates for stone forms
    ...
    if determined_key_for_logic == 'petrified': # Special handling for stone forms
        # Player class now holds stone_crouch_image_frame and stone_image_frame directly
    ...
    # Check if it's one of the common placeholders from assets.py
    elif len(current_frames_list) == 1 and \
         determined_key_for_logic not in ['petrified', 'idle', 'run', 'fall', 'frozen', 'crouch', 'crouch_attack', 'ladder_idle', 'wall_hang']: # Don't check placeholders for these naturally static states
        frame_pixmap = current_frames_list[0]
        # Check against common placeholder colors and a typical placeholder size
        if frame_pixmap.size() == QSize(30, 40): # Example placeholder size
            qimg = frame_pixmap.toImage()
            if not qimg.isNull():
                pixel_color = qimg.pixelColor(0,0)
                if pixel_color == qcolor_red or pixel_color == qcolor_blue:
                    is_current_animation_valid = False # Likely a red/blue placeholder

    if not is_current_animation_valid:
        original_determined_key_before_fallback = determined_key_for_logic
        # Fallback to 'idle' if the determined animation is invalid
        if determined_key_for_logic != 'idle': # Avoid self-referential fallback if 'idle' itself is the issue
        ...
        # Check if 'idle' itself is also invalid (placeholder or missing)
        ...
        if is_idle_still_invalid:
            ...
            # Set a final, very basic magenta placeholder if everything fails
            if hasattr(player, 'image') and player.image and not player.image.isNull():
    ...
    if not current_frames_list: # Should be caught by above, but as a final safeguard
        ...
        if hasattr(player, 'image') and player.image and not player.image.isNull():
            player.image.fill(qcolor_blue) # Fill with blue if list becomes none
        return
    ...
    # Only re-determine if not in a "sticky" visual state like petrified/smashed
        ...
        if new_render_key_candidate != determined_key_for_logic:
            ...
            if render_key_for_final_image == 'petrified':
                ...
                # Check if current_frames_list became invalid after this, if so, fallback to idle
                    ...
                    # If the new key's animation is bad, and the old one was also bad (or list is now None), use idle
    ...
    if not current_frames_list or player.current_frame < 0 or player.current_frame >= len(current_frames_list):
        ...
        if not current_frames_list: # Absolute fallback if list became empty
            if hasattr(player, 'image') and player.image and not player.image.isNull():
                player.image.fill(qcolor_yellow) # Fill with yellow if list empty after all checks
            return
    ...
    if render_key_for_final_image == 'petrified' or render_key_for_final_image == 'smashed':
        display_facing_right = player.facing_at_petrification # Stone visuals use facing at time of petrification

    final_image_to_set = image_for_this_frame
    # Flip image if player is facing left, UNLESS it's a petrified/smashed state
    # (stone images are typically pre-rendered for one direction or handled by the asset itself)
    if not display_facing_right:
        ...
        if not (render_key_for_final_image == 'petrified' or render_key_for_final_image == 'smashed'):
            ...
            # else: final_image_to_set remains image_for_this_frame (original) if toImage fails
    ...
    image_content_has_changed = (not hasattr(player, 'image') or player.image is None) or \
                                ...
                                (player.image is not final_image_to_set) # Direct ref check
    ...
    if image_content_has_changed or (player._last_facing_right != facing_direction_for_flip_check):
        # Store current midbottom before changing image (which might change rect size)
        ...
        player.image = final_image_to_set # Set the new image
        # Update player's collision rect based on the new image and re-anchor to old midbottom
        ...
        else: # Fallback rect update if method is missing
