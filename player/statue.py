...
"""
# version 2.1.6 (Smashed pieces gravity fix and refined physics)
import os
...
class Statue:
    def __init__(self, center_x: float, center_y: float, statue_id: Optional[Any] = None,
        ...
        self.pos = QPointF(float(center_x), float(center_y)) # pos is visual center
        ...
        self.is_dead = False # Will be true if smashed or health depleted
        self.death_animation_finished = False
        self._alive = True

        self.vel = QPointF(0.0, 0.0)
        self.on_ground = False # Will be determined by physics or if it's a non-falling type
        self._gravity = float(getattr(C, 'PLAYER_GRAVITY', 0.7)) # Use player gravity as a base for falling pieces

        # Determine destructibility and health
        self.is_destructible_by_property = self.properties.get("destructible", True)
        default_health_if_no_prop = 1 # Statues that are destructible default to 1 HP
    ...
    def smash(self):
        if not self.is_smashed and self._valid_init and self._alive:
            ...
            else: self.image = self._create_placeholder_qpixmap(QColor(*getattr(C, 'DARK_GRAY',(50,50,50))), "Smash")
            self._update_rect_from_image_and_pos()
            # Gravity will take over if it was airborne. Do not force on_ground = True here.
    ...
    def apply_physics_step(self, dt_sec: float, platforms_list: List[Any]):
        ...
        if self.is_smashed and self.death_animation_finished: # Fully smashed and animation done
            self.vel.setY(0.0)
            self.on_ground = True # Static rubble
            return
        ...
        if not self.on_ground:
            ...
            self.vel.setY(0.0) # Normal statue on ground, or fully finished rubble.
        ...
        if abs(scaled_vel_y_for_pos_update) > 1e-5:
        ...
        if self.vel.y() >= 0 or (self.is_smashed and not self.on_ground):
            self.on_ground = False
            for platform_obj in platforms_list:
                if isinstance(platform_obj, Statue) and platform_obj is self: continue
                if isinstance(platform_obj, Statue) and platform_obj.is_smashed: continue
                ...
                if self.rect.intersects(platform_obj.rect):
                    ...
                    if overlap_y > 0:
                        ...
                        if prev_bottom_y_estimate <= platform_obj.rect.top() + 1.0:
            ...
            if not self.death_animation_finished:
                if now - self.smashed_timer_start > C.STONE_SMASHED_DURATION_MS:
                    ...
                    if now - self.last_anim_update > anim_speed:
                        self.last_anim_update = now; self.current_frame_index += 1
                     ...
                     if self.image != self.smashed_frames[0] and self.smashed_frames[0] and not self.smashed_frames[0].isNull():
                        self.image = self.smashed_frames[0]; self._update_rect_from_image_and_pos(); image_updated = True
        if not self.is_smashed and not image_updated:
            if self.initial_image_frames and self.initial_image_frames[0] and not self.initial_image_frames[0].isNull():
                 if self.image != self.initial_image_frames[0]:
                    self.image = self.initial_image_frames[0]; self._update_rect_from_image_and_pos(); self.current_frame_index = 0
            else:
                ...
                self._update_rect_from_image_and_pos()
        if self.is_dead and self.death_animation_finished and self._alive: self.kill()

    def get_network_data(self) -> Dict[str, Any]:
        return {
            'id': self.statue_id, 'type': 'Statue', 'pos': (self.pos.x(), self.pos.y()),
            'vel_y': self.vel.y(), 'on_ground': self.on_ground,
            'is_smashed': self.is_smashed, 'smashed_timer_start': self.smashed_timer_start,
            'current_frame_index': self.current_frame_index, '_valid_init': self._valid_init,
            '_alive': self._alive, 'is_dead': self.is_dead,
    ...
    def set_network_data(self, data: Dict[str, Any]):
        ...
        if new_is_smashed != self.is_smashed:
            ...
            if self.is_smashed:
                self.is_dead = True; self.death_animation_finished = False
        ...
        is_crouched_variant_net = self.properties.get("is_crouched_variant", False) or self.properties.get("was_crouching", False)
        if self.is_smashed:
            ...
            if self.smashed_frames and 0 <= self.current_frame_index < len(self.smashed_frames) and \
               self.smashed_frames[self.current_frame_index] and not self.smashed_frames[self.current_frame_index].isNull():
                ...
                self.image = self.smashed_frames[0]
            else: self.image = self._create_placeholder_qpixmap(QColor(*getattr(C, 'DARK_GRAY',(50,50,50))), "SmashNet")
        else:
            self.current_frame_index = 0
            if self.initial_image_frames and self.initial_image_frames[0] and not self.initial_image_frames[0].isNull():
                 self.image = self.initial_image_frames[0]
