...
from player import Player # Keep for type hints if needed, but direct use minimized
...
import app_game_modes # Contains game mode logic
...
class NetworkThread(QThread):
    ...
    def run(self):
        from server_logic import run_server_mode as rs_mode # Local import
        from client_logic import run_client_mode as rc_mode # Local import
        try:
            ...
            if self.mode == "host" and self.server_state and main_window_instance:
                ...
                rs_mode(
                    ...
                    lambda: self.client_fully_synced_signal.emit() # Pass the callback for when client is synced
                )
                ...
                rc_mode(
                    ...
                    self._ui_status_update_callback,
                    self.target_ip_port,
                    self._get_p2_input_snapshot_main_thread_passthrough, # Pass player instance
                    lambda: QApplication.processEvents()
                )
...
class MainWindow(QMainWindow):
    ...
    network_status_update = Signal(str, str, float) # Emitted by NetworkThread via _ui_status_update_callback
    lan_server_search_status = Signal(str, object)  # Emitted by LANServerSearchThread
    ...
    ip_input_dialog: Optional[IPInputDialog] = None # Changed to use game_ui.IPInputDialog
    current_modal_dialog: Optional[str] = None # Tracks which modal is open for input routing
    ...
    NUM_MAP_COLUMNS = 3
    NetworkThread = NetworkThread # Make class accessible for type hinting if needed elsewhere
    render_print_limiter = PrintLimiter(default_limit=1, default_period=3.0) # For limiting some MainWindow prints
    ...
    def __init__(self):
        ...
        try:
            ...
            self._pygame_joy_button_prev_state = [] # Initialize for each joystick found
            for i in range(pygame.joystick.get_count()):
                joy = pygame.joystick.Joystick(i); joy.init()
                self._pygame_joysticks.append(joy)
                self._pygame_joy_button_prev_state.append({}) # One dict per joystick
        ...
        # Initialize UI navigation state variables
        self._menu_selected_button_idx = 0
        self._map_selection_selected_button_idx = 0
        self._lan_search_list_selected_idx = 0 # For QListWidget selection
        self._ip_dialog_selected_button_idx = 0 # For Ok/Cancel buttons
        self._last_pygame_joy_nav_time = 0.0
        self._pygame_joy_axis_was_active_neg = {} # Key: joy_idx or f"{joy_idx}_x"
        self._pygame_joy_axis_was_active_pos = {} # Key: joy_idx or f"{joy_idx}_x"
        self._main_menu_buttons_ref = []
        self._map_selection_buttons_ref = [] # Will hold QPushButton map items
        self._ip_dialog_buttons_ref = [] # For Ok/Cancel in IP dialog
        self._current_active_menu_buttons = self._main_menu_buttons_ref # Start with main menu
        self._current_active_menu_selected_idx_ref = "_menu_selected_button_idx" # Attr name on self to use
        ...
        try: game_config.load_config() # This now also forces P1 to keyboard
        ...
        self.map_select_widget = _create_map_select_widget(self) # Now uses QGridLayout
        ...
        self.lan_server_search_status.connect(self.on_lan_server_search_status_update_slot) # Connect the new signal
    ...
    def _handle_config_load_failure(self):
        ...
        # Ensure P1 is keyboard if config fails badly
        game_config.CURRENT_P1_INPUT_DEVICE = game_config.DEFAULT_P1_INPUT_DEVICE
        game_config.P1_MAPPINGS = game_config.DEFAULT_KEYBOARD_P1_MAPPINGS.copy()
        game_config.CURRENT_P2_INPUT_DEVICE = game_config.DEFAULT_P2_INPUT_DEVICE
        game_config.P2_MAPPINGS = game_config.DEFAULT_KEYBOARD_P2_MAPPINGS.copy()
        info("MAIN PySide6: Fallback to default keyboard mappings due to config load failure.")
    ...
    @Slot(str, object) # Changed from (str, str, int, str) to (str, object) to match new LANServerSearchThread signal
    def on_lan_server_search_status_update_slot(self, status_key: str, data: object): app_game_modes.on_lan_server_search_status_update_logic(self, status_key, data) # Pass object type for data
    ...
    def on_return_to_menu_from_sub_view(self):
        ...
        if source_view == "editor" and self.actual_editor_module_instance:
            ...
            if hasattr(self.actual_controls_module_instance, 'save_mappings') and callable(self.actual_controls_module_instance.save_mappings):
                self.actual_controls_module_instance.save_mappings() # Save before detaching
    ...
    def show_view(self, view_name: str):
        ...
        if self.current_view_name == "game_scene" and view_name != "game_scene" and self.current_game_mode:
            self.stop_current_game_mode(show_menu=False) # Stop game but don't show menu yet if switching to editor/settings
        ...
        if view_name == "menu":
            ...
            self._current_active_menu_buttons = [] # No button nav in game scene itself
        ...
        if target_page:
            ...
            # QTimer.singleShot(0, lambda: focus_target.setFocus(Qt.FocusReason.OtherFocusReason))
            focus_target.setFocus(Qt.FocusReason.OtherFocusReason) # Try immediate focus
        ...
        clear_qt_key_events_this_frame() # Clear pending key events on view switch
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if active_ui_element in ["menu", "map_select"] and not event.isAutoRepeat():
            ...
            elif event.key() == Qt.Key.Key_Left and active_ui_element == "map_select": _navigate_current_menu_pygame_joy(self, -self.NUM_MAP_COLUMNS); event.accept(); return # Corrected map nav
            elif event.key() == Qt.Key.Key_Right and active_ui_element == "map_select": _navigate_current_menu_pygame_joy(self, self.NUM_MAP_COLUMNS); event.accept(); return # Corrected map nav
            ...
            # Allow normal typing in QLineEdit
            if event.key() not in [Qt.Key.Key_Left, Qt.Key.Key_Right, Qt.Key.Key_Return, Qt.Key.Key_Enter, Qt.Key.Key_Space, Qt.Key.Key_Escape, Qt.Key.Key_Tab, Qt.Key.Key_Backtab]:
                # Let QLineEdit handle most keys
                if self.ip_input_dialog.line_edit.hasFocus():
                    super().keyPressEvent(event) # Pass to QLineEdit
                    return
            # Handle navigation between buttons for IP dialog
            if event.key() in [Qt.Key.Key_Left, Qt.Key.Key_Right, Qt.Key.Key_Tab, Qt.Key.Key_Backtab]: # Tab/Shift+Tab or Left/Right to switch button focus
            ...
            elif event.key() in [Qt.Key.Key_Return, Qt.Key.Key_Enter, Qt.Key.Key_Space]: # Activate focused button
                _activate_ip_dialog_button(self); event.accept(); return
        ...
        if event.key() == Qt.Key.Key_Escape and not event.isAutoRepeat():
            ...
            else: self.show_view("menu") # Fallback
            event.accept(); return
    ...
    def get_p1_input_snapshot_for_server_thread(self, player_instance: Any) -> Dict[str, Any]:
        # Pass game_elements explicitly to get_input_snapshot
    ...
    def update_game_loop(self):
        _poll_pygame_joysticks_for_ui_navigation(self) # Handles UI nav for menus/dialogs
        
        # Process Pygame events if any joystick is configured to use Pygame for game input
        if self.current_game_mode and \
           ( (game_config.CURRENT_P1_INPUT_DEVICE and "joystick_pygame_" in game_config.CURRENT_P1_INPUT_DEVICE) or \
            ...
            pygame.event.pump() # Keep Pygame events flowing for joystick input during gameplay
        ...
        if self.current_game_mode == "couch_play" and self.app_status.app_running:
            if game_is_ready and not init_in_progress:
                ...
                continue_game = run_couch_play_mode(
                    ...
                    lambda: QApplication.processEvents(),
                    lambda: dt_sec,
                    lambda msg: self.game_scene_widget.update_game_state(0, download_msg=msg) # type: ignore
                )
                if not continue_game: self.stop_current_game_mode(show_menu=True)
            elif init_in_progress and MainWindow.render_print_limiter.can_print("waiting_init_couch"):
                debug("GameLoop (Couch): Waiting for game initialization to complete...")
            ...
            if game_is_ready and not init_in_progress:
                # Host game logic (P1 update, enemies, etc.) runs here
                # This part is now managed inside server_logic.run_server_mode's loop
                pass # Server thread handles its own game loop simulation
            elif init_in_progress and MainWindow.render_print_limiter.can_print("waiting_init_host_active"):
                debug("GameLoop (HostActive): Waiting for game initialization to complete...")
        
        elif self.current_game_mode == "host_waiting" and self.app_status.app_running and self.server_state:
            if game_is_ready and not init_in_progress:
                # Server is waiting for client, P1 can still move around locally
                dt_sec = 1.0 / C.FPS
            ...
            elif init_in_progress and MainWindow.render_print_limiter.can_print("waiting_init_host_wait"):
                debug("GameLoop (HostWait): Waiting for game initialization to complete...")
            ...
            # Client logic is mostly reactive, driven by its own thread and server updates.
            # Player input is fetched by the client_logic thread via callback.
            # Local animation updates happen in client_logic based on server state.
            pass
        
        if self.current_view_name == "game_scene":
            # Only update GameSceneWidget if elements are likely populated
            if self.game_elements.get("platforms_list") is not None or self.game_elements.get("level_data") is not None:
                 self.game_scene_widget.update_game_state(0) # 0 can be placeholder for current_ticks if not used by GSW
            else:
                if MainWindow.render_print_limiter.can_print("gsw_update_skipped_early"):
                     debug("GameSceneWidget update_game_state skipped as platforms_list/level_data not yet in game_elements (early in loop).")
        
        clear_qt_key_events_this_frame() # Clear discrete key events after processing for this frame
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.actual_editor_module_instance:
            ...
            if isinstance(self.actual_editor_module_instance, QMainWindow): # Editor is a QMainWindow
                # Attempt to call its closeEvent or a similar method if it has one
        ...
        self.app_status.quit_app() # This will signal threads and eventually QApplication.quit()
        app_game_modes.stop_current_game_mode_logic(self, show_menu=False) # Clean up current game mode state
...
def main():
    ...
    main_window = MainWindow(); main_window.showMaximized() # Show maximized as per your previous structure
    exit_code = app.exec()
    info(f"MAIN PySide6: QApplication event loop finished. Exit code: {exit_code}")
    if APP_STATUS.app_running: APP_STATUS.app_running = False # Ensure flag is set if loop exits some other way
...
if __name__ == "__main__":
    try:
        ...
        # Attempt to show a Qt message box
        try:
            error_app = QApplication.instance(); 
            if error_app is None: error_app = QApplication(sys.argv) # Must have an app instance
