...
def load_gif_frames(full_path_to_gif_file: str) -> List[QPixmap]:
        ...
        while True:
            try:
                ...
                qimage_frame = QImage(rgba_pil_frame.tobytes(), rgba_pil_frame.width, rgba_pil_frame.height, 
                                      rgba_pil_frame.width * 4, 
                                      QImage.Format.Format_RGBA8888)
                if qimage_frame.isNull():
                    error(f"Assets Error: QImage conversion failed for frame {frame_index} in '{full_path_to_gif_file}'."); frame_index += 1; continue
                qpixmap_frame = QPixmap.fromImage(qimage_frame)
                if qpixmap_frame.isNull():
                    error(f"Assets Error: QPixmap conversion failed for frame {frame_index} in '{full_path_to_gif_file}'."); frame_index += 1; continue
                loaded_frames.append(qpixmap_frame); frame_index += 1
            except EOFError: break
            except Exception as e_frame: error(f"Assets Error: Exception processing frame {frame_index} in '{full_path_to_gif_file}': {e_frame}"); frame_index += 1
    ...
    except Exception as e_load: error(f"Assets Error: General exception loading GIF '{full_path_to_gif_file}' with Pillow/Qt: {e_load}")
...
if __name__ == "__main__":
    ...
    if loaded_gray_enemy_animations: info(f"Assets Test (Gray Enemy): Successfully loaded ENEMY animation data. Found states: {', '.join(k for k,v in loaded_gray_enemy_animations.items() if v)}")
