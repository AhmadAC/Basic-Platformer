...
"""
# version 2.0.6 (Corrected map_module_name usage with LevelLoader)
import sys
import os
...
DEFAULT_LEVEL_MODULE_NAME = "original" # Default map folder/stem name
...
def initialize_game_elements(
    ...
    map_module_name: Optional[str] = None # This is now the map FOLDER/STEM name
) -> bool:
    ...
    # LevelLoader.load_map now expects the map_name (folder/stem) and the base maps directory
    ...
    game_elements_ref["loaded_map_name"] = current_map_to_load # Store the folder/stem name
    ...
    if for_game_mode in authoritative_modes_for_spawn:
        ...
        for i, statue_data in enumerate(game_elements_ref["statue_spawns_data_cache"]):
            try:
                s_id = statue_data.get('id', f"map_statue_rs_{i}") 
