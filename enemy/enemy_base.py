...
MODIFIED: Updated asset paths to reflect new 'assets/' root structure.
          - Generic enemy animations now assumed under 'assets/enemy_characters/soldier/{color_name}/'
          - Common stone/smashed assets now under 'assets/shared/Stone/'
MODIFIED: Added zapped attributes.
MODIFIED: Added overall_fire_effect_start_time for 5s fire cycle management.
MODIFIED: Zapped GIF path correctly formed for generic soldier types.
MODIFIED: Corrected logger fallback assignment.
MODIFIED: More robust logger setup for import failures using correct relative path.
"""
# version 2.0.12 (Logger import path fix, robust logger setup)
...
from main_game.assets import load_enemy_animations, load_gif_frames, resource_path

# --- Logger Setup ---
import logging
_enemy_base_logger_instance = logging.getLogger(__name__ + "_enemy_base_internal_fallback")
if not _enemy_base_logger_instance.hasHandlers():
    _handler_eb_fb = logging.StreamHandler(sys.stdout)
    _formatter_eb_fb = logging.Formatter('ENEMY_BASE (InternalFallback): %(levelname)s - %(filename)s:%(lineno)d - %(message)s')
    _handler_eb_fb.setFormatter(_formatter_eb_fb)
    _enemy_base_logger_instance.addHandler(_handler_eb_fb)
    _enemy_base_logger_instance.setLevel(logging.DEBUG)
    _enemy_base_logger_instance.propagate = False

def _fallback_log_info(msg, *args, **kwargs): _enemy_base_logger_instance.info(msg, *args, **kwargs)
def _fallback_log_debug(msg, *args, **kwargs): _enemy_base_logger_instance.debug(msg, *args, **kwargs)
def _fallback_log_warning(msg, *args, **kwargs): _enemy_base_logger_instance.warning(msg, *args, **kwargs)
def _fallback_log_error(msg, *args, **kwargs): _enemy_base_logger_instance.error(msg, *args, **kwargs)
def _fallback_log_critical(msg, *args, **kwargs): _enemy_base_logger_instance.critical(msg, *args, **kwargs)
...
try:
    ...
    debug("EnemyBase: Successfully aliased project's logger functions from main_game.logger.")
except ImportError:
    critical("CRITICAL ENEMY_BASE: Failed to import logger from main_game.logger. Using internal fallback print statements for logging.")
except Exception as e_logger_init:
    critical(f"CRITICAL ENEMY_BASE: Unexpected error during logger setup from main_game.logger: {e_logger_init}. Using internal fallback.")
# --- End Logger Setup ---
...
class EnemyBase:
    def __init__(self, start_x: float, start_y: float, patrol_area: Optional[QRectF] = None,
        ...
        character_base_asset_folder = os.path.join("assets", "enemy_characters", "soldier")
        ...
        elif color_name and color_name not in available_enemy_asset_folders and color_name != "knight_type_specific":
            warning(f"EnemyBase Warning (ID: {self.enemy_id}): Processed color '{processed_color_input}' (from input '{color_name}') not in soldier colors {available_enemy_asset_folders}. Using random soldier color if available.")
            self.final_asset_color_name = random.choice(available_enemy_asset_folders) if available_enemy_asset_folders else "unknown"
        elif not color_name:
            self.final_asset_color_name = random.choice(available_enemy_asset_folders) if available_enemy_asset_folders else "unknown"
        else:
            self.final_asset_color_name = color_name
        ...
        if self.final_asset_color_name == "unknown":
            if available_enemy_asset_folders:
                ...
                warning(f"EnemyBase Warning (ID: {self.enemy_id}): color_name resolved to 'unknown', falling back to first available soldier color: '{self.final_asset_color_name}'.")
            else:
                critical(f"EnemyBase CRITICAL (ID: {self.enemy_id}): No available enemy asset folders (soldier colors) to choose from. Animation loading will fail.")
                self._valid_init = False
        
        self.color_name = self.final_asset_color_name
        ...
        if self._valid_init and self.final_asset_color_name != "unknown" and self.final_asset_color_name in available_enemy_asset_folders:
            chosen_enemy_asset_folder = os.path.join(character_base_asset_folder, self.final_asset_color_name)
            debug(f"EnemyBase (ID: {self.enemy_id}): Loading generic ENEMY (soldier-type) animations from: '{chosen_enemy_asset_folder}'")
            self.animations = load_enemy_animations(relative_asset_folder=chosen_enemy_asset_folder)
        elif self._valid_init and self.final_asset_color_name == "knight_type_specific":
            debug(f"EnemyBase (ID: {self.enemy_id}): final_asset_color_name is '{self.final_asset_color_name}'. Knight subclass will handle specific animation loading.")
            self.animations = {}
        else:
            warning(f"EnemyBase (ID: {self.enemy_id}): final_asset_color_name '{self.final_asset_color_name}' is not a standard soldier color. Animation loading might be handled by subclass or fail.")
            self.animations = {}

        self.image: Optional[QPixmap] = None
        self.rect = QRectF()

        if self.animations is None or (self.final_asset_color_name in available_enemy_asset_folders and not self.animations):
            critical_msg_suffix = f"from '{chosen_enemy_asset_folder}'" if 'chosen_enemy_asset_folder' in locals() else "(no valid color/asset path)"
        ...
        elif self.animations is not None: # Check explicitly for not None
            self._last_facing_right_visual: bool = True
            ...
            if not initial_idle_animation or not initial_idle_animation[0] or initial_idle_animation[0].isNull():
                 warning(f"EnemyBase Warning (ID: {self.enemy_id}, FinalAssetColor: {self.final_asset_color_name}): ENEMY 'idle' animation missing/empty in loaded set. Attempting fallback.")
            ...
            if initial_idle_animation and initial_idle_animation[0] and not initial_idle_animation[0].isNull():
                ...
                if self.final_asset_color_name != "knight_type_specific":
        ...
        self.death_animation_finished: bool = not self._valid_init
        ...
        try:
            if self.animations and self.animations.get('idle') and self.animations['idle'][0] and not self.animations['idle'][0].isNull():
                self.standard_height: float = float(self.animations['idle'][0].height())
                 ...
                 self.standard_height: float = float(self.image.height())
        ...
        if self.final_asset_color_name in available_enemy_asset_folders :
            self.zapped_gif_path = os.path.join(character_base_asset_folder, self.final_asset_color_name, "__Zapped.gif")
        else:
            self.zapped_gif_path = None
        ...
        if self._valid_init:
            stone_common_folder = os.path.join('assets', 'shared', 'Stone')
    ...
    def reset(self):
        if not self._valid_init and self.animations is None and \
           self.final_asset_color_name != "unknown" and \
           self.final_asset_color_name != "knight_type_specific" and \
           self.final_asset_color_name in ['green', 'pink', 'purple', 'gray', 'yellow', 'orange']:
            
            soldier_base_folder_reset = os.path.join("assets", "enemy_characters", "soldier")
            chosen_enemy_asset_folder_on_reset = os.path.join(soldier_base_folder_reset, self.final_asset_color_name)
            
            self.animations = load_enemy_animations(relative_asset_folder=chosen_enemy_asset_folder_on_reset)
            if self.animations is not None:
                ...
                warning(f"EnemyBase (ID: {self.enemy_id}): Still failed to load anims on reset for soldier type. Remains invalid.")
        ...
        if painter.window().intersects(screen_rect_qrectf.toRect()):
            ...
            if getattr(C, "DRAW_ENEMY_ABOVE_HEALTH_BAR", False) and \
               self.current_health < self.max_health and not self.is_dead and not self.is_petrified:
                ...
                try:
                    from main_game.game_ui import draw_health_bar_qt
                    ...
                    if hasattr(self, 'print_limiter') and self.print_limiter.can_log(f"draw_health_bar_missing_{self.enemy_id}"):
                        warning(f"EnemyBase {self.enemy_id}: main_game.game_ui.draw_health_bar_qt not found, cannot draw health bar.")