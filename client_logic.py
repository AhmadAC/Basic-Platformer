        ...
        for event in pygame.event.get():
             ...
             if event.type == pygame.VIDEORESIZE:
                if not screen.get_flags() & pygame.FULLSCREEN:
                    ...
                    search_text_surf = fonts.get("large", pygame.font.Font(None,30)).render("Searching...", True, C.WHITE)
...
def run_client_mode(screen: pygame.Surface, clock: pygame.time.Clock,
    ...
    while map_sync_phase_active and client_state_obj.app_running:
        ...
        if not client_state_obj.app_running: break
        dialog_title = "Synchronizing"; dialog_msg_map = "Waiting for map info..."; dialog_prog_map = -1
        if client_state_obj.map_download_status == "checking": dialog_msg_map = f"Checking for map: {client_state_obj.server_selected_map_name}..."
            ...
            dialog_msg_map = f"{client_state_obj.map_received_bytes/1024:.1f}/{client_state_obj.map_total_size_bytes/1024:.1f}KB"
            dialog_prog_map = client_state_obj.map_download_progress if client_state_obj.map_total_size_bytes > 0 else 0
        ...
        elif client_state_obj.map_download_status == "error": dialog_title = "Map Error"; dialog_msg_map = f"Failed map: {client_state_obj.server_selected_map_name}"; dialog_prog_map = -1
        ...
        try:
            ...
            for msg in decoded_messages:
                ...
                if cmd == "set_map":
                    ...
                    if client_state_obj.map_total_size_bytes > 0: client_state_obj.map_download_progress = (client_state_obj.map_received_bytes/client_state_obj.map_total_size_bytes)*100
    ...
    updated_game_elements = initialize_game_elements(current_screen_width, current_screen_height, "join_ip", game_elements_ref, client_state_obj.server_selected_map_name)
    ...
    if game_elements_ref.get("camera") and "level_pixel_width" in game_elements_ref:
        ...
        if hasattr(cam, "set_level_dimensions"): cam.set_level_dimensions(game_elements_ref["level_pixel_width"], game_elements_ref["level_min_y_absolute"], game_elements_ref["level_max_y_absolute"])
    pygame.display.set_caption(f"Platformer - CLIENT (You are P2: WASD+VB | Self-Harm:H | Heal:G | Reset:Enter | Weapons: {C.P2_SHADOW_PROJECTILE_KEY % 1000 - C.K_KP0 + 6 if C.K_0 <= C.P2_SHADOW_PROJECTILE_KEY <= C.K_9 else '?'}, {C.P2_GREY_PROJECTILE_KEY % 1000 - C.K_KP0 + 7 if C.K_0 <= C.P2_GREY_PROJECTILE_KEY <= C.K_9 else '?'})")
    p2_controlled_by_client = game_elements_ref.get("player2"); p1_remote_on_client = game_elements_ref.get("player1")
    debug(f"Client: P1 (remote): {p1_remote_on_client}, P2 (local): {p2_controlled_by_client}")
    if p1_remote_on_client: debug(f"Client P1 Valid: {p1_remote_on_client._valid_init}, Pos: {p1_remote_on_client.pos if hasattr(p1_remote_on_client,'pos') else 'N/A'}")
    if p2_controlled_by_client: debug(f"Client P2 Valid: {p2_controlled_by_client._valid_init}, Pos: {p2_controlled_by_client.pos if hasattr(p2_controlled_by_client,'pos') else 'N/A'}")
    p2_client_key_map = {'left':pygame.K_a,'right':pygame.K_d,'up':pygame.K_w,'down':pygame.K_s,'attack1':pygame.K_v,'attack2':pygame.K_b,'dash':pygame.K_LSHIFT,'roll':pygame.K_LCTRL,'interact':pygame.K_e}
    client_game_active = True; client_state_obj.server_state_buffer = b""; client_state_obj.last_received_server_state = None
    
    p2_local_action_events: Dict[str, bool] = {} # For client-side pause

    while client_game_active and client_state_obj.app_running:
        dt_sec = clock.tick(C.FPS) / 1000.0; now_ticks_client = pygame.time.get_ticks()
        client_initiated_actions = {'action_reset':False,'action_self_harm':False,'action_heal':False,'fire_shadow_event':False,'fire_grey_event':False, 'pause_event': False} # Added pause_event
        server_indicated_game_over = client_state_obj.last_received_server_state and client_state_obj.last_received_server_state.get('game_over', False)
        
        pygame_events_client = pygame.event.get()
        keys_pressed_client = pygame.key.get_pressed()

        # --- Client-side input processing for local actions (like pause/exit) ---
        if p2_controlled_by_client and p2_controlled_by_client._valid_init and not p2_controlled_by_client.is_dead:
            p2_local_action_events = p2_controlled_by_client.process_input(pygame_events_client, game_elements_ref.get("platform_sprites"), keys_pressed_override=keys_pressed_client)
        
        if p2_local_action_events.get("pause"): # Check if P2 pressed their mapped pause button
            info("Client: P2 local pause action detected. Requesting exit to server.")
            client_initiated_actions['pause_event'] = True # Signal server about pause intention
            client_game_active = False # Also allow local exit
        ...
        for event in pygame_events_client:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE: client_game_active = False # K_ESCAPE is a direct exit
                if event.key == pygame.K_RETURN and server_indicated_game_over: client_initiated_actions['action_reset']=True
                ...
                if p2_controlled_by_client:
                    if event.key == C.P2_SHADOW_PROJECTILE_KEY: client_initiated_actions['fire_shadow_event']=True
                    if event.key == C.P2_GREY_PROJECTILE_KEY: client_initiated_actions['fire_grey_event']=True
        ...
        if p2_controlled_by_client and hasattr(p2_controlled_by_client, 'get_input_state_for_network'):
             p2_input_state_for_server = p2_controlled_by_client.get_input_state_for_network(keys_pressed_client, pygame_events_client, p2_client_key_map)
        p2_input_state_for_server.update(client_initiated_actions) # Include the pause_event flag
        if client_state_obj.client_tcp_socket:
        ...
        if client_camera:
            ...
            elif p2_controlled_by_client and p2_controlled_by_client.alive(): cam_focus_target_client=p2_controlled_by_client
