...
def run_couch_play_mode(
    ...
    if current_chest and isinstance(current_chest, Chest) and current_chest.alive() and \
       not current_chest.is_collected_flag_internal and current_chest.state == 'closed':
        ...
        # Store rect's bottom before applying physics for this step.
        # This is a snapshot of where the chest *was* before this frame's physics step moved its pos_midbottom.
        # We must ensure rect is up-to-date with pos_midbottom BEFORE this snapshot.
        ...
        if hasattr(current_chest, 'apply_physics_step'):
            current_chest.apply_physics_step(dt_sec) # This updates vel_y and pos_midbottom, then rect
        ...
        if hasattr(current_chest, 'rect') and isinstance(current_chest.rect, QRectF):
            for platform_collidable in platforms_list:
                ...
                if current_chest.rect.intersects(platform_collidable.rect):
                    # Condition: Chest is moving down (or was stationary and started intersecting due to platform move - less likely for static platforms)
                    # AND its bottom edge is now at or below the platform's top edge
                    # AND its previous bottom edge was at or above the platform's top edge (or very close)
                    if current_chest.vel_y >= 0 and \
                       current_chest.rect.bottom() >= platform_collidable.rect.top() and \
                       old_chest_bottom_for_collision_check <= platform_collidable.rect.top() + 1.0 : # +1 for small overlaps
                        ...
                        if actual_overlap_width_chest >= min_horizontal_overlap_chest:
                            ...
                            # log_debug(f"Chest landed on platform: {platform_collidable.rect}") # Optional
                            break
            
            if hasattr(current_chest, '_update_rect_from_image_and_pos'): # Final sync after potential collision adjustment
                current_chest._update_rect_from_image_and_pos()
        else:
    ...
    for collectible in list(collectible_items_list):
    ...
    return True# couch_play_logic.py
# -*- coding: utf-8 -*-
"""
Handles game logic for local couch co-op mode using PySide6.
UI rendering and input capture are handled by the main Qt application.
