...
# -*- coding: utf-8 -*-
"""
## version 2.0.10 (Start maximized in standalone if no saved geometry)
...
from PySide6.QtWidgets import (
    ...
    QColorDialog, QInputDialog, QLabel, QSizePolicy # Added QSizePolicy
)
...
class EditorMainWindow(QMainWindow):
    def __init__(self, parent: Optional[QWidget] = None, embed_mode: bool = False): 
        ...
        # Window title and initial geometry are handled after UI init and restore attempt for standalone
        ...
        if not self._is_embedded:
            ...
            if restored_successfully:
                ...
                self.show() # Show with restored size/position
            else:
                logger.info("Standalone mode: No saved geometry/state or restoration failed. Showing maximized.")
                # Set a default geometry before maximizing, which can be useful on some systems
            ...
            # Still try to restore dock state etc., even if not showing the main window itself.
    ...
    def export_map_as_image(self):
        ...
        try:
            ...
            image_size = QSizePolicy(int(target_rect.width()), int(target_rect.height())) # type: ignore # QSizePolicy used as QSize
            if image_size.width() <=0 or image_size.height() <=0: QMessageBox.critical(self, "Export Error", f"Invalid image dimensions for export: {image_size.width()}x{image_size.height()}"); return # type: ignore
            image = QImage(image_size, QImage.Format.Format_ARGB32_Premultiplied) # type: ignore
    ...
    if not embed_mode: 
        ...
        try:
            if not main_window.isVisible() and not main_window._is_embedded: 
                 ...
                 # self.show() or self.showMaximized() is now handled inside __init__ for standalone
            exit_code = app.exec()
