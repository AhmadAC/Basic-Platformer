...
VERSION 2.1.0 (Pygame-free export)
"""
import sys
import os
import json
...
# Main game constants (already Pygame-free)
try:
...
def export_map_to_game_python_script(editor_state: EditorState) -> bool:
    logger.info(f"Exporting map data (Pygame-free). Map name: '{editor_state.map_name_for_function}'")
...
# Generated by Platformer Level Editor (PySide6 Version - Pygame-free data export)
...
"""
    try:
        ...
        logger.info(f"Map data exported to game script (Pygame-free): {os.path.basename(py_filepath_to_use)}")
