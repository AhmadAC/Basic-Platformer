...
VERSION 2.3.6 (Removed all stray semicolons)
"""
import sys
import os
import json
...
def init_new_map_state(editor_state: EditorState, map_name_for_function: str,
    ...
    # (Logic from version 2.3.2 remains - only P1/P2 default spawns)
    ...
    if border_asset_data:
        ...
        for i in range(map_width_tiles): # Top
        ...
        for i in range(map_width_tiles): # Bottom
        ...
        for i in range(2, map_height_tiles - 2): # Left
        ...
        for i in range(2, map_height_tiles - 2): # Right
...
def save_map_to_json(editor_state: EditorState) -> bool:
    ...
    maps_abs_dir = getattr(C, "MAPS_DIR", ED_CONFIG.MAPS_DIRECTORY)
    if not os.path.isabs(maps_abs_dir):
        maps_abs_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), maps_abs_dir)
...
def load_map_from_json(editor_state: EditorState, json_filepath: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Error loading map from JSON '{json_filepath}': {e}", exc_info=True)
        return False
...
def _merge_rect_objects_to_data(objects_raw: List[Dict[str, Any]], object_category_name: str) -> List[Dict[str, Any]]:
    ...
    for obj_orig in objects_raw:
        ...
        if 'image_path' in obj_orig:
            obj['image_path'] = obj_orig['image_path']
        ...
        for j in range(i + 1, len(sorted_h)):
            ...
            if str(p_next.get('type')) == str(current_strip.get('type')) and \
               ...
               str(p_next.get('image_path', '')) == str(current_strip.get('image_path', '')):
                current_strip['w'] += p_next.get('w', 0.0)
                p_next['merged'] = True
    ...
    for i, s_base in enumerate(sorted_v):
        ...
        for j in range(i + 1, len(sorted_v)):
            ...
            if str(s_next.get('type')) == str(current_block.get('type')) and \
               ...
               str(s_next.get('image_path', '')) == str(current_block.get('image_path', '')):
                current_block['h'] += s_next.get('h', 0.0)
                s_next['merged'] = True
        ...
        final_entry = {
            'rect': (current_block.get('x'), current_block.get('y'), current_block.get('w'), current_block.get('h')),
            'type': current_block.get('type'),
            'color': current_block.get('color'),
            'properties': current_block.get('properties', {})
        }
        if 'image_path' in current_block:
            final_entry['image_path'] = current_block['image_path']
...
def export_map_to_game_python_script(editor_state: EditorState) -> bool:
    ...
    if not ensure_maps_directory_exists():
        logger.error(f"Maps directory issue. PY data export aborted.")
        return False
    ...
    for obj_data in editor_state.placed_objects:
        ...
        if not all([object_game_type_id != "unknown", wx is not None, wy is not None, asset_key != ""]):
            continue
        ...
        if asset_entry.get("surface_params"):
            ...
            orig_w_tuple_val, orig_h_tuple_val = asset_entry["original_size_pixels"]
        ...
        is_platform_type = any(pt in object_game_type_id.lower() for pt in ["platform", "wall", "ground", "ledge"])
        is_ladder_type = "ladder" in object_game_type_id.lower()
        is_hazard_type = "hazard" in object_game_type_id.lower()
        is_statue_type = "object_stone" in object_game_type_id.lower()
...
def delete_map_files(editor_state: EditorState, json_filepath_to_delete: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Error deleting map files for '{map_name_base}': {e}", exc_info=True)
        return False
        ...
        if filename.endswith(".py") and filename != "__init__.py":
            ...
            try:
                ...
                def replace_func(match):
                    ...
                    key_as_string_literal = captured_key_name[1:-1] if (captured_key_name.startswith("'") and captured_key_name.endswith("'")) or \
                                                                    (captured_key_name.startswith('"') and captured_key_name.endswith('"')) else captured_key_name
                    found_match = True
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(maps_dir_to_fix):
        print(f"ERROR: Maps directory does not exist: {maps_dir_to_fix}")
    else:
