#################### START OF FILE: editor_file_operations.py ####################
...
import os
import shutil
from PySide6.QtCore import Qt, Slot, QTimer, Signal, Slot, QThread, QSize, QTimer, QRectF # Added QRectF for type hinting
...
if TYPE_CHECKING:
    from .editor_main_window import EditorMainWindow # Use this if you rename editor.py
    # from .editor import EditorMainWindow # Or this if editor.py remains the main window file
...
def _internal_save_map_json(editor_state: 'EditorState', parent_window: 'EditorMainWindow') -> bool:
    ...
    if editor_map_utils.save_map_to_json(editor_state):
        if logger: logger.info(f"Editor data saved: {os.path.basename(editor_state.current_json_filename or 'unknown.json')}.")
        return True
    else:
...
def _internal_export_map_py(editor_state: 'EditorState', parent_window: 'EditorMainWindow') -> bool:
    ...
    if editor_map_utils.export_map_to_game_python_script(editor_state):
        if logger: logger.info(f"Map exported for game: {os.path.basename(editor_state.current_map_filename or 'unknown.py')}.")
        return True
    else:
    ...
    if ok and map_name:
        ...
        if ok_size and size_str:
            try:
                ...
                if not main_win._is_embedded: # Assuming _is_embedded is accessible or passed
...
def load_map_action(main_win: 'EditorMainWindow'):
    ...
    file_path, _ = QFileDialog.getOpenFileName(main_win, "Load Map JSON file", maps_base_dir, json_filter)
    if file_path:
        if logger: logger.info(f"Attempting to load map from: {file_path}")
        if editor_map_utils.load_map_from_json(editor_state, file_path):
            main_win.map_view_widget.load_map_from_state()
            main_win.asset_palette_widget.clear_selection()
            main_win.asset_palette_widget.populate_assets()
            main_win.properties_editor_widget.clear_display()
            ...
            QMessageBox.critical(main_win, "Load Error", f"Failed to load map from: {os.path.basename(file_path)}")
    else:
...
def save_map_action(main_win: 'EditorMainWindow') -> bool:
    ...
    if json_saved_ok and py_exported_ok:
        ...
        editor_state.unsaved_changes = True # Still unsaved if PY export failed
        ...
        if not main_win._is_embedded:
            main_win.update_window_title() # Show '*'
        main_win.update_edit_actions_enabled_state()
        return False
...
def rename_map_action(main_win: 'EditorMainWindow'):
    ...
    if ok and new_name_str:
        ...
        new_map_folder_path = editor_map_utils.get_map_specific_folder_path(editor_state, clean_new_name) # Important: use editor_state for consistency
        ...
        if not os.path.exists(old_map_folder_path):
            ...
            return
        try:
            if logger: logger.info(f"Attempting rename of map folder '{old_map_folder_path}' to '{new_map_folder_path}'.")
            ...
            # This re-initializes paths within editor_state based on the new name
            ...
            if not save_map_action(main_win): # Try to save with new name
            ...
            if not main_win._is_embedded:
                main_win.update_window_title()
            main_win.update_edit_actions_enabled_state()
            main_win.asset_palette_widget.populate_assets() # Refresh asset palette (e.g. custom assets paths)
            main_win.selection_pane_widget.populate_items()
...
def delete_map_folder_action(main_win: 'EditorMainWindow'):
    ...
    if reply == QMessageBox.StandardButton.Yes:
        if editor_map_utils.delete_map_folder_and_contents(editor_state, map_name_to_delete):
            ...
            editor_state.reset_map_context() # Resets current_map_filename etc.
            main_win.map_view_widget.load_map_from_state()
            main_win.asset_palette_widget.clear_selection()
            main_win.asset_palette_widget.populate_assets()
            main_win.properties_editor_widget.clear_display()
    ...
    try:
        ...
        for item_id, item_view in main_win.map_view_widget._map_object_items.items():
            ...
            if obj_data_item and obj_data_item.get("editor_hidden", False):
                if item_view.isVisible(): # Only backup if it was visible but should be hidden for export
        ...
        for item_view_bkp in hidden_items_backup:
            item_view_bkp.setVisible(True) # Restore original visibility
...
#################### END OF FILE: editor_file_operations.py ####################