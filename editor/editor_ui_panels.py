...
from PySide6.QtGui import QIcon, QPalette, QColor, QPixmap, QFocusEvent, QKeyEvent, QPainter, QImage
from PySide6.QtCore import Qt, Signal, Slot, QSize

from . import editor_config as ED_CONFIG
from .editor_state import EditorState
from . import editor_history
from . import editor_map_utils # For getting map folder path
from .editor_assets import get_asset_pixmap # For palette icons
...
class AssetPaletteWidget(QWidget):
    asset_selected = Signal(str) # asset_editor_key or custom:filename.ext
    ...
    def __init__(self, editor_state: EditorState, parent: Optional[QWidget] = None):
        ...
        self.asset_list_widget.setIconSize(QSize(ED_CONFIG.ASSET_THUMBNAIL_SIZE, ED_CONFIG.ASSET_THUMBNAIL_SIZE)) # type: ignore
    ...
    def _on_select_paint_color(self):
        ...
        if new_q_color.isValid():
            self.editor_state.current_selected_asset_paint_color = new_q_color.getRgb()[:3] # type: ignore
    ...
    def populate_assets(self, filter_override: Optional[str] = None):
        ...
        item_cell_width = 85
        item_cell_height = ED_CONFIG.ASSET_THUMBNAIL_SIZE + 20 # type: ignore
        ...
        if current_filter_text.lower() == "custom":
            if self.editor_state.map_name_for_function and self.editor_state.map_name_for_function != "untitled_map":
                ...
                no_map_item.setFlags(no_map_item.flags() & ~Qt.ItemFlag.ItemIsEnabled) # type: ignore
        ...
        assets_to_display_tuples.sort(key=lambda x: x[0])

        new_selected_item_index = -1
        for idx, (key, data) in enumerate(assets_to_display_tuples):
            pixmap_for_palette: Optional[QPixmap] = None
            if key.startswith(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX): # type: ignore
                temp_qimage = QImage(data["source_file"])
                if not temp_qimage.isNull():
                    data["original_size_pixels"] = (temp_qimage.width(), temp_qimage.height())
                    pixmap_for_palette = get_asset_pixmap(key, data,
                                                          QSize(ED_CONFIG.ASSET_THUMBNAIL_SIZE, ED_CONFIG.ASSET_THUMBNAIL_SIZE), # type: ignore
                                                          get_native_size_only=False)
            else:
                 pixmap_for_palette = data.get("q_pixmap")
            ...
            if pixmap_for_palette and not pixmap_for_palette.isNull():
                ...
                list_item = QListWidgetItem(QIcon(pixmap_for_palette), "")
    ...
    def on_item_clicked(self, item: QListWidgetItem):
        if not (item.flags() & Qt.ItemFlag.ItemIsEnabled): # type: ignore
            return
        ...
        if asset_key_or_custom_id.startswith(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX): # type: ignore
            ...
            if asset_data_for_logic:
                if asset_data_for_logic.get("category") == "tool" or str(asset_data_for_logic.get("game_type_id", "")).startswith("tool_"):
                    self.tool_selected.emit(asset_key_or_custom_id)
                    if hasattr(self.parent_window, 'show_status_message'):
                        self.parent_window.show_status_message(f"Tool selected: {display_name_for_status}") # type: ignore
                else:
                    self.asset_selected.emit(asset_key_or_custom_id)
                    if hasattr(self.parent_window, 'show_status_message'):
                        self.parent_window.show_status_message(f"Asset selected: {display_name_for_status}") # type: ignore
    ...
    def _update_sub_focus_visuals(self):
        ...
        self.asset_list_widget.setStyleSheet(f"""
            QListWidget::item:selected {{ border: 1px solid #007ACC; background-color: #c0d5eA; color: black; }} 
        ...
        if self._controller_has_focus:
            focus_border_style_str = str(ED_CONFIG.ASSET_PALETTE_CONTROLLER_SUBFOCUS_BORDER) # type: ignore
...
class PropertiesEditorDockWidget(QWidget):
    properties_changed = Signal(dict) # map_object_data_ref
    controller_focus_requested_elsewhere = Signal()
    upload_image_for_trigger_requested = Signal(dict) # trigger_object_data_ref
    ...
    def display_asset_properties(self, asset_key_or_custom_id: Optional[str]):
        ...
        asset_data: Optional[Dict] = None # type: ignore
        ...
        if asset_key_or_custom_id.startswith(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX): # type: ignore
            ...
            filename = asset_key_or_custom_id.split(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX)[1] # type: ignore
    ...
    def display_custom_asset_palette_info(self, custom_asset_id: str):
        self._clear_dynamic_widgets_from_form()
        self.current_object_data_ref = None
        self.current_asset_type_for_defaults = custom_asset_id

        filename = custom_asset_id.split(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX)[1] # type: ignore
        title = f"Custom Asset: {filename}"
        ...
        if game_type_id_for_props and game_type_id_for_props in ED_CONFIG.EDITABLE_ASSET_VARIABLES: # type: ignore
            ...
            self.form_layout.addRow(QLabel("No specific default properties defined."))
        self.scroll_widget.adjustSize()
    ...
    def _create_property_field(self, var_name: str, definition: Dict[str, Any], current_value: Any, layout: QFormLayout):
        ...
        if prop_type == "int":
            ...
            self._update_rgba_color_button_style(color_button, color_val_prop) # type: ignore
    ...
    def _create_dimension_fields(self, obj_data_ref: Dict[str, Any], layout: QFormLayout):
        ...
        height_spinner.setMinimum(int(ED_CONFIG.BASE_GRID_SIZE / 4)) # type: ignore
        height_spinner.setMaximum(int(ED_CONFIG.BASE_GRID_SIZE * 100)) # type: ignore
        height_spinner.setValue(int(obj_data_ref.get("current_height", ED_CONFIG.BASE_GRID_SIZE))) # type: ignore
    ...
    def _change_rgba_color_property(self, var_name: str, button_ref: QPushButton):
        ...
        dialog.setCurrentColor(QColor(*current_color_val)) # type: ignore
        
        if dialog.exec():
            new_q_color = dialog.selectedColor()
            new_color_tuple_rgba = new_q_color.getRgb()
            
            if props.get(var_name) != new_color_tuple_rgba: # type: ignore
                editor_history.push_undo_state(self.editor_state) # type: ignore
                props[var_name] = new_color_tuple_rgba # type: ignore
    ...
    def _on_property_value_changed(self, var_name: str, new_value: Any):
        ...
        if var_name not in self.current_object_data_ref: # Check if it's a sub-property
    ...
    def _change_object_color(self, map_object_data_ref: Optional[Dict[str, Any]]):
        ...
        if new_q_color.isValid():
            new_color_tuple = new_q_color.getRgb()[:3] # type: ignore
