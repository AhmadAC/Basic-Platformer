...
from PySide6.QtGui import (
    ...
    QWheelEvent, QMouseEvent, QKeyEvent, QFocusEvent, QCursor, QContextMenuEvent
)
...
# NEW: Import custom item classes
...
# Renamed original MapObjectItem to StandardMapObjectItem to avoid confusion
# with BaseResizableMapItem if both were to coexist with similar names.
# For this version, we assume all palette items that are not custom images/triggers
# will also use a BaseResizableMapItem or a similar structure if they need resizing.
# For simplicity, let's assume StandardMapObjectItem is for non-resizable palette items for now.
class StandardMapObjectItem(QGraphicsPixmapItem): # For regular palette assets
    def __init__(self, editor_key: str, game_type_id: str, pixmap: QPixmap,
        ...
        self.setZValue(map_object_data_ref.get("layer_order", 0)) # Standard assets also respect layer_order
...
class MapViewWidget(QGraphicsView):
    ...
    def __init__(self, editor_state: EditorState, parent: Optional[QWidget] = None):
        ...
        self.map_scene.setProperty("is_resizing_item_internal", False) 
        ...
        # Now stores BaseResizableMapItem or StandardMapObjectItem
        self._map_object_items: Dict[int, QGraphicsItem] = {} # Keyed by id(obj_data)
        ...
        self._resizing_item_ref: Optional[BaseResizableMapItem] = None # Now BaseResizableMapItem
        self._resize_start_mouse_pos_scene: Optional[QPointF] = None
        self._resize_start_item_rect_scene: Optional[QRectF] = None # Item's original scene rect
    ...
    # Controller focus methods... (largely unchanged, ensure they use correct item types if needed)
    def _update_controller_cursor_visuals(self): # Unchanged
    ...
    def on_controller_focus_gained(self): self._controller_has_focus = True; self.editor_state.controller_mode_active = True; # ... (rest unchanged)
    def on_controller_focus_lost(self): self._controller_has_focus = False; self.editor_state.controller_mode_active = False; # ... (rest unchanged)
    def handle_controller_action(self, action: str, value: Any): # Unchanged in logic, but _perform_place_action is now more complex
        ...
        # ... (rest of controller actions unchanged)
    ...
    def _select_object_at_controller_cursor(self): # Logic for item type check
        ...
        found_item_to_select: Optional[QGraphicsItem] = None # Now generic QGraphicsItem
        for item in items_at_cursor:
            if isinstance(item, (StandardMapObjectItem, BaseResizableMapItem)): # Check for our types
        ...
        if found_item_to_select:
            ...
            # Emit the map_object_data_ref for properties panel
    ...
    def clear_scene(self): # Updated to handle BaseResizableMapItem handles
    ...
    def load_map_from_state(self): # Largely unchanged
    ...
    def update_background_color(self, emit_view_changed=True): # Unchanged
        self.map_scene.setBackgroundBrush(QColor(*self.editor_state.background_color))
        if emit_view_changed: self.view_changed.emit()

    def draw_grid(self): # Unchanged
    ...
    def update_grid_visibility(self, emit_view_changed=True): # Unchanged
    ...
    def draw_placed_objects(self): # MODIFIED to use new item classes
        ...
        for item_id in items_to_remove_ids:
            ...
            if isinstance(map_obj_item_to_remove, BaseResizableMapItem):
                map_obj_item_to_remove.show_resize_handles(False)
                for handle in map_obj_item_to_remove.resize_handles:
                    if handle.scene(): self.map_scene.removeItem(handle)
                map_obj_item_to_remove.resize_handles.clear()
        ...
        for obj_data in sorted_placed_objects:
            ...
            map_scene_item: Optional[QGraphicsItem] = None # Base type for all items

            if item_data_id in self._map_object_items: 
                map_scene_item = self._map_object_items[item_data_id]
                # Update existing item's data reference and visuals
                ...
                # Ensure position if it was changed via properties
            ...
            if isinstance(map_scene_item, BaseResizableMapItem) and map_scene_item.isSelected():
                map_scene_item.show_resize_handles(True)
    ...
    # screen_to_scene_coords, etc. remain same
    ...
    def scale_view(self, factor: float): # Unchanged
    ...
    def keyPressEvent(self, event: QKeyEvent): # Unchanged in this context
        # ... (key press logic from previous, including resize cancel with Esc) ...
        key = event.key(); modifiers = event.modifiers()
        if self._is_resizing_item and key == Qt.Key.Key_Escape:
            self._is_resizing_item = False; self._resizing_item_ref = None; self.map_scene.setProperty("is_resizing_item_internal", False)
            # This simple revert might not be ideal if undo stack was already pushed for resize start.
            # A more robust cancel would be to pop from undo stack here.
            # For now, we rely on the user to undo if they want the exact pre-resize state.
            self.map_view_widget.load_map_from_state() # Re-load to revert any partial resize
            QApplication.restoreOverrideCursor(); self.show_status_message("Resize cancelled."); event.accept(); return

        if modifiers & Qt.KeyboardModifier.ControlModifier:
            if key == Qt.Key.Key_Plus or key == Qt.Key.Key_Equal: self.zoom_in(); event.accept(); return
            # ... other Ctrl shortcuts
        # ... other key pan/delete logic
        super().keyPressEvent(event)

    def mousePressEvent(self, event: QMouseEvent): # MODIFIED for resize handles
        self._is_dragging_map_object = False
        self._is_resizing_item = False 
        self.map_scene.setProperty("is_resizing_item_internal", False)
        ...
        for item_candidate in items_under_mouse:
            if isinstance(item_candidate, QGraphicsRectItem) and isinstance(item_candidate.parentItem(), BaseResizableMapItem):
                parent_map_item = item_candidate.parentItem() # type: BaseResizableMapItem
                if parent_map_item.isSelected() and item_candidate in parent_map_item.resize_handles:
                    try:
                        handle_index = parent_map_item.resize_handles.index(item_candidate)
                        ...
                        self._resize_start_item_rect_scene = parent_map_item.sceneBoundingRect() # Rect in scene coords
                        self.map_scene.setProperty("is_resizing_item_internal", True)
                        editor_history.push_undo_state(self.editor_state) 
                        event.accept()
                        return
                    except ValueError: pass # Not a handle we track

        item_under_mouse: Optional[QGraphicsItem] = None
        for itm in items_under_mouse: 
            if isinstance(itm, (StandardMapObjectItem, BaseResizableMapItem)): # Check both types
                item_under_mouse = itm; break
        
        if event.button() == Qt.MouseButton.LeftButton:
            if self._is_resizing_item: return
            ...
            # ... (rest of left click logic for place/color_pick/rubberband)
        ...
        elif event.button() == Qt.MouseButton.RightButton: # MODIFIED for context menu
        ...
        elif event.button() == Qt.MouseButton.MiddleButton: # Unchanged
    ...
    def mouseMoveEvent(self, event: QMouseEvent): # MODIFIED for resize
        scene_pos = self.mapToScene(event.position().toPoint())
        ...
        if self._is_resizing_item and self._resizing_item_ref and self._resize_start_mouse_pos_scene and self._resize_start_item_rect_scene and self._resize_handle_active is not None:
            delta_scene = scene_pos - self._resize_start_mouse_pos_scene
            current_item_rect_scene = QRectF(self._resize_start_item_rect_scene) # Start from original rect in scene coords for this drag stroke
            ...
            # Adjust based on handle (these are changes in scene coordinates)
            if self._resize_handle_active in [HANDLE_TOP_LEFT, HANDLE_TOP_MIDDLE, HANDLE_TOP_RIGHT]:
                new_y_s += delta_scene.y(); new_h_s -= delta_scene.y()
            if self._resize_handle_active in [HANDLE_BOTTOM_LEFT, HANDLE_BOTTOM_MIDDLE, HANDLE_BOTTOM_RIGHT]:
                new_h_s += delta_scene.y()
            if self._resize_handle_active in [HANDLE_TOP_LEFT, HANDLE_MIDDLE_LEFT, HANDLE_BOTTOM_LEFT]:
                new_x_s += delta_scene.x(); new_w_s -= delta_scene.x()
            if self._resize_handle_active in [HANDLE_TOP_RIGHT, HANDLE_MIDDLE_RIGHT, HANDLE_BOTTOM_RIGHT]:
                new_w_s += delta_scene.x()

            min_dim_scene = float(self.editor_state.grid_size) / 2.0 # Min size in scene units
            if new_w_s < min_dim_scene: new_w_s = min_dim_scene
            if new_h_s < min_dim_scene: new_h_s = min_dim_scene
            
            if QApplication.keyboardModifiers() & Qt.KeyboardModifier.ShiftModifier and self._resizing_item_ref.original_aspect_ratio:
                aspect = self._resizing_item_ref.original_aspect_ratio
                # Determine which dimension change drives the aspect ratio adjustment
                # This logic needs to be careful about which corner/edge is being dragged
                if self._resize_handle_active in [HANDLE_TOP_MIDDLE, HANDLE_BOTTOM_MIDDLE]: # Vertical resize
                    new_w_s = new_h_s * aspect
                    new_x_s = self._resize_start_item_rect_scene.x() + (self._resize_start_item_rect_scene.width() - new_w_s) / 2 # Keep centered
                elif self._resize_handle_active in [HANDLE_MIDDLE_LEFT, HANDLE_MIDDLE_RIGHT]: # Horizontal resize
                    new_h_s = new_w_s / aspect
                    new_y_s = self._resize_start_item_rect_scene.y() + (self._resize_start_item_rect_scene.height() - new_h_s) / 2 # Keep centered
                else: # Corner resize
                    # Simplistic: maintain aspect based on width change, adjust height, then reposition
                    # A more robust solution would check X vs Y delta magnitude for corners
                    # Preserve original top-left if dragging from bottom-right, etc.
                    if self._resize_handle_active == HANDLE_BOTTOM_RIGHT:
                        new_h_s = new_w_s / aspect
                    elif self._resize_handle_active == HANDLE_TOP_LEFT:
                        old_br_s = self._resize_start_item_rect_scene.bottomRight()
                        new_h_s = new_w_s / aspect
                        new_x_s = old_br_s.x() - new_w_s
                        new_y_s = old_br_s.y() - new_h_s
                    # (Add logic for other corners if precise anchor point during aspect resize is needed)
                    # For now, a simple proportional resize from top-left is implied for other corners
                    elif self._resize_handle_active == HANDLE_TOP_RIGHT:
                         old_bl_s_y = self._resize_start_item_rect_scene.bottomLeft().y()
                         new_h_s = new_w_s / aspect
                         new_y_s = old_bl_s_y - new_h_s
                    elif self._resize_handle_active == HANDLE_BOTTOM_LEFT:
                         old_tr_s_x = self._resize_start_item_rect_scene.topRight().x()
                         new_h_s = new_w_s / aspect
                         new_x_s = old_tr_s_x - new_w_s


            item_data = self._resizing_item_ref.map_object_data_ref
            item_data["world_x"] = int(new_x_s)
            item_data["world_y"] = int(new_y_s)
            item_data["current_width"] = int(new_w_s)
            item_data["current_height"] = int(new_h_s)
        ...
        if event.buttons() == Qt.MouseButton.LeftButton: # ... (rest of move/place/color logic)
        ...
        elif event.buttons() == Qt.MouseButton.RightButton: # ... (erase logic)
        ...
        elif self.middle_mouse_panning and event.buttons() == Qt.MouseButton.MiddleButton: # ... (pan logic)
    ...
    def mouseReleaseEvent(self, event: QMouseEvent): # MODIFIED for resize
        if self._is_resizing_item:
            self._is_resizing_item = False
            self.map_scene.setProperty("is_resizing_item_internal", False)
            QApplication.restoreOverrideCursor()
            if self._resizing_item_ref:
                 # Data should already be updated during mouseMove, now make it permanent for undo stack
                 # The initial undo was pushed on press, this finalizes the state for next undo.
                 # No, the history is pushed on press. This release just finalizes the UI state.
                 self.handle_map_content_changed() # To ensure properties panel might update
                 self.show_status_message("Object resized.")
            self._resizing_item_ref = None; self._resize_handle_active = None
            self._resize_start_item_rect_scene = None; self._resize_start_mouse_pos_scene = None
            event.accept(); return

        # ... (rest of mouse release, e.g., drag map object, middle mouse)
        if event.button() == Qt.MouseButton.LeftButton and self._is_dragging_map_object:
            self._is_dragging_map_object = False 
            item_dragged: Optional[QGraphicsItem] = None # type: ignore
    ...
    def contextMenuEvent(self, event: QContextMenuEvent): # MODIFIED for item types
        # Find item under mouse, check if it's BaseResizableMapItem for layering
        # items = self.items(event.pos()) # items in viewport coordinates
        scene_pos = self.mapToScene(event.pos())
        item_under_mouse = self.itemAt(scene_pos.toPoint()) # Item at scene coordinates

        if isinstance(item_under_mouse, BaseResizableMapItem):
            data_ref = getattr(item_under_mouse, 'map_object_data_ref', None)
            if data_ref: # Check if this item type should have a context menu
                ...
                if asset_key == ED_CONFIG.CUSTOM_IMAGE_ASSET_KEY or asset_key == ED_CONFIG.TRIGGER_SQUARE_ASSET_KEY: # type: ignore
                    ...
                    event.accept()
                    return
        elif isinstance(item_under_mouse, StandardMapObjectItem): # If standard items also get context menus
            pass # Add logic if needed

        if not event.isAccepted():
            super().contextMenuEvent(event)

    # enterEvent, leaveEvent, _check_edge_scroll, perform_edge_scroll (remain same)
    def enterEvent(self, event: QFocusEvent):  # type: ignore
        if not self.edge_scroll_timer.isActive(): self.edge_scroll_timer.start()
        super().enterEvent(event)
    def leaveEvent(self, event: QFocusEvent):  # type: ignore
    ...
    def _check_edge_scroll(self, mouse_pos_viewport: QPointF): # Unchanged
    ...
    @Slot()
    def perform_edge_scroll(self): # Unchanged
    ...
    def _perform_place_action(self, grid_x: int, grid_y: int, continuous: bool = False, is_first_action: bool = False):
        # ... (Logic from previous version, ensure it correctly calls _place_single_object_on_map) ...
        # This method now mostly determines *which* asset to place based on palette selection
        ...
        if is_custom_asset_from_palette:
            ...
            asset_to_place_key_for_data = ED_CONFIG.CUSTOM_IMAGE_ASSET_KEY # Generic key for the object data
            asset_definition_for_placement = { # This is mostly for the _place_single function's benefit
                "game_type_id": ED_CONFIG.CUSTOM_IMAGE_ASSET_KEY,
                "asset_editor_key": ED_CONFIG.CUSTOM_IMAGE_ASSET_KEY, # Matching data key
                "source_file_path": f"Custom/{filename}", 
                "colorable": False,
                "category": "custom" 
            }
        else: # Standard asset or tool from palette
    ...
    def _place_single_object_on_map(self, asset_key_for_data: str, asset_definition: Dict, grid_x: int, grid_y: int) -> bool:
        # ... (Logic from previous, now handles CUSTOM_IMAGE_ASSET_KEY and TRIGGER_SQUARE_ASSET_KEY correctly for dimensions and properties) ...
        ...
        # Prevent duplicate non-spawn, non-custom, non-trigger on same tile
        ...
        new_obj_data: Dict[str, Any] = {
            ...
            "layer_order": 0, # Default layer order
            "properties": ED_CONFIG.get_default_properties_for_asset(game_id) # type: ignore
        }

        if is_custom_image_type:
            new_obj_data["source_file_path"] = asset_definition.get("source_file_path", "") # e.g. Custom/img.png
            ...
            else: # Fallback if no image path
                new_obj_data["original_width"]=new_obj_data["current_width"]=ED_CONFIG.BASE_GRID_SIZE*2
                new_obj_data["original_height"]=new_obj_data["current_height"]=ED_CONFIG.BASE_GRID_SIZE*2

        elif is_trigger_type:
            new_obj_data["current_width"] = ED_CONFIG.BASE_GRID_SIZE * 2 # Default trigger size
        ...
        if is_spawn_type: # Remove existing spawns of same type
            self.editor_state.placed_objects = [obj for obj in self.editor_state.placed_objects if obj.get("game_type_id") != game_id]
            # Corresponding QGraphicsItems will be removed by draw_placed_objects diffing

        self.editor_state.placed_objects.append(new_obj_data)
        self.draw_placed_objects() # This will create/update the visual item
        return True


    def _perform_erase_action(self, grid_x: int, grid_y: int, continuous: bool = False, is_first_action: bool = False):
        # ... (Logic from previous, ensure it correctly finds and removes items of any type) ...
        ...
        for obj_data in self.editor_state.placed_objects: # Find topmost item
            ...
            if obj_rect.contains(target_point_scene):
                ...
                if current_obj_z >= highest_z: # Prioritize higher Z, then last in list if Z is same
        ...
        if item_to_remove_data:
            ...
            if item_id in self._map_object_items:
                ...
                if isinstance(map_obj_item, BaseResizableMapItem): # Remove handles too
                    map_obj_item.show_resize_handles(False)
                    for handle in map_obj_item.resize_handles:
                        if handle.scene(): self.map_scene.removeItem(handle)
                    map_obj_item.resize_handles.clear()
    ...
    def _perform_color_tile_action(self, grid_x: int, grid_y: int, continuous: bool = False, is_first_action: bool = False):
        # ... (Logic from previous, ensure it only targets colorable standard assets) ...
        ...
        for obj_data in self.editor_state.placed_objects:
            ...
            obj_x = obj_data.get("world_x", 0); obj_y = obj_data.get("world_y", 0)
            obj_w, obj_h = asset_info.get("original_size_pixels", (self.editor_state.grid_size, self.editor_state.grid_size))
            obj_rect = QRectF(float(obj_x), float(obj_y), float(obj_w), float(obj_h))
            if obj_rect.contains(target_point_scene):
                current_obj_z = obj_data.get("layer_order", 0)
    ...
    def delete_selected_map_objects(self): # MODIFIED to handle BaseResizableMapItem handles
        ...
        for item_map_obj in items_to_process:
            item_id = id(item_map_obj.map_object_data_ref) # type: ignore
            if item_id in self._map_object_items:
                del self._map_object_items[item_id] 
            
            if isinstance(item_map_obj, BaseResizableMapItem):
                item_map_obj.show_resize_handles(False)
                for handle in item_map_obj.resize_handles:
                    if handle.scene(): self.map_scene.removeItem(handle)
                item_map_obj.resize_handles.clear()
    ...
    @Slot(str)
    def on_asset_selected(self, asset_editor_key: Optional[str]): # Unchanged
    ...
    def _update_hover_preview(self, world_x: float, world_y: float): # Unchanged
        if self.current_tool == "place" and self.editor_state.selected_asset_editor_key:
            ...
            if asset_key.startswith(ED_CONFIG.CUSTOM_ASSET_PALETTE_PREFIX): # type: ignore
                ...
                elif asset_data: original_w, original_h = asset_data.get("original_size_pixels", (ED_CONFIG.BASE_GRID_SIZE, ED_CONFIG.BASE_GRID_SIZE)); pixmap = get_asset_pixmap(asset_key, asset_data, QSize(original_w,original_h), None, get_native_size_only=True)
    ...
    @Slot(str)
    def on_tool_selected(self, tool_key: str): # Unchanged
        ...
        if tool_key == "tool_eraser": self.current_tool = "erase"; self._controller_tool_mode = "erase"; self.editor_state.current_tile_paint_color = None 
        elif tool_key == "tool_tile_color_picker": # ... (color picker logic)
            ...
            if self.parent_window: 
                ...
                new_q_color = QColorDialog.getColor(QColor(*initial_c), self.parent_window, "Select Tile Paint Color")
    ...
    @Slot(dict)
    def on_object_properties_changed(self, changed_object_data_ref: Dict[str, Any]): # Unchanged
        self.update_specific_object_visuals(changed_object_data_ref)
        # self.map_content_changed.emit() is usually handled by main window after this

    @Slot()
    def on_scene_selection_changed(self): # MODIFIED for item types and handles
        selected_qt_items = self.map_scene.selectedItems()
        
        # Hide handles for all items first
        for item_id, map_obj_item_generic in self._map_object_items.items():
            if isinstance(map_obj_item_generic, BaseResizableMapItem):
                map_obj_item_generic.show_resize_handles(False)
        ...
        if len(selected_qt_items) == 1:
            ...
            if isinstance(selected_item_generic, (StandardMapObjectItem, BaseResizableMapItem)):
                ...
                if data_ref:
                    ...
                    if isinstance(selected_item_generic, BaseResizableMapItem):
                        selected_item_generic.show_resize_handles(True) # Show for current selection
                else: self.map_object_selected_for_properties.emit(None)
            else: self.map_object_selected_for_properties.emit(None)
        else: 
            self.map_object_selected_for_properties.emit(None) 
            
    def show_status_message(self, message: str, timeout: int = 2000): # Unchanged
