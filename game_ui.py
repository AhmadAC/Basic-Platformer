...
class GameSceneWidget(QWidget):
    ...
    def resizeEvent(self, event: QResizeEvent):
        ...
        log_info(f"GameSceneWidget resizeEvent to: {new_width}x{new_height}") # This matches the log

        camera = self.get_camera()
        if camera:
            log_debug(f"GameSceneWidget: Updating camera screen dimensions to {new_width}x{new_height}")
            camera.set_screen_dimensions(new_width, new_height) # This internally calls static_update

            # ADDED: After screen dimensions are set, explicitly re-update camera focus
            # This ensures the camera centers on the player with the NEW screen size.
            player1 = self.game_elements.get("player1")
            player2 = self.game_elements.get("player2")

            focus_target = None
            # Determine the primary focus target for the camera
            if player1 and hasattr(player1, 'alive') and player1.alive() and \
               hasattr(player1, '_valid_init') and player1._valid_init and \
               not getattr(player1, 'is_dead', True) and not getattr(player1, 'is_petrified', False):
                 focus_target = player1
            elif player2 and hasattr(player2, 'alive') and player2.alive() and \
                 hasattr(player2, '_valid_init') and player2._valid_init and \
                 not getattr(player2, 'is_dead', True) and not getattr(player2, 'is_petrified', False):
                 focus_target = player2
            # Fallback if primary targets are not ideal (e.g., one is dead, petrified)
            elif player1 and hasattr(player1, 'alive') and player1.alive() and \
                 hasattr(player1, '_valid_init') and player1._valid_init:
                 focus_target = player1
            elif player2 and hasattr(player2, 'alive') and player2.alive() and \
                 hasattr(player2, '_valid_init') and player2._valid_init:
                 focus_target = player2
            ...
            if focus_target:
                ...
                # static_update was already called inside set_screen_dimensions, so camera is clamped.
                # If no target, static_update is the correct behavior.
