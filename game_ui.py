...
"""
# version 2.0.14 (Further refined custom image drawing log)
...
def draw_player_hud_qt(painter: QPainter, x: float, y: float, player_instance: Player,
    ...
    draw_health_bar_qt(painter, health_bar_pos_x, health_bar_pos_y, hud_health_bar_width, hud_health_bar_height, float(player_instance.current_health), float(player_instance.max_health))
...
class GameSceneWidget(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        for i, entity in enumerate(all_renderables):
            ...
            elif isinstance(entity, dict) and 'rect' in entity and 'image' in entity: # Likely a custom image from processed_custom_images_for_render
        ...
        for p_instance_hud in active_players_for_hud:
             if p_instance_hud and p_instance_hud.alive() and not getattr(p_instance_hud, 'is_petrified', False):
                ...
                hud_width_estimate = float(getattr(C, 'HUD_HEALTH_BAR_WIDTH', 100.0)) + 100.0
    ...
    def populate_maps(self):
        ...
        if os.path.exists(maps_dir_abs) and os.path.isdir(maps_dir_abs):
            try:
                ...
                prio_maps = ["original", "lava", "cpu_extended", "noenemy", "bigmap1", "one", "8", "back", "big", "chest", "custom", "three"] # Added new maps to priority
                final_ordered_maps = [m for m in prio_maps if m in available_map_folder_names] + \
                                     [m for m in sorted(available_map_folder_names) if m not in prio_maps] # Sort remaining non-priority
