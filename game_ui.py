...
# ... (get_clipboard_text_qt, set_clipboard_text_qt, draw_health_bar_qt, draw_player_hud_qt remain the same) ...
def draw_health_bar_qt(painter: QPainter, x: float, y: float,
    ...
    pen = QPen(qcolor_black); pen.setWidth(1); painter.setPen(pen); painter.drawRect(background_rect)
...
def draw_player_hud_qt(painter: QPainter, x: float, y: float, player_instance: Any,
    ...
    health_bar_pos_x = x; health_bar_pos_y = y + label_height_offset + 5.0
    ...
    draw_health_bar_qt(painter, health_bar_pos_x, health_bar_pos_y, hud_health_bar_width, hud_health_bar_height, player_instance.current_health, player_instance.max_health)
    ...
    def paintEvent(self, event: Any): # event type is QPaintEvent
        ...
        if camera_instance:
            for i, entity in enumerate(all_renderables):
                ...
                entity_id_for_log = id(entity)
                entity_type_for_log = type(entity).__name__
                platform_type_attr = getattr(entity, 'platform_type', 'N/A') if is_static_tile else 'N/A'

                if GameSceneWidget.render_print_limiter.can_print(f"entity_check_{entity_id_for_log}"):
                    logger.debug(f"  Paint Check Entity {i} ({entity_type_for_log}, PType: {platform_type_attr}): "
                                 f"CanRender={can_render_entity}, Alive={is_alive_entity}, "
                ...
                if can_render_entity and entity_rect is not None: # MyPy hint
                    ...
                    if GameSceneWidget.render_print_limiter.can_print(f"entity_render_details_{entity_id_for_log}"):
                        logger.debug(f"    Entity {i} ({entity_type_for_log}, PType: {platform_type_attr}): "
        ...
        if self.download_status_message:
            ...
            painter.fillRect(dialog_rect, QColor(50,50,50, 200)); painter.setPen(QColor(*C.WHITE)); painter.drawRect(dialog_rect)
            title_font = self.fonts.get("large_qfont", QFont("Arial", 24, QFont.Weight.Bold)); msg_font = self.fonts.get("medium_qfont", QFont("Arial", 12))
            painter.setFont(title_font); painter.drawText(dialog_rect.adjusted(10,10,-10,-10), Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, "File Transfer")
            painter.setFont(msg_font); painter.drawText(dialog_rect.adjusted(10, QFontMetrics(title_font).height() + 20, -10, -10), Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, self.download_status_message)
            if self.download_progress_percent is not None and self.download_progress_percent >= 0:
                bar_margin = 20.0; bar_h = 30.0
                msg_text_rect = QFontMetrics(msg_font).boundingRect(dialog_rect.adjusted(10, QFontMetrics(title_font).height() + 20, -10, -10), Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, self.download_status_message)
                ...
                painter.fillRect(bar_rect, QColor(*C.GRAY)); fill_width = bar_rect.width() * (self.download_progress_percent / 100.0)
                painter.fillRect(QRectF(bar_rect.topLeft(), QSizeF(fill_width, bar_rect.height())), QColor(*C.GREEN))
                painter.setPen(QColor(*C.WHITE)); painter.drawRect(bar_rect); prog_text = f"{self.download_progress_percent:.1f}%"
                painter.setPen(QColor(*C.BLACK)); painter.drawText(bar_rect, Qt.AlignmentFlag.AlignCenter, prog_text)
        painter.end()

# ... (SelectMapDialog and IPInputDialog remain the same) ...
class SelectMapDialog(QDialog):
    def __init__(self, fonts: Dict[str, QFont], parent: Optional[QWidget] = None):
        ...
        layout = QVBoxLayout(self); self.list_widget = QListWidget(self); self.populate_maps(); layout.addWidget(self.list_widget)
    ...
    def populate_maps(self):
        ...
        if os.path.exists(maps_dir) and os.path.isdir(maps_dir):
            try:
                ...
                else: self.list_widget.addItem("No selectable maps found."); self.button_box.button(QDialogButtonBox.StandardButton.Ok).setEnabled(False)
        ...
        else: self.list_widget.addItem(f"Maps directory not found: {maps_dir}"); self.button_box.button(QDialogButtonBox.StandardButton.Ok).setEnabled(False)
    def accept(self):
        ...
        if current_item and current_item.text() and not current_item.text().startswith("No") and not current_item.text().startswith("Error"):
            self.selected_map_name = current_item.text(); super().accept()
        elif current_item: QMessageBox.information(self, "No Map", "No valid map selected.")
...
class IPInputDialog(QDialog):
    def __init__(self, default_ip_port: str = "127.0.0.1:5555", parent: Optional[QWidget] = None):
        ...
        layout = QVBoxLayout(self); self.label = QLabel(f"Format: IP_ADDRESS:PORT (e.g., 192.168.1.100:5555)\nCtrl+V to paste may work.")
        self.label.setWordWrap(True); layout.addWidget(self.label); self.line_edit = QLineEdit(self)
        self.line_edit.setText(default_ip_port); layout.addWidget(self.line_edit)
        self.button_box = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
