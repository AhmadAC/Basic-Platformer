...
# PySide6 imports (if needed for specific Qt types, though mostly works with primitives)
from PySide6.QtCore import Qt # For Qt.Key enum comparison if used directly

import constants as C
import config as game_config
# joystick_handler is used by main.py to get joystick info for config,
# but this module now receives pre-processed joystick_data.
from utils import PrintLimiter

input_print_limiter = PrintLimiter(default_limit=20, default_period=5.0)

# --- Monotonic Timer (if needed for timing within this handler) ---
_input_handler_start_time = time.monotonic() # time needs to be imported if not already
def get_input_handler_ticks(): # In milliseconds
    return int((time.monotonic() - _input_handler_start_time) * 1000)


def process_player_input_logic_pyside(
    player: Any,  # Player instance
    qt_keys_held_snapshot: Dict[Qt.Key, bool], # Snapshot of currently HELD Qt keys
    qt_key_events_this_frame: List[Any],    # List of QKeyEvent objects for DISCRETE presses this frame
    active_mappings: Dict[str, Any],        # Player's control mappings (Qt.Key or joystick dicts)
    platforms_list: List[Any],              # For can_stand_up check
    joystick_data: Optional[Dict[str, Any]] = None # Pre-processed joystick state
) -> Dict[str, bool]:
    ...
    # --- Part 1: Process universal events (reset, pause) from DISCRETE key events ---
    for qt_event in qt_key_events_this_frame:
        if qt_event.type() == QKeyEvent.Type.KeyPress: # Only process key presses for events
            key_code = Qt.Key(qt_event.key()) # Ensure it's a Qt.Key enum
            
            # Check if keyboard mapping matches for reset or pause
    ...
    # Joystick universal events (reset, pause) - check current button state against previous
        ...
        for action_name_check in ["reset", "pause"]:
            ...
            if isinstance(mapping, dict):
                if mapping.get("type") == "button":
                    ...
                    if current_buttons.get(btn_id, False) and not prev_buttons.get(btn_id, False): # Just pressed
                        action_events[action_name_check] = True
                elif mapping.get("type") == "hat": # Hat events are tricky, usually for menus
                    hat_id = mapping.get("id")
                    hat_val_target = tuple(mapping.get("value", (0,0))) # Ensure tuple
                    current_hat_val = tuple(joystick_data.get('hats', {}).get(hat_id, (0,0)))
                    # Simple check: if current hat matches target and wasn't matching previously (more complex for "just pressed" hat)
                    # For now, let's assume hat actions are okay to be triggered if held, or main.py filters them.
                    # This part needs more robust "just pressed" logic for hats if critical.
                    if current_hat_val == hat_val_target and hat_val_target != (0,0):
                         action_events[action_name_check] = True # Could make this a state if needed
                # Axis for reset/pause is less common, but can be handled similarly to buttons if threshold just crossed
    ...
    if not is_joystick_input_type: # Keyboard
        for action_name in ["left", "right", "up", "down"]:
            ...
            if isinstance(key_code, Qt.Key) and qt_keys_held_snapshot.get(key_code, False): # Check if it's Qt.Key
        ...
        for action_name in ["left", "right", "up", "down"]:
            ...
            if isinstance(mapping_details, dict):
                ...
                if m_type == "axis" and m_id is not None:
                    ...
                    hat_val_target = tuple(m_value_prop) # Ensure tuple
                    if tuple(current_hats.get(m_id, (0,0))) == hat_val_target and hat_val_target != (0,0):
                        action_state[action_name] = True
                elif m_type == "button" and m_id is not None: # For held movement buttons
    ...
    # Aiming logic (remains largely the same conceptually)
    aim_x, aim_y = 0.0, 0.0
    ...
    # --- Part 3: Populate remaining action_events from DISCRETE key events for game actions ---
    for qt_event in qt_key_events_this_frame:
        if qt_event.type() == QKeyEvent.Type.KeyPress:
            key_code = Qt.Key(qt_event.key())
            # Skip if it's a universal key already processed for event
            if not (active_mappings.get("reset") == key_code or active_mappings.get("pause") == key_code):
                for action_name, mapped_val in active_mappings.items():
                    if isinstance(mapped_val, Qt.Key) and key_code == mapped_val: # Check if mapped_val is Qt.Key
    ...
    # Joystick game action events (from "just pressed" logic)
    if is_joystick_input_type and joystick_data:
        current_buttons = joystick_data.get('buttons_current', {})
        prev_buttons = joystick_data.get('buttons_prev', {})
        current_hats = joystick_data.get('hats',{})
        # prev_hats = joystick_data.get('hats_prev',{}) # Need previous hat state for "just pressed" hat
        current_axes = joystick_data.get('axes',{})
        # prev_axes = joystick_data.get('axes_prev',{}) # Need previous axis state for "just pressed" axis

        for action_name, map_details in active_mappings.items():
            if action_name in ["reset", "pause"]: continue # Already handled
            if isinstance(map_details, dict):
                ...
                if m_type == "button":
                    ...
                    # This requires comparing current_hats with prev_hats for "just pressed"
                    # Simplified: trigger if hat is currently in target position (can lead to multiple triggers if held)
                    # For true "just pressed" hat, main.py input loop needs to store previous hat state.
                    ...
                    # "Just pressed" for axis (crossed threshold)
                    axis_val = current_axes.get(m_id, 0.0)
                    ...
                    # prev_axis_val = prev_axes.get(m_id, 0.0) # Need prev_axes from joystick_data
                    # passed_thresh_prev = (isinstance(m_value_prop, (int,float)) and 
                    #                       ((m_value_prop < 0 and prev_axis_val < -m_threshold) or 
                    #                        (m_value_prop > 0 and prev_axis_val > m_threshold)))
                    
                    # if passed_thresh_now and not passed_thresh_prev: # Just crossed threshold
                    # Simplified: if axis is beyond threshold, treat as event (can lead to multiple triggers)
    ...
    # (This part remains conceptually similar, but uses the new action_events and player state flags)
    ...
    # JUMP LOGIC
    joystick_up_for_jump = False # This needs to be derived from joystick_data if that's implemented fully
    if joystick_data:
        current_buttons = joystick_data.get('buttons_current',{})
        prev_buttons = joystick_data.get('buttons_prev',{})
        ...
        if isinstance(jump_mapping, dict) and jump_mapping.get("type") == "button":
            ...
            if current_buttons.get(btn_id,False) and not prev_buttons.get(btn_id,False):
                joystick_up_for_jump = True # "Jump" button just pressed
    ...
    # Auto-state updates (conceptually similar, relies on player flags set above)
