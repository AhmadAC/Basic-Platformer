...
# -*- coding: utf-8 -*-
"""
version 1.0.1.7 (Allow input processing for movement if aflame/deflame)
...
def process_player_input_logic(player, keys_pressed_from_pygame, pygame_events,
    ...
    # MODIFIED: Adjust input blocking for aflame/deflame
    # Game actions are blocked if player is dead, in hit stun, petrified, or frozen.
    # Aflame/deflame will allow movement input but might block other actions like attacks.
    is_fully_input_blocked_for_game_actions = player.is_dead or \
                                           ...
                                           (getattr(player, 'is_defrosting', False) and player.state == 'defrost') # Defrosting is also immobilizing for actions

    # Specific check for actions like attacks/dashes when on fire (movement is handled separately)
    are_attacks_blocked_by_fire = (getattr(player, 'is_aflame', False) and player.state == 'aflame') or \
                                  (getattr(player, 'is_deflaming', False) and player.state == 'deflame')
    ...
    # This part is NOT blocked by is_fully_input_blocked_for_game_actions, allowing movement intention even if other actions are blocked.
    if not is_joystick_input_type:
    ...
    # Update player intention states (for movement and aiming)
    ...
    elif action_state["down"] or player.is_crouching: # Aim down if holding down or already crouching
        aim_y = 1.0
    if aim_x != 0.0 or aim_y != 0.0:
        player.fireball_last_input_dir.x, player.fireball_last_input_dir.y = aim_x, aim_y
    elif player.fireball_last_input_dir.length_squared() == 0: # Default aim if no input
        player.fireball_last_input_dir.x = 1.0 if player.facing_right else -1.0
        player.fireball_last_input_dir.y = 0.0


    # --- Early exit if game actions are blocked ---
    if is_fully_input_blocked_for_game_actions:
        player.acc.x = 0 # Stop horizontal acceleration if fully blocked
        # Movement intentions (is_trying_to_move_left/right) are already set above and might be used by physics if only partially blocked (e.g., aflame)
    ...
    for event in pygame_events:
        is_reset_or_pause_event_key = False # Already handled above, skip here
    ...
    # Note: is_trying_to_move_left/right are already set above.

    player.acc.x = 0 # Reset horizontal acceleration, it will be set if movement input is active
    ...
    can_control_horizontal = not (player.is_dashing or player.is_rolling or player.is_sliding or player.on_ladder or
                                 (player.is_attacking and player.state.endswith('_nm')) or # No movement for no-movement attacks
                                 player.state in ['turn','hit','death','death_nm','wall_climb','wall_climb_nm','wall_hang',
                                                  'frozen', 'defrost']) # MODIFIED: Removed aflame/deflame
    ...
    if can_control_horizontal:
        if player.is_trying_to_move_left and not player.is_trying_to_move_right:
            ...
            if player.facing_right and player.on_ground and not player.is_crouching and not player.is_attacking and player.state in ['idle','run', 'aflame', 'deflame']: player.set_state('turn') # MODIFIED
            player.facing_right = False
        elif player.is_trying_to_move_right and not player.is_trying_to_move_left:
            player.acc.x = C.PLAYER_ACCEL
            if not player.facing_right and player.on_ground and not player.is_crouching and not player.is_attacking and player.state in ['idle','run', 'aflame', 'deflame']: player.set_state('turn') # MODIFIED
            player.facing_right = True

    if player.on_ladder: # Ladder movement overrides normal horizontal accel
         player.vel.y = 0
    ...
    if action_events.get("crouch"): # Crouch toggle logic
        if player.is_crouching:
            ...
            can_slide_now = player.on_ground and player.state in ['run', 'aflame', 'deflame'] and \
                                 ...
                                 not player.on_ladder and player.state not in ['turn','hit', 'frozen', 'defrost'] # MODIFIED
            if can_slide_now:
                player.set_state('slide_trans_start' if player.animations.get('slide_trans_start') else 'slide')
            else: # Try to just crouch
                can_crouch_now = player.on_ground and not player.on_ladder and not player.is_sliding and \
                                   not (player.is_dashing or player.is_rolling or player.is_attacking or \
                                        player.state in ['turn','hit','death','death_nm', 'frozen', 'defrost']) # MODIFIED
                if can_crouch_now:
                    player.is_crouching = True
                    player.set_state('crouch_trans' if player.animations.get('crouch_trans') else 'crouch')

    # Update visual state if crouching and moving
    ...
    if action_events.get("attack1") and not are_attacks_blocked_by_fire: # MODIFIED
        can_attack_melee = not player.is_attacking and not player.is_dashing and not player.is_rolling and \
                           ...
                           player.state not in ['turn','hit', 'frozen', 'defrost'] # MODIFIED
        if can_attack_melee:
            player.attack_type = 4 if player.is_crouching else 1 # Crouch attack or normal attack1
    ...
    if action_events.get("attack2") and not are_attacks_blocked_by_fire: # MODIFIED
        ...
        if can_attack_melee2:
            ...
            if player.is_crouching and not player.is_attacking : # Allow if crouched and not already attacking
                player.attack_type = 4; anim_key_attack2 = 'crouch_attack' # Re-use crouch attack for attack2 if crouched
            elif not player.is_attacking: # Not crouched, not attacking
                player.attack_type = 2; anim_key_attack2 = 'attack2' if is_moving_for_attack2 else 'attack2_nm'
            else: anim_key_attack2 = None # Already attacking, don't change state
            if anim_key_attack2: player.set_state(anim_key_attack2)

    if action_events.get("dash") and not are_attacks_blocked_by_fire: # MODIFIED
    ...
    if action_events.get("roll") and not are_attacks_blocked_by_fire: # MODIFIED
    ...
    if action_events.get("interact"): # Interact is usually not blocked by fire
    ...
    # Projectiles can be fired even if on fire, as per player._generic_fire_projectile check
    ...
    # Auto-state updates based on physics (if not in an overriding action state)
    is_in_override_state_for_movement = player.is_attacking or player.is_dashing or player.is_rolling or player.is_sliding or \
                                        player.is_taking_hit or player.state in [
                                            ...
                                            'frozen', 'defrost'] # MODIFIED: Removed aflame, deflame
                                            # Petrified/smashed handled by is_fully_input_blocked
        ...
        if player.on_ladder:
            if abs(player.vel.y) > 0.1 and player.state != 'ladder_climb': player.set_state('ladder_climb')
             ...
             if player.is_crouching:
                 ...
                 if player.state not in ['run', 'aflame', 'deflame']: player.set_state('run' if not (player.is_aflame or player.is_deflaming) else player.state) # MODIFIED
             else: # Not crouching, on ground, not trying to move
                 if player.state not in ['idle', 'aflame', 'deflame']: player.set_state('idle' if not (player.is_aflame or player.is_deflaming) else player.state) # MODIFIED
             ...
             if player.touching_wall != 0 and not player.is_dashing and not player.is_rolling:
                 ...
                 if player.vel.y > C.PLAYER_WALL_SLIDE_SPEED * 0.5 or climb_expired: # Sliding down
                 ...
                 else: # Hanging or just touching
                     if player.state != 'wall_slide': player.set_state('wall_slide'); player.can_wall_jump = True
             elif player.vel.y > getattr(C, 'MIN_SIGNIFICANT_FALL_VEL', 1.0) and player.state not in ['jump','jump_fall_trans']:
                  if player.state not in ['fall', 'aflame', 'deflame']: player.set_state('fall' if not (player.is_aflame or player.is_deflaming) else player.state) # MODIFIED
             elif player.state not in ['jump','jump_fall_trans','fall', 'aflame', 'deflame']: # Not significantly falling, not jumping, not on fire
