...
from projectiles import Fireball # <<< IMPORT FIREBALL

# --- Print Limiter Utility ---
# ... (PrintLimiter class remains the same) ...
class PrintLimiter:
    ...
    def can_print(self, message_key, limit=None, period=None):
        ...
        if self.counts[message_key] < limit:
            ...
            # print(f"[PrintLimiter] Suppressing further prints for '{message_key}' for {period:.1f}s (limit: {limit})") # Keep commented
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, start_x, start_y, player_id=1):
        ...
        self.projectile_sprites_group = None # Set by main.py
        self.all_sprites_group = None      # Set by main.py

        self.fireball_key = None
        if self.player_id == 1:
            self.fireball_key = pygame.K_1
        elif self.player_id == 2:
            self.fireball_key = pygame.K_0 


    def set_projectile_group_references(self, projectile_group, all_sprites_group):
        self.projectile_sprites_group = projectile_group
        self.all_sprites_group = all_sprites_group

    # ... (set_state and animate methods remain the same as your previous version) ...
    def set_state(self, new_state):
        ...
        if can_change_state:
            if Player.print_limiter.can_print(f"player_{self.player_id}_set_state", limit=50, period=2.0): # Increased limit for debugging
    ...
    def animate(self):
        ...
        if self.is_dead: 
            state_key = 'death_nm' if abs(self.vel.x) < 0.5 and abs(self.vel.y) < 1.0 and 'death_nm' in self.animations and self.animations['death_nm'] else 'death'
            if state_key not in self.animations or not self.animations[state_key]: state_key = 'death'
        elif self.is_attacking:
            if self.attack_type == 1: state_key = 'attack' if moving_intended_by_input and 'attack' in self.animations and self.animations['attack'] else 'attack_nm'
            elif self.attack_type == 2: state_key = 'attack2' if moving_intended_by_input and 'attack2' in self.animations and self.animations['attack2'] else 'attack2_nm'
            elif self.attack_type == 3: state_key = 'attack_combo' if moving_intended_by_input and 'attack_combo' in self.animations and self.animations['attack_combo'] else 'attack_combo_nm'
            ...
            if state_key not in self.animations or not self.animations[state_key]:
                 ...
                 if base_state in self.animations and self.animations[base_state]:
                     state_key = base_state
                 else: 
                     state_key = 'idle'
             ...
             state_key = 'wall_climb' if is_actively_climbing and 'wall_climb' in self.animations and self.animations['wall_climb'] else 'wall_climb_nm'
            ...
            if self.is_crouching: state_key = 'crouch_walk' if moving_intended_by_input and 'crouch_walk' in self.animations and self.animations['crouch_walk'] else 'crouch'
            ...
            if now - self.last_anim_update > current_anim_frame_duration:
                ...
                if self.current_frame >= len(animation):
                    ...
                    if self.state in non_looping_states:
                         ...
                         if current_s == 'jump': next_s = 'jump_fall_trans' if 'jump_fall_trans' in self.animations and self.animations['jump_fall_trans'] else 'fall'
    ...
    def _process_input_logic(self, keys, events, key_config):
        ...
        current_fireball_intent_x = 0.0
        current_fireball_intent_y = 0.0
        if keys[key_config['left']]: current_fireball_intent_x = -1.0
        elif keys[key_config['right']]: current_fireball_intent_x = 1.0
        if keys[key_config['up']]: current_fireball_intent_y = -1.0
        elif keys[key_config['down']]: current_fireball_intent_y = 1.0
        
        if current_fireball_intent_x != 0.0 or current_fireball_intent_y != 0.0:
            self.fireball_last_input_dir.x = current_fireball_intent_x
            self.fireball_last_input_dir.y = current_fireball_intent_y
            ...
            if event.type == pygame.KEYDOWN:
                 ...
                 if event.key == key_config['attack1']:
                      ...
                      if can_attack_action:
                           ...
                           attack_state_key = 'attack' if is_moving_for_attack and 'attack' in self.animations and self.animations['attack'] else 'attack_nm'
                           if self.is_crouching: self.attack_type = 4; attack_state_key = 'crouch_attack'
                           self.set_state(attack_state_key)
                 ...
                 if event.key == key_config['attack2']:
                      ...
                      if can_attack_action:
                           ...
                           attack_state_key = ''
                           if is_in_combo_window and 'attack_combo' in self.animations and self.animations['attack_combo']:
                               self.attack_type = 3 
                               attack_state_key = 'attack_combo' if is_moving_for_attack and 'attack_combo' in self.animations and self.animations['attack_combo'] else 'attack_combo_nm'
                           elif self.is_crouching and 'crouch_attack' in self.animations and self.animations['crouch_attack']:
                               self.attack_type = 4 
                               attack_state_key = 'crouch_attack'
                           elif 'attack2' in self.animations and self.animations['attack2']:
                               self.attack_type = 2 
                               attack_state_key = 'attack2' if is_moving_for_attack and 'attack2' in self.animations and self.animations['attack2'] else 'attack2_nm'
                           elif self.attack_type == 0 and 'attack' in self.animations and self.animations['attack']: 
                               self.attack_type = 1; attack_state_key = 'attack' if is_moving_for_attack else 'attack_nm'
                           
                           if attack_state_key : self.set_state(attack_state_key)
                 ...
                 if event.key == key_config['down']:
                      ...
                      if can_slide_action:
                           ...
                           if start_slide_state in self.animations and self.animations[start_slide_state]: self.set_state(start_slide_state)
                 ...
                 if self.fireball_key and event.key == self.fireball_key:
                     self.fire_fireball()
        ...
        if not is_in_manual_override_state: 
            if self.on_ladder:
                if abs(self.vel.y) > 0.1 : self.set_state('ladder_climb' if 'ladder_climb' in self.animations and self.animations['ladder_climb'] else 'idle')
                else: self.set_state('ladder_idle' if 'ladder_idle' in self.animations and self.animations['ladder_idle'] else 'idle')
            elif self.on_ground:
                 if self.is_crouching:
                     target_crouch_state = 'crouch_walk' if is_trying_to_move_lr_thistick and 'crouch_walk' in self.animations and self.animations['crouch_walk'] else 'crouch'
                     self.set_state(target_crouch_state if target_crouch_state in self.animations and self.animations[target_crouch_state] else 'idle')
                 elif is_trying_to_move_lr_thistick: self.set_state('run' if 'run' in self.animations and self.animations['run'] else 'idle')
                 ...
                 if self.touching_wall != 0:
                     ...
                     if self.vel.y > C.PLAYER_WALL_SLIDE_SPEED * 0.5 or wall_climb_expired:
                     ...
                     elif self.is_holding_climb_ability_key and abs(self.vel.x) < 1.0 and not wall_climb_expired and 'wall_climb' in self.animations and self.animations['wall_climb']:
                      ...
                      self.set_state('fall' if 'fall' in self.animations and self.animations['fall'] else 'idle')
    ...
    def fire_fireball(self):
        ...
        if self.projectile_sprites_group is None or self.all_sprites_group is None:
            if Player.print_limiter.can_print(f"player_{self.player_id}_fireball_no_group"):
                print(f"Player {self.player_id}: Cannot fire fireball, projectile/all_sprites group not set.")
            return

        now = pygame.time.get_ticks()
        if now - self.fireball_cooldown_timer >= C.FIREBALL_COOLDOWN:
            self.fireball_cooldown_timer = now
            ...
            # Corrected method name from length_sq to length_squared
            ...
            offset_distance = (self.rect.width / 2) + (C.FIREBALL_DIMENSIONS[0] / 2) + -35
            if abs(current_aim_direction.y) > 0.8: 
                offset_distance = (self.rect.height / 2) + (C.FIREBALL_DIMENSIONS[1] / 2) + -35
            ...
            if current_aim_direction.length_squared() > 0:
                ...
                spawn_x += (1.0 if self.facing_right else -1.0) * offset_distance

            new_fireball = Fireball(spawn_x, spawn_y, current_aim_direction, self)
            self.projectile_sprites_group.add(new_fireball)
            self.all_sprites_group.add(new_fireball)
            
            if Player.print_limiter.can_print(f"player_{self.player_id}_fire_fireball_msg"):
                print(f"Player {self.player_id} fires fireball. Dir: {current_aim_direction}, Pos: ({spawn_x:.1f}, {spawn_y:.1f})")
        else:
            if Player.print_limiter.can_print(f"player_{self.player_id}_fireball_cooldown"):
                print(f"Player {self.player_id} fireball on cooldown.")
    ...
    def get_network_data(self):
        data = {'pos': (self.pos.x, self.pos.y), 'vel': (self.vel.x, self.vel.y), 'state': self.state,
    ...
    def set_network_data(self, data): 
        ...
        fb_aim_x = data.get('fireball_aim_x')
        fb_aim_y = data.get('fireball_aim_y')
        if fb_aim_x is not None and fb_aim_y is not None:
            self.fireball_last_input_dir.x = float(fb_aim_x)
            self.fireball_last_input_dir.y = float(fb_aim_y)
    ...
    def handle_network_input(self, input_data_dict): 
        ...
        net_aim_x = input_data_dict.get('fireball_aim_x')
        net_aim_y = input_data_dict.get('fireball_aim_y')
        if net_aim_x is not None and net_aim_y is not None:
            if float(net_aim_x) != 0.0 or float(net_aim_y) != 0.0:
                self.fireball_last_input_dir.x = float(net_aim_x)
                self.fireball_last_input_dir.y = float(net_aim_y)
        ...
        if input_data_dict.get('attack1_pressed_event', False) and can_act_net:
            ...
            attack_state_key = 'crouch_attack' if self.is_crouching else ('attack' if (is_trying_to_move_left_net or is_trying_to_move_right_net) else 'attack_nm')
            self.set_state(attack_state_key)
        if input_data_dict.get('attack2_pressed_event', False) and can_act_net:
            self.attack_type = 4 if self.is_crouching else 2 
            attack_state_key = 'crouch_attack' if self.is_crouching else ('attack2' if (is_trying_to_move_left_net or is_trying_to_move_right_net) else 'attack2_nm')
            self.set_state(attack_state_key)

        if input_data_dict.get('fireball_pressed_event', False) and can_act_net:
            self.fire_fireball() 
    ...
    def get_input_state(self, current_keys, current_events, key_map_for_player=None):
        ...
        player_fireball_key = None
        if self.player_id == 1: player_fireball_key = pygame.K_1
        elif self.player_id == 2: player_fireball_key = pygame.K_0 
        ...
        for event in current_events:
            if event.type == pygame.KEYDOWN:
                ...
                if player_fireball_key and event.key == player_fireball_key:
                    input_state['fireball_pressed_event'] = True
        return input_state