...
"""
# version 2.1.1 (Removed wall climb)
...
from assets import load_all_player_animations, load_gif_frames, resource_path # Ensure load_all_player_animations is still correct
...
class Player:
    ...
    def __init__(self, start_x: float, start_y: float, player_id: int = 1,
        ...
        if self.animations is None:
            critical(f"Player Init Error (ID: {self.player_id}): Failed loading animations. Player invalid.")
            self._valid_init = False
        ...
        # REMOVE wall_climb_timer and wall_climb_duration
        ...
        self.is_holding_climb_ability_key: bool = False # This can still be used for ladders
        ...
        if self._valid_init and self.animations:
            try:
                ...
                else: self.crouching_collision_height = self.standing_collision_height / 2.0
                ...
                if initial_idle_frames and initial_idle_frames[0] and not initial_idle_frames[0].isNull(): self.image = initial_idle_frames[0]
        ...
        if not self._valid_init:
            ...
            warning(f"Player {self.player_id}: Init failed.")
        else:
            self.last_anim_update = get_current_ticks_monotonic()
            debug(f"Player {self.player_id} initialized. Valid: {self._valid_init}.")

    # ... (_init_stone_assets, _assign_projectile_keys, _create_placeholder_qpixmap, _is_placeholder_qpixmap, _update_rect_from_image_and_pos remain the same)
    ...
    def _update_rect_from_image_and_pos(self, midbottom_pos_qpointf: Optional[QPointF] = None):
        ...
        if self.is_petrified and not self.is_stone_smashed: current_image = self.stone_crouch_image_frame if self.was_crouching_when_petrified else self.stone_image_frame
        elif self.is_stone_smashed and self.stone_smashed_frames:
            frame_idx_smashed = self.current_frame; smashed_frames_to_use = self.stone_crouch_smashed_frames if self.was_crouching_when_petrified else self.stone_smashed_frames
            if smashed_frames_to_use and len(smashed_frames_to_use) > 0: frame_idx_smashed = frame_idx_smashed % len(smashed_frames_to_use); current_image = smashed_frames_to_use[frame_idx_smashed]
            else: current_image = self.stone_image_frame
        img_w, img_h = 0.0, 0.0
        if current_image and not current_image.isNull(): img_w, img_h = float(current_image.width()), float(current_image.height())
        else:
            ...
            img_h = h_fallback; img_w = img_h * 0.5
        rect_x = target_pos.x() - img_w / 2.0; rect_y = target_pos.y() - img_h
        if not hasattr(self, 'rect') or self.rect is None: self.rect = QRectF(rect_x, rect_y, img_w, img_h)
        else: self.rect.setRect(rect_x, rect_y, img_w, img_h)
    ...
    def update_status_effects(self, current_time_ms: int):
        if self.is_aflame:
            if current_time_ms - self.aflame_timer_start > C.PLAYER_AFLAME_DURATION_MS: set_player_state(self, 'deflame_crouch' if self.is_crouching else 'deflame')
            elif C.PLAYER_AFLAME_DAMAGE_PER_TICK > 0 and current_time_ms - self.aflame_damage_last_tick > C.PLAYER_AFLAME_DAMAGE_INTERVAL_MS: self.take_damage(C.PLAYER_AFLAME_DAMAGE_PER_TICK); self.aflame_damage_last_tick = current_time_ms
        elif self.is_deflaming:
            if current_time_ms - self.deflame_timer_start > C.PLAYER_DEFLAME_DURATION_MS: set_player_state(self, 'crouch' if self.is_crouching else ('idle' if self.on_ground else 'fall'))
        if self.is_frozen:
            if current_time_ms - self.frozen_effect_timer > C.PLAYER_FROZEN_DURATION_MS: set_player_state(self, 'defrost')
        elif self.is_defrosting:
            if current_time_ms - self.frozen_effect_timer > (C.PLAYER_FROZEN_DURATION_MS + C.PLAYER_DEFROST_DURATION_MS): set_player_state(self, 'idle' if self.on_ground else 'fall')
    ...
    def petrify(self):
        ...
        debug(f"Player {self.player_id}: Petrifying."); self.facing_at_petrification = self.facing_right; self.was_crouching_when_petrified = self.is_crouching
        self.is_petrified = True; self.is_stone_smashed = False; self.is_dead = True; self.current_health = 0; self.vel = QPointF(0,0); self.acc = QPointF(0,0)
        self.is_attacking = False; self.is_dashing = False; self.is_rolling = False; self.is_sliding = False; self.on_ladder = False; self.is_taking_hit = False
        self.is_aflame = False; self.is_deflaming = False; self.is_frozen = False; self.is_defrosting = False; self.death_animation_finished = True
        set_player_state(self, 'petrified')
    def smash_petrification(self):
        if self.is_petrified and not self.is_stone_smashed:
            debug(f"Player {self.player_id}: Smashing petrification."); self.is_stone_smashed = True; self.stone_smashed_timer_start = get_current_ticks_monotonic()
            self.death_animation_finished = False; set_player_state(self, 'smashed')
    ...
    def can_stand_up(self, platforms_list: List[Any]) -> bool:
        ...
        current_feet_y = self.rect.bottom(); current_center_x = self.rect.center().x(); standing_width = self.rect.width()
        if self.animations and self.animations.get('idle') and self.animations['idle'][0] and not self.animations['idle'][0].isNull(): standing_width = float(self.animations['idle'][0].width())
        potential_standing_rect = QRectF(0, 0, standing_width, self.standing_collision_height); potential_standing_rect.moveBottom(current_feet_y); potential_standing_rect.moveCenter(QPointF(current_center_x, potential_standing_rect.center().y()))
        for platform_obj in platforms_list:
            if hasattr(platform_obj, 'rect') and isinstance(platform_obj.rect, QRectF) and potential_standing_rect.intersects(platform_obj.rect):
                if platform_obj.rect.bottom() > potential_standing_rect.top() and platform_obj.rect.top() < self.rect.top(): return False
        return True

    # Delegated methods
    def set_state(self, new_state: str): set_player_state(self, new_state)
    def animate(self): update_player_animation(self)
    def process_input(self, qt_keys_held_snapshot: Dict[Qt.Key, bool], qt_key_event_data_this_frame: List[Tuple[QKeyEvent.Type, Qt.Key, bool]], platforms_list: List[Any], joystick_data_for_handler: Optional[Dict[str, Any]] = None ):
        # (This method remains unchanged)
        ...
        if self.control_scheme == "keyboard_p1": active_mappings = game_config.P1_MAPPINGS
            ...
            if Player.print_limiter.can_print(f"p_input_scheme_fallback_{self.player_id}"): warning(f"Player {self.player_id}: Unrecognized control_scheme '{self.control_scheme}'. Using default map.")
    ...
    def _generic_fire_projectile(self, projectile_class: type, cooldown_attr_name: str, cooldown_const: int, projectile_config_name: str):
        ...
        if current_time_ms - last_fire_time >= cooldown_const:
            ...
            if Player.print_limiter.can_print(f"fired_{projectile_config_name}_{self.player_id}"): debug(f"Player {self.player_id} fired {projectile_config_name} at ({spawn_x:.1f},{spawn_y:.1f}) dir ({aim_dir.x():.1f},{aim_dir.y():.1f})")
            if projectile_config_name == 'blood' and self.current_health > 0: self.current_health -= self.current_health * 0.05;
    ...
    def update(self, dt_sec: float, platforms_list: List[Any], ladders_list: List[Any], hazards_list: List[Any], other_players_list: List[Any], enemies_list: List[Any]):
        # (This method remains unchanged)
        ...
        if self.is_stone_smashed:
            ...
            self.animate(); return
        if self.is_petrified: update_player_core_logic(self, dt_sec, platforms_list, ladders_list, hazards_list, other_players_list, enemies_list); return
    ...
    def draw_pyside(self, painter: QPainter, camera: Any):
        ...
        if painter.window().intersects(screen_rect_qrectf.toRect()): painter.drawPixmap(screen_rect_qrectf.topLeft(), self.image)

    # Player.reset_state() is GONE. Resetting player attributes is now handled by
    # game_state_manager._reset_player_attributes_internal()