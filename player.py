...
MODIFIED: Zapped GIF path added to _init_stone_assets (conceptually, should be renamed).
"""
# version 2.1.12 (Zapped GIF Path)
...
class Player:
    ...
    def __init__(self, start_x: float, start_y: float, player_id: int = 1,
        ...
        self.is_zapped: bool = False # Added
        self.zapped_timer_start: int = 0 # Added
        self.zapped_damage_last_tick: int = 0 # Added
        self.zapped_gif_path: Optional[str] = None # Added
        ...
        self._init_common_status_assets(asset_folder) # MODIFIED to pass asset_folder
    ...
    def _init_common_status_assets(self, player_asset_folder: str): # MODIFIED parameter
        ...
        def load_or_placeholder(path_suffix, default_placeholder_color, default_placeholder_text, is_list=False, asset_folder_override: Optional[str] = None):
            anim_key_fallback: Optional[str] = None
            is_zapped_asset = "__Zapped.gif" in path_suffix

            if "Stone.png" in path_suffix: anim_key_fallback = 'petrified'
            elif "Smashed.gif" in path_suffix: anim_key_fallback = 'smashed'
            elif "StoneCrouch.png" in path_suffix: anim_key_fallback = 'petrified'
            elif "StoneCrouchSmashed.gif" in path_suffix: anim_key_fallback = 'smashed'
            elif is_zapped_asset: anim_key_fallback = 'zapped'
            ...
            if anim_key_fallback and self.animations:
                ...
                if player_specific_frames and not self._is_placeholder_qpixmap(player_specific_frames[0]):
                    debug(f"Player {self.player_id} StatusAsset: Using player's own '{anim_key_fallback}' anim for '{path_suffix}'.")
        ...
        # Load Zapped GIF - this was missing from the original request's logic structure
        self.zapped_gif_path = os.path.join(player_asset_folder, "__Zapped.gif") # Use player's own zapped GIF
        # Note: The actual loading of this into self.animations['zapped'] is handled by load_all_player_animations.
        # This line in _init_common_status_assets is more for record-keeping or if a direct path was needed here.
        # It does not directly load the zapped animation into self.animations here, that happens in __init__.
        if not os.path.exists(resource_path(self.zapped_gif_path)):
             warning(f"Player {self.player_id}: Zapped GIF not found at expected player-specific path '{self.zapped_gif_path}'. "
                     "The 'zapped' animation in self.animations might be a placeholder if load_all_player_animations also failed for it.")
    ...
    def update_status_effects(self, current_time_ms: int) -> bool:
        ...
        if (self.is_aflame or self.is_deflaming) and \
           ...
           self.overall_fire_effect_start_time > 0:
            ...
            if current_time_ms - self.overall_fire_effect_start_time > 5000:
                ...
                self.set_state('idle', current_time_ms)
        ...
        # --- Zapped Effect ---
        if self.is_zapped: # MODIFIED: Add zapped check
            if current_time_ms - self.zapped_timer_start > C.PLAYER_ZAPPED_DURATION_MS:
                self.set_state('idle' if self.on_ground else 'fall', current_time_ms)
            # Optional: Apply zapped damage if PLAYER_ZAPPED_DAMAGE_PER_TICK > 0
            # Currently set to 0 in constants, so this block would be skipped.
            elif C.PLAYER_ZAPPED_DAMAGE_PER_TICK > 0 and \
                 current_time_ms - self.zapped_damage_last_tick > C.PLAYER_ZAPPED_DAMAGE_INTERVAL_MS:
                if hasattr(self, 'take_damage'):
                    self.take_damage(C.PLAYER_ZAPPED_DAMAGE_PER_TICK)
                self.zapped_damage_last_tick = current_time_ms
            return True # Zapped state overrides other logic


        if self.is_frozen:
            if self.is_aflame or self.is_deflaming:
                if hasattr(self, 'print_limiter') and self.print_limiter.can_log(f"frozen_cancels_fire_{self.player_id}"):
            ...
            if current_time_ms - self.frozen_effect_timer > C.PLAYER_FROZEN_DURATION_MS:
                self.set_state('defrost', current_time_ms)
            return True 
        ...
        if self.is_aflame:
            if current_time_ms - self.aflame_timer_start > C.PLAYER_AFLAME_DURATION_MS:
                ...
                self.set_state('deflame_crouch' if self.is_crouching else 'deflame', current_time_ms)
            ...
            if current_time_ms - self.deflame_timer_start > C.PLAYER_DEFLAME_DURATION_MS:
                ...
                self.set_state(player_state_on_deflame_end, current_time_ms)
    ...
    def apply_zapped_effect(self): # Added
    ...
    def petrify(self):
        if self.is_petrified or (self.is_dead and not self.is_petrified) or self.is_zapped:
            if hasattr(self, 'print_limiter') and self.print_limiter.can_log(f"petrify_blocked_{self.player_id}"):
                debug(f"Player {self.player_id}: petrify called but already petrified, truly dead, or zapped. Ignoring.")
            return
