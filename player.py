...
"""
# version 2.1.0 (Removed Player.reset_state(), player reset handled globally)
...
class Player:
    ...
    def __init__(self, start_x: float, start_y: float, player_id: int = 1,
        ...
        # All player attributes are now initialized/reset by game_state_manager._reset_player_attributes
        # We still need to set them to default values here for type hinting and initial object creation.
        ...
        # --- Initialize all attributes to default/empty states ---
        ...
        if self._valid_init and self.animations:
            # ... (collision height setup as before) ...
            try:
        ...
        if not self._valid_init: # Final check after all init steps
            self.is_dead = True; self._alive = False; self.current_health = 0
            warning(f"Player {self.player_id}: Initialization concluded with _valid_init as False.")
        else:
            self.last_anim_update = get_current_ticks_monotonic()
            debug(f"Player {self.player_id} initialized. Valid: {self._valid_init}. Pos: ({self.pos.x():.1f},{self.pos.y():.1f})")
    ...
    def process_input(self,
        ...
        if self.control_scheme == "keyboard_p1": active_mappings = game_config.P1_MAPPINGS
            ...
            else: active_mappings = game_config.DEFAULT_GENERIC_JOYSTICK_MAPPINGS
        else:
    ...
    # reset_state METHOD IS NOW REMOVED FROM PLAYER CLASS
    # Player attributes are reset by game_state_manager._reset_player_attributes()