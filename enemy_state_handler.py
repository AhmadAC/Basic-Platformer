...
# DO NOT do top-level import of enemy_animation_handler here
# try:
...
def set_enemy_state(enemy: Any, new_state: str):
    ...
    try:
        ...
        critical("ENEMY_STATE_HANDLER (set_enemy_state): Failed to import update_enemy_animation locally.")
        def update_enemy_animation(enemy_arg: Any): pass # Fallback dummy for this scope
    ...
    if current_state == new_state and new_state not in ['hit', 'attack', 'attack_nm', 'zapped']: # Added zapped as re-triggerable
    ...
    enemy.state_timer = current_time_ms
    ...
    if new_state == 'idle':
        ...
        # Immobilize when entering frozen state
        if hasattr(enemy, 'vel'): enemy.vel.setX(0); enemy.vel.setY(0)
        if hasattr(enemy, 'acc'): enemy.acc.setX(0); enemy.acc.setY(0)
    elif new_state == 'defrost':
        enemy.is_defrosting = True
        enemy.is_frozen = False
        # Movement still restricted during defrost
        if hasattr(enemy, 'vel'): enemy.vel.setX(0); enemy.vel.setY(0)
        if hasattr(enemy, 'acc'): enemy.acc.setX(0); enemy.acc.setY(0)
    elif new_state == 'zapped': # New state handling
        enemy.is_zapped = True
        enemy.zapped_timer_start = current_time_ms
        enemy.zapped_damage_last_tick = current_time_ms
        # Immobilize when entering zapped state
        if hasattr(enemy, 'vel'): enemy.vel.setX(0); # Y might still be affected by gravity if airborne
        if hasattr(enemy, 'acc'): enemy.acc.setX(0);
        if getattr(enemy, 'on_ground', False): # If on ground, stop Y movement too
             if hasattr(enemy, 'vel'): enemy.vel.setY(0)
             if hasattr(enemy, 'acc'): enemy.acc.setY(0)
    elif new_state == 'petrified':
        enemy.is_petrified = True
        enemy.facing_at_petrification = enemy.facing_right # Store facing for static stone image
        # Immobilize
        if hasattr(enemy, 'vel'): enemy.vel.setX(0); enemy.vel.setY(0)
        if hasattr(enemy, 'acc'): enemy.acc.setX(0);
        if not getattr(enemy, 'on_ground', True) and hasattr(enemy, 'acc'): # Allow gravity if petrified mid-air
            enemy.acc.setY(float(getattr(C, 'ENEMY_GRAVITY', 0.7)))
    elif new_state == 'smashed':
        enemy.is_stone_smashed = True
        enemy.smashed_timer_start = current_time_ms # For animation/effect duration
        enemy.is_dead = True # Smashed means defeated
        enemy.death_animation_finished = False # Smashed animation needs to play
        if hasattr(enemy, 'vel'): enemy.vel.setX(0); enemy.vel.setY(0) # Stop movement
        if hasattr(enemy, 'acc'): enemy.acc.setX(0); enemy.acc.setY(0)
    elif new_state == 'stomp_death':
        # Most flags (is_stomp_dying, is_dead, timers) are set by stomp_kill_enemy in status_effects
        # This state entry ensures animation is cued.
        pass
