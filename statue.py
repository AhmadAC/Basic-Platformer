...
"""
# version 2.1.3 (Crouched/Standing variant handling)
import os
...
class Statue:
    def __init__(self, center_x: float, center_y: float, statue_id: Optional[Any] = None,
                 initial_image_path_override: Optional[str] = None, # Renamed for clarity
                 smashed_anim_path_override: Optional[str] = None,  # Renamed for clarity
        ...
        # Use overrides if provided, otherwise use determined base paths
        ...
        # Load initial stone image (standing or crouched)
        ...
        # Load smashed animation (standing or crouched variant)
        ...
        self.pos = QPointF(float(center_x), float(center_y)) # Position is visual center
        ...
        self.is_dead = False # Becomes true when smashed
        self.death_animation_finished = False # True when smashed animation ends
        self._alive = True # False when truly "gone" from game

        self.vel = QPointF(0.0, 0.0)
        self.on_ground = False
        self._gravity = float(getattr(C, 'PLAYER_GRAVITY', 0.7))

        # Ensure properties from editor like destructible, health, can_be_pushed are used
        self.max_health = int(self.properties.get("health", 3)) # Default statue health
        self.current_health = self.max_health
        self.can_be_pushed = self.properties.get("can_be_pushed", False) # Statues usually aren't pushed unless specified
        self.push_resistance = float(self.properties.get("push_resistance", 10.0)) # Very high by default

    def _is_placeholder_qpixmap(self, pixmap: QPixmap) -> bool:
        if pixmap.isNull(): return True
        if pixmap.size() == QSizeF(30,40): # A common placeholder size
    ...
    def dimensions(self) -> QSizeF:
        ...
        # Fallback dimensions if image is bad
        is_crouched_form = self.properties.get("is_crouched_variant", False) or \
                           self.properties.get("was_crouching", False)
    ...
    def kill(self):
        ...
        self._alive = False
        self.is_dead = True # Ensure is_dead reflects the killed state
        self.death_animation_finished = True # Ensure death animation is marked finished
    ...
    def smash(self):
        if not self.is_smashed and self._valid_init and self._alive:
            ...
            self.is_dead = True # Smashed means defeated
            ...
            else: # Fallback if smashed_frames are bad
            ...
            # Smashed statues stop moving and become non-solid for platform checks
            self.vel = QPointF(0.0, 0.0)
            self.on_ground = True # Effectively static once smashed

    def take_damage(self, damage_amount: int):
        if not self.is_smashed and self._valid_init and self._alive:
            # Statues might have health from properties, but for now, any damage smashes them
            if self.properties.get("destructible", True): # Default to destructible
                health_property = self.properties.get("health", 1) # Default to 1 HP
                self.current_health -= damage_amount
                if self.current_health <= 0:
                    debug(f"Statue {self.statue_id} took {damage_amount} damage, health depleted. Smashing.")
                    self.smash()
                else:
                    debug(f"Statue {self.statue_id} took {damage_amount} damage. Health remaining: {self.current_health}")
            else:
                 debug(f"Statue {self.statue_id} took {damage_amount} damage, but is not destructible.")
    ...
    def apply_physics_step(self, dt_sec: float, platforms_list: List[Any]):
        ...
        if not self.on_ground:
            self.vel.setY(self.vel.y() + self._gravity) # Gravity is per physics update
            self.vel.setY(min(self.vel.y(), getattr(C, 'TERMINAL_VELOCITY_Y', 18.0)))
        
        # Velocity is "per physics tick", so for position update, it's already scaled.
        # If using game_loop_dt*C.FPS scaling for pos, vel should be units/second.
        # For now, assuming vel is "displacement per physics tick"
        # self.pos.setY(self.pos.y() + self.vel.y()) 
        # Let's match player physics: vel is per physics tick, game loop uses scaled_vel for position
        scaled_vel_y_for_pos_update = self.vel.y() #* dt_sec * C.FPS (If vel was units/sec)
                                                  # if vel is already "per update", then just vel.y()
                                                  # Let's assume vel is "displacement per update"
        ...
        for platform_obj in platforms_list:
            if isinstance(platform_obj, Statue) and platform_obj is self:
                continue # Don't collide with self if somehow in platforms_list
            if isinstance(platform_obj, Statue) and platform_obj.is_smashed: # Don't land on smashed statues
                continue
            if not hasattr(platform_obj, 'rect') or not isinstance(platform_obj.rect, QRectF):
                continue
            if self.rect.intersects(platform_obj.rect):
                if self.vel.y() >= 0: # Moving downwards or still
                    ...
                    if overlap_y > 0:
                        ...
                        if actual_h_overlap >= min_h_overlap_pixels and prev_bottom_y_estimate <= platform_obj.rect.top() + 1.0:
                            ...
                            self.pos.setY(self.rect.center().y() - self.rect.height()/2 + self.rect.height()) # Recalc pos from rect if midbottom assumed
                            self.pos.setY(self.rect.center().y()) # If pos is visual center
                            self.vel.setY(0.0)
                            self.on_ground = True
                            break
        self._update_rect_from_image_and_pos() # Ensure rect matches final pos
    ...
    def update(self, dt_sec: float = 0.0):
        ...
        if self.is_smashed:
            if not self.death_animation_finished:
                ...
                elif self.smashed_frames and len(self.smashed_frames) > 1: # Animated smash
                ...
                elif self.smashed_frames and len(self.smashed_frames) == 1: # Single frame for smashed
                     if self.image != self.smashed_frames[0] and self.smashed_frames[0] and not self.smashed_frames[0].isNull():
                        self.image = self.smashed_frames[0]
                        self._update_rect_from_image_and_pos()
                        image_updated_this_frame = True
            # else: death_animation_finished is True, waiting to be killed by main loop
        
        # If NOT smashed AND image hasn't been updated by smashed logic, set to initial image if different
        if not self.is_smashed and not image_updated_this_frame:
            if self.initial_image_frames and self.initial_image_frames[0] and not self.initial_image_frames[0].isNull():
                 if self.image != self.initial_image_frames[0]: # Only update if different
                    self.image = self.initial_image_frames[0]
                    self._update_rect_from_image_and_pos()
                    self.current_frame_index = 0 # Reset frame for initial state
            else: # Fallback if initial_image_frames are bad
        ...
        if self.is_dead and self.death_animation_finished and self._alive:
            self.kill() # Calls self._alive = False
    ...
    def get_network_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pos': (pos_x, pos_y), # Send visual center
            'vel_y': vel_y_val,
            ...
            '_alive': self._alive, # Send _alive status
            'is_dead': self.is_dead, # Send is_dead for consistency
            'death_animation_finished': self.death_animation_finished,
            'properties': self.properties.copy(), # Send properties for variant reconstruction
        }

    def set_network_data(self, data: Dict[str, Any]):
        self._valid_init = data.get('_valid_init', self._valid_init)
        self._alive = data.get('_alive', self._alive) # Sync alive state
        
        if not self._valid_init or not self._alive: # If not valid or not alive, no further state processing
            return
        ...
        # Sync properties first as they might influence visual choice
        ...
        if new_is_smashed != self.is_smashed:
            ...
            if self.is_smashed:
                ...
                self.death_animation_finished = False # Start smash animation
                ...
                self.last_anim_update = get_current_ticks_monotonic() # Reset anim timer for smash
        ...
        # Determine which set of frames to use based on properties (for variant reconstruction)
        is_crouched_variant_net = self.properties.get("is_crouched_variant", False) or \
                                  self.properties.get("was_crouching", False)
        
        # Re-select base image paths based on (potentially updated) properties
        stone_asset_folder = os.path.join('characters', 'Stone')
        base_initial_filename_net = "__StoneCrouch.png" if is_crouched_variant_net else "__Stone.png"
        base_smashed_filename_net = "__StoneCrouchSmashed.gif" if is_crouched_variant_net else "__StoneSmashed.gif"
        
        # Only reload frames if the variant type *changed* due to network update
        # This assumes __init__ already loaded the correct variant based on initial properties.
        # If `properties` from network causes a variant change, frames need reload.
        # For simplicity, if `properties` are part of network data, we might always re-check paths.
        # This is a bit heavy; ideally, only reload if variant-determining props changed.
        # For now, assume __init__ handled the primary visual variant load, and set_network_data
        # mainly syncs state (is_smashed, frame_index) for that variant.
        
        # Correct image selection based on current state
        if self.is_smashed:
            ...
            self.current_frame_index = 0 # Initial stone is usually single frame
            if self.initial_image_frames and self.initial_image_frames[0] and not self.initial_image_frames[0].isNull():
                 self.image = self.initial_image_frames[0]
            else:
    ...
    def platform_type(self) -> str:
        # This is used by game_setup when adding statues to platforms_list.
        # If smashed, it shouldn't be treated as a standard platform.
        # However, `is_smashed` is checked directly in collision handlers now.
        # This property can remain for general typing if needed elsewhere.
    ...
    def draw_pyside(self, painter: QPainter, camera: Any):
        ...
        # Draw if it's "alive" in the sense of being visible (not yet fully killed and removed by game loop)
        # OR if it's smashed and its death animation (smash effect) is not finished.
